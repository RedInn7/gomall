...
`pkg/web3/contracts/Escrow.sol` 的核心是这个状态机：
...
- **审计 + 单测**：见 `pkg/web3/contracts/Escrow.test.txt`，每一条状态转移都有正反两条 case
...
`pkg/web3/escrow/` 当前是占位实现（避免一个 100MB 的 go-ethereum 依赖进业务模块）。真正上链时按 `pkg/web3/contracts/README.md` 跑：

```bash
solc --abi --bin --optimize pkg/web3/contracts/Escrow.sol -o build/
...
- 合约预期行为：`pkg/web3/contracts/Escrow.test.txt`，列出 A-G 七组场景共 22 条 case（部署、fund、release、refund、dispute、资金安全、事件）
...
- 合约：`pkg/web3/contracts/Escrow.sol`
- 合约文档：`pkg/web3/contracts/README.md`
- 合约预期行为：`pkg/web3/contracts/Escrow.test.txt`
