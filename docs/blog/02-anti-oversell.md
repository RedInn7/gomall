...
`internal/coupon/repo.go` 的 `ClaimWithDBLock`：
...
_, err = NewCouponDao(ctx).PersistClaim(userId, batchId, batch.ValidDays)
if err != nil {
...
- DB 锁实现：`internal/coupon/repo.go` 的 `ClaimWithDBLock`
- 模式切换：`internal/coupon/service.go` 看 `req.Mode`
