...
- 异步下单 service：`internal/order/async.go`
- 异步消费 service：`internal/order/consumer.go`
- RMQ 拓扑：`repository/rabbitmq/order_async.go`
- 启动接线：`initialize/order_async.go`、`cmd/main.go`
- HTTP 入口：`internal/order/handler.go`（`EnqueueOrderHandler` / `OrderStatusHandler`）
- 路由：`routes/router.go`
- 单元测试：`internal/order/async_test.go`
