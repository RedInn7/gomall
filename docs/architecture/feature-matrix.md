...
文档分支，主干为 `main`。每条 feature 都给出关键代码入口与配套 Markdown 文档；
课程顺序以 `docs/lecture/README.md` 为准，主题文章位于 `docs/blog/`。
...
| 库存预热 + Redis Lua 原子扣减（两桶） | ✅ | `repository/cache/inventory.go` ReserveStock/Commit/Release | lecture 10 |
| IP 维度令牌桶限流 | ✅ | `middleware/ratelimit.go` TokenBucket | lecture 12-13 |
| 用户维度滑动窗口限流 | ✅ | `middleware/ratelimit.go` SlidingWindow | lecture 12-13 |
| 三态熔断器 | ✅ | `middleware/circuitbreaker.go` | lecture 03 / 12 |
| 幂等中间件 (Idempotency-Key) | ✅ | `middleware/idempotency.go` | blog 01 |
| Outbox 模式 (PR #57) | ✅ | `service/outbox/publisher.go` | blog 04 |
| Saga 取消订单 (PR #58) | ✅ | `service/order_cancel.go` | blog 04 |
| RMQ TTL 延迟队列 (30min 关单) | ✅ | `repository/rabbitmq/order_delay.go` | lecture 00（下） |
| Cron 兜底关单 | ✅ | `service/order_task.go`, `initialize/cron.go` | lecture 00（下） |
| 缓存一致性 (Cache Aside + 双删) | ✅ | `repository/cache/product.go` | blog 03 |
| 游标分页 (订单列表) | ✅ | `repository/db/dao/order.go` | lecture 00（上） |
| 商品搜索 ES + outbox 增量索引 (PR #59) | ✅ | `service/search/`, `repository/es/` | lecture 05-06 |
...
- `docs/lecture/README.md`——唯一课程顺序与全部 Markdown 讲义
