...
| -------------------- | -------------------------------------- | -------------------------- | ------------------------------ | ------------------------------------------ | ------------------------------------------------------------ |
| **召回 (Recall)**    | **查全**（宁可多找，不能漏找）         | 速度快、覆盖广、低延时     | 千万级 $\rightarrow$ 几千/几百 | 倒排索引、向量检索 (KNN)、多路召回         | 搜“苹果”，把所有含“苹果”的手机、电脑、手机壳、水果全部捞出来 |
| **过滤 (Filtering)** | **合规/硬约束**（非黑即白，一票否决）  | 业务规则、准确性           | 几千 $\rightarrow$ 几百        | 布尔查询、BitSet、Tag 过滤                 | 剔除已下架、无库存、未通过审核、地域不配送的商品             |
| **排序 (Ranking)**   | **查准/转化**（让最想买/最相关的在前） | 商业价值、用户意图、精准度 | 几百 $\rightarrow$ 几十        | BM25、LTR (Learning to Rank)、CTR 预估模型 | 手机排在手机壳前面、销量/好评度高的排在前面                  |
| **降级 (Fallback)**  | **可用性/保底**（服务挂了也要有响应）  | 系统高可用、用户体验兜底   | 兜底固定数量                   | 缓存兜底、MySQL 模糊降级、静态热门兜底     | ES 挂了时，读 Redis 热门数据或走 MySQL `like` 查基础列表     |

项目创建名为 `product` 的索引。`name`、`title` 和 `info` 使用 `text` 类型（写入时，内容会被分词器拆分成一个个词项（Term）存入倒排索引）与 `standard` analyzer（默认分词器）；`category_id`、`num`、`boss_id` 等保存结构化数据，价格目前以 `keyword` 保存（**不进行全文分词**，只用于**精确匹配** ）。
