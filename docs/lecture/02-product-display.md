...
> gomall · 转化漏斗最宽一跳、首屏 1 秒、三层缓存与深分页 COUNT 取舍
>
> 商品展示是转化漏斗最宽的一跳：用户必然经过、必然以毫秒计耐心、必然有商家变更冲突。这份讲义用 70% 的篇幅讲"为什么这么做"，代码只占 30%。核心命题只有一句——**商品展示不是"把数据从 DB 取出来"，是"把流量挡在 DB 之外"**。
...
同一套商品展示代码，五个角色各自盯着不同的指标：

| 角色 | 关心什么 | 商品展示影响 |
|---|---|---|
| C 端用户 | 搜得到 / 看得清 / 不卡 | 详情页 RT 100ms = 跳出率 +2%；首屏 > 3s 流失 90% |
...
| SRE | 双 11 零点能不能扛 100k QPS | 99% Redis 命中 = DB 实际只看 320 QPS，单机够用 |

商品展示是**转化漏斗最宽的一跳**：用户必然经过、必然以毫秒计耐心、必然有商家变更冲突。

### 用户旅程视图：详情页是漏斗的腰

```mermaid
flowchart LR
    login["登录<br/>DAU 100%"] --> browse["浏览/详情<br/>约 80%"]
    browse --> cart["加购<br/>约 15%"]
...
| 列表/类目 p99 延迟 | P3 信息类 | < 1s | 列表 2.5s（含全表 COUNT） |
| 可用率 | P1 | 99.9% | N/A |
...
商品列表 24.5 RPS / p95 2.5s 在 P3 信息类口径下**未触发事故**（当前 QPS 远低于阈值），其延迟来自 `COUNT(*)` 全表计数这一已知特性；提速模板（游标分页）已在订单列表落地，可直接平移。

### 业务边界：这一节不解决什么

摊牌不丢人。把不做什么写清楚，比把做了什么写得花哨更值钱：

- **不做 SKU 多规格详情**：当前商品 model 是单 SKU，没有"颜色/尺码"维度。
- **不做视频 / 直播流**：详情页只有 `ImgPath` 单图，视频解码 / 切片 / CDN 调度是独立链路。
- **没有 AB 测试框架**：双 max-age 切流量、内容个性化、灰度发布都需要外置 AB 平台。
- **没有个性化推荐**：详情页底部"猜你喜欢"位是空的，路线图依赖召回 + 排序服务。
- **商品搜索召回**：见 deck 03（ES + Milvus hybrid），不在本 deck。
- **评价 / 问答 / 售后入口**：见 deck 09，路线图阶段。

---

## 二、首屏 1 秒决定订单

这一节回答四个问题：详情页首屏黄金 1 秒缓存命中 vs 回源差多少；为什么业务**必须接受**最终一致而非强一致；HTTP cache 哪几个接口能挂、TTL 怎么定；`product/list` 的 24.5 RPS / p95 2.5s 从业务侧怎么解读。
...
> **"回源"不可怕，"扫表"才可怕。** `/product/show` 走主键 + 缓存兜底，业务侧从来不是瓶颈；`/product/list` 的延迟全部来自 `COUNT(*)` 全表计数，正是[第七节](#七深分页-count-的工程取舍)要量化拆解的对象。
...
- **SRE**：`product/list` 24.5 RPS / p95 2.5s 是 **1M 行表上 `COUNT(*)` 全表计数的固有代价**，不是 bug。
...
| `/product/list`（含 COUNT） | **24.5** | **2.50s** |

同样是商品域的 GET 接口，吞吐差 **2,500 倍**，p95 差 **800 倍**。区别只有一个：`ShowProduct` 走主键，`ListProducts` 多了一个 `SELECT COUNT(*)`。
...
物化视图临界点：**QPS > 1k && 类目稳定**。gomall 当前 24.5 RPS 没到，先建索引。无限滚动是最优解：`order/list` 已用游标 + 缓存把 QPS 从 8.3 拉到 **58,406**（约 7000 倍），商品列表可复用同样模板：cursor = `(sort_key, id)` 二元组。
...
A：P1 信息类 99.9% / p99 < 200ms，实测 p95 3ms 有 60 倍余量。事故触发：p99 > 200ms 持续 5min，或可用率跌破 99.9%。商品列表 24.5 RPS / p95 2.5s 当前**不算事故**（P3 上限 1s 但 QPS 远低于阈值），延迟来自全表 `COUNT(*)`，QPS 增长前可平移游标分页模板提速。
