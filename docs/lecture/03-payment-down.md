...
1. **重复请求不可避免。** 客户端没有办法仅凭一次超时，判断服务端到底有没有提交事务。
2. **Redis 负责减少重复执行，数据库负责守住资金结果。** 幂等记录可以丢，订单状态和流水唯一约束不能跟着失效。
3. **熔断只统计系统故障。** 密码错误、余额不足不是系统坏了，不能拿来触发熔断。
4. **commit 是处理方式的分界线。** commit 前失败可以回滚；commit 后失败只能回放、重试、补偿或对账。

后面的中间件顺序、幂等状态机、熔断和故障演练，都是在回答这四件事。

### 90 秒现场：commit 成功，响应却丢了
...
sequenceDiagram
    ...
    H-->>C: 写出 HTTP 响应
    H-->>I: handler 返回
    I->>I: 保存 done；失败则重试并回退 init
```

这里故意把 HTTP 响应画在 `done` 前面。当前 `responseRecorder` 会把响应继续写给底层 Writer，`c.Next()` 返回后，中间件才提交幂等结果。因此客户端已经看到成功、Redis 还没写成 `done` 的窗口真实存在。
...
account_transaction 有 (ref_order_id, direction, biz_type) 唯一索引
```
...
```text
Redis 幂等：减少重复进入业务
订单状态：防止同一订单被不同渠道重复推进
流水唯一索引：防止同一业务类型、同一方向重复入账
```

`biz_type` 是唯一键的一部分，所以余额、Stripe 和 Web3 三种渠道不会互相撞这个索引。跨渠道并发结算时，最后一道共同防线仍然是订单必须处于 `WaitPay`，并且条件更新只能影响一行。
...
当前余额支付的 `/paydown` 主要访问本地数据库，路由上的熔断器保护的也是这条链路。Stripe 已经接入独立的 `/paydown/stripe` 和 webhook，但它们目前没有挂这个熔断器；以后接支付宝、微信或给 Stripe 增加外部调用保护时，需要按具体依赖分别设置熔断策略：
...
这里还有一个运行前提：`outbox_event` 表必须已经建好。当前启动迁移没有包含 `outbox.OutboxEvent`；全新数据库如果没有手工建表，事务会在插入 outbox 时直接回滚，还到不了“commit 后由 publisher 重试”这一步。正式演示前要先补齐迁移。
...
2. **订单和流水是否一致**：余额、Stripe、Web3 已支付订单是否分别有 `order_pay`、`stripe_pay`、`web3_pay` 流水；
...
这条 SQL 不是完整的资金系统，但它足以说明：流水不是日志，而是可以查询、比对和追责的业务证据。

---

## 六、故障演练：断线和崩溃会留下不同状态

**客户端断线和服务端崩溃必须分开注入，否则会把 `done` 和 `processing` 混成同一种故障。**
...
3. 重新颁发一个有效 key 再提交，确认 `WaitPay` 条件更新会挡住跨渠道重复结算，同一 `biz_type` 的流水唯一索引会再挡一层同渠道重放。

完成两组实验后，回答：客户端断线和服务端崩溃为什么得到不同的幂等状态？换成新 key 后，数据库靠什么守住资金？
...
当前实现遇到 Redis 调用错误会直接拒绝请求。更值得检查的是：旧记录过期或用户换了一个有效 key 后，重复请求仍可能进入 handler。订单的 `WaitPay` 条件更新必须挡住跨渠道重复结算，流水唯一索引继续挡住同一 `biz_type`、同一方向的重复入账。
...
客户端会重试。若幂等结果已保存，直接回放；若没保存，重复请求会再次进入业务。订单状态守卫负责挡住跨渠道重复结算，流水唯一约束负责挡住同一业务类型的重复入账。
...
3. 写 SQL 找出余额支付成功、但缺少 `order_pay` 流水的订单；再把查询扩展到 `stripe_pay` 和 `web3_pay`。
