...
**答案提示：** 当前实现中，“只传 `name`”和“指定 `category_id`”会暴露最明显的路径差异。测试应该先固定现状，再由团队决定理想契约；否则直接修改 mock 期望，容易把缺陷藏起来。
...
**答案提示：** 事务中的商品 DAO 和 Outbox DAO 必须绑定同一个 `*gorm.DB`；Outbox 插入失败应让事务回滚。发布 RabbitMQ 消息仍在事务提交之后由后台进程完成，不能把网络发送塞进数据库事务。
