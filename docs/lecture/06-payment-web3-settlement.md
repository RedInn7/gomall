...
## 时间安排（约 49 分钟，最长不超过 60 分钟）

| 时间 | 内容 | 课堂产出 |
|---|---|---|
| 0–4 分钟 | 承接上一讲 | 判断“看见日志”是否等于到账 |
| 4–17 分钟 | 确认深度、订阅与回扫 | 算出本轮安全扫描区间 |
| 17–25 分钟 | 日志解码、去重与 outbox | 找出一个漏事件窗口 |
| 25–36 分钟 | 消费者校验与错误分流 | 区分重试和 DLQ |
| 36–44 分钟 | 数据库结算事务 | 解释幂等落点 |
| 44–49 分钟 | 可靠性边界与演示 | 给事故选择恢复入口 |

预留 11 分钟给代码停顿和提问。不会再展开钱包签名，也不讲合约 release、refund、dispute 状态机。
