...
| 段落 | 时间 | 讲什么 |
|---|---:|---|
| 一、支付链路的四个判断 | 4 min | 结论和 90 秒订单状态回顾 |
| 二、“钱扣了，订单没变”是怎么发生的 | 3 min | 用客诉现场解释事务边界 |
| 三、订单决定谁付钱、付多少 | 8 min | 订单归属、状态、金额口径 |
| 四、两个账户必须一起改 | 9 min | 行锁、锁序、支付密码、余额扣减 |
| 五、流水让每一分钱可追查 | 7 min | debit / credit、唯一索引、对账 |
| 六、支付成功后怎样推进订单 | 5 min | 库存、状态机、outbox |
| 七、MySQL 提交后怎样处理 Redis | 2 min | best effort 与对账边界 |

Stripe 和 Web3 不塞进这一集。它们的“资金从哪里来”不同，但订单结算尾段复用同一套规则，放到后面的支付通道讲。

表内内容共 38 分钟，另外留 2 分钟给开场和转场。五角色风险表扫一遍即可，不逐行展开。

正片只逐行走读五处代码：`PaymentDownReq`、`orderPayableCents`、`LockTwoUsersForUpdate`、两条流水写入和 `MarkOrderPaidWithCheck`。其余代码块用来课后复查，录制时只点出输入、输出和事务位置；附录不录。
...
支付从一张 `WaitPay` 订单开始。订单里已经固定了买家、卖家、商品、数量和优惠后的应付金额，库存也已经完成预留。本讲把这些结果当作输入，不展开购物车和下单过程；第五讲再解释订单怎样生成、库存怎样预留。
