...
![Gomall Hybrid Search 业务流程](./assets/hybrid-search-flow.png)
...
商品明明符合用户需求，却可能因为没有出现相同关键词而无法被搜到。这不是排序问题：商品如果没有进入候选集，后续的排序算法再好也无济于事。
...
这套方案保留 Elasticsearch，并让两种搜索分别处理各自擅长的问题：
...
| 术语 | 本章中的含义 |
| --- | --- |
| 召回（Recall） | 从全部商品中先找出一批候选，还不是最终排序 |
| Embedding | 一段文本对应的浮点数数组；本项目固定为 768 维 |
| Score | 相关度得分，通常越大越相关，例如 ES `_score` |
| Distance | 两个向量之间的距离；L2 distance 越小越接近 |
...
系统中有 1000 万件商品，不可能对每件商品都执行复杂计算后再排序。一次搜索通常要经过：

```text
用户 Query → 召回 → 计算相关度 → 排序 → TopK → 返回结果
```

### 召回（Recall）

召回是从海量商品中快速找出一小批可能相关的候选。例如：
...
召回解决的是“先从几千万件商品中快速找出哪些候选”。

### 关键词召回：倒排索引 + BM25

这是搜索系统中最经典的召回方式。用户搜索：
...
因此，如果项目中出现以下关键词，通常说明系统正在执行关键词召回：
...
如果两个向量很接近，就可以认为两段文本的语义可能比较接近。

Embedding 将文本相似度问题转换为数学上的向量相似度问题：
...
以常见的 L2 Distance（欧氏距离）为例：

$d(x,y)=\sqrt{\sum_{i=1}^{768}(x_i-y_i)^2}$

它本质上是二维空间中“两点之间的直线距离”在 768 维空间中的延伸。
...
| Score | 相关度分数，通常越大越好 |
| Distance | 向量之间的距离，L2 通常越小越好 |
| TopK | 最终只保留前 K 名 |

最容易混淆的是召回和 TopK：召回负责先找出候选，TopK 负责从排序结果中保留前 K 个。
...
这一层负责把前端搜索请求送入后端搜索服务，真正的召回与排序逻辑由 `SemanticSearch()` 完成。
...
- MySQL：在最后根据商品 ID 查询当前商品数据，不负责检索相似商品。

### 想一想

假设服务器已经配置 `MILVUS_ADDR`，并按正常启动流程完成初始化。根据实际调用路径判断：一次语义搜索会访问真实 Milvus 吗？
...
两路各召回 30 个候选。某个商品可能在 ES 中排第 14、在 Milvus 中排第 12，但两路都认为它相关，融合后便有机会进入最终 Top10。
...
归一化后，两路结果才能方便地融合。Milvus 使用 L2 distance，数值越小越相近，因此向量分支会先反转方向；例如，距离 `0.2` 的相关度必须高于距离 `0.8` 的相关度。
...
按照 `0.5 / 0.5` 的权重计算，商品 101、103、102 的最终得分依次为 `0.75`、`0.45`、`0.3`，排序结果也是这个顺序。

这里的 `0.5 / 0.5` 是两路召回的权重。实际业务中可以根据搜索效果调整；例如，更重视语义搜索时，可以提高 `SemanticScore` 的权重。

### 3.6 MySQL 回查并返回 TopK

ES 和 Milvus 搜索完成后，还需要回查一次 MySQL。
...
系统根据搜索得到的商品 ID，到 MySQL 批量查询完整商品信息。MySQL 在这里负责补齐当前商品数据，不负责相似商品检索。
...
即使前面还有 50 个候选，最终也只返回最相关的前 10 个商品。
