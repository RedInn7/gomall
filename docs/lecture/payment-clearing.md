...
| 部分 | 要回答的问题 | 时间 |
|---|---|---:|
| 业务事故 | 为什么支付成功不能直接给卖家钱 | 6 min |
| 账户模型 | 钱从哪个账户进入哪个账户 | 7 min |
| 清算单 | 一笔资金事实需要保存什么 | 7 min |
| 三种渠道 | Wallet、Stripe、Web3 怎样统一记账 | 11 min |
| 重放与异常 | 重复回调和真实重复收款怎样区分 | 5 min |
| 测试 | 怎样证明清算阶段没有提前放款 | 4 min |
| **合计** |  | **40 min** |

这一讲只讲“平台收妥资金”这一段。订单完成后如何给卖家放款，放在《支付结算：订单完成后怎么把钱给卖家》里。
...
如果系统此时直接给卖家余额加 100 元，接下来可能发生三件事：
...
代码统一用分存储金额，100 元记为 `10000 cents`。
...
| `Channel`、`ProviderRef` | 钱从哪里来，外部凭证是什么 |
