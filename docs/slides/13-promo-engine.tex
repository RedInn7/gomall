...
\begin{tikzpicture}[node distance=4mm and 9mm,every node/.append style={align=center,font=\scriptsize,text width=26mm,minimum height=12mm}]
  ...
  \draw[arrow] (close.east) -- (rsv.west);
...
\end{center}
\vspace{1mm}
{\scriptsize \textbf{为什么 release 起新事务而不是嵌入关单事务}：关单流程已经有 outbox 事件 \texttt{order.cancelled} 在写，promo.released 是\textbf{次级事件}，独立事务更利于失败隔离 —— promo 退预算失败不影响关单本身。}
