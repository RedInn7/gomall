...
\end{itemize}
\srcnote{internal/promo/model.go:9--13 与 Coupon 的边界注释；internal/coupon/service.go:1--110 不变}
\end{frame}
...
\srcnote{internal/promo/service.go:163--195 CalculateBestDiscount 的注释里写明 SLO；README SLO P0/P1 章节同口径}
\end{frame}
...
\end{itemize}
\srcnote{internal/promo/service.go:65--75 PickBestPromoRule 注释里写明业务取舍}
\end{frame}
...
\srcnote{internal/promo/service\_test.go:33--52 TestPromo\_StepThresholdPicksHighestDiscount 单测验证该业务取舍}
\end{frame}
...
\end{itemize}
\srcnote{internal/promo/service.go:116--134 computeDiscountOnBase 同时覆盖两种类型；internal/promo/service\_test.go:55--92 两条测试分别验证选择方向}
\end{frame}
...
\end{itemize}
\srcnote{internal/promo/service.go:116--134 computeDiscountOnBase 实现向下取整；internal/promo/model.go:16--30 bps 常量定义}
\end{frame}
...
\srcnote{internal/promo/model.go:21--23 Scope 三档常量；internal/promo/repo.go:78--105 ListActiveForCart 按 scope OR 拼条件}
\end{frame}
...
\end{itemize}
\srcnote{internal/promo/service.go:89--98 applicableSubtotal；internal/promo/service\_test.go:104--129 TestPromo\_CategoryScopeOnlyAppliesToCategory 验证}
\end{frame}
...
\srcnote{internal/promo/repo.go:78--105 ListActiveForCart；internal/promo/service.go:163--195 CalculateBestDiscount}
\end{frame}

\begin{frame}[fragile]{核心计算逻辑：30 行 PickBestPromoRule}
\begin{lstlisting}[language=Go,firstnumber=69,
  caption={\srcnote{internal/promo/service.go:69--88 PickBestPromoRule}}]
...
\end{itemize}
\srcnote{internal/promo/service.go:69--88 PickBestPromoRule 的注释里完整写明该业务取舍}
\end{frame}
...
\end{itemize}
\srcnote{internal/promo/model.go:38--40 字段定义；internal/promo/repo.go:110--128 AtomicConsumeBudget 单条 UPDATE 带 RowsAffected 兜底}
\end{frame}

\begin{frame}[fragile]{AtomicConsumeBudget：单条 SQL + RowsAffected 兜底}
\begin{lstlisting}[language=Go,firstnumber=110,
  caption={\srcnote{internal/promo/repo.go:110--128 AtomicConsumeBudget}}]
...
\end{itemize}
\srcnote{internal/promo/repo\_test.go:13--52 TestPromo\_AtomicConsumeBudget\_Exhaust；stressTest/REPORT.md "Redis Lua vs DB 锁" 章节}
\end{frame}
...
\srcnote{internal/promo/repo.go:127 RowsAffected=0 时返回 ErrPromoBudgetExhausted；pkg/e/code.go:60--63 80001--80003 业务码段}
\end{frame}
...
\srcnote{internal/promo/service.go:239--260 ApplyDiscountInTx 嵌入业务事务；internal/order/service.go:68 同模式调 outbox.Insert}
\end{frame}

\begin{frame}[fragile]{ApplyDiscountInTx：14 行的事务嵌入}
\begin{lstlisting}[language=Go,firstnumber=239,
  caption={\srcnote{internal/promo/service.go:239--260 ApplyDiscountInTx}}]
...
\end{itemize}
\srcnote{internal/promo/service.go:239--260；internal/shared/outbox/publisher.go:65--85 publisher 异步发布 outbox 行}
\end{frame}
...
\end{itemize}
\srcnote{internal/promo/service.go:261--284 ReleaseDiscount；internal/promo/repo.go:130--140 RestoreBudget WHERE 兜底}
\end{frame}
...
\srcnote{internal/promo/service.go:261--284；internal/order/cancel.go:31--55 关单事务为参考}
\end{frame}
...
\end{itemize}
\srcnote{internal/promo/service.go:163--195 calculate 是无状态读；internal/promo/service.go:239--260 apply 重新算 = 用户感知不一致}
\end{frame}
...
\begin{itemize}
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{同步下单接入满减}：\texttt{internal/order/service.go::OrderCreate} 已调用 \texttt{CalculateBestDiscount} + \texttt{ApplyDiscountInTx}，订单写入 \texttt{PromoRuleID / PromoDiscountCents / FinalCents} 三字段。预算耗尽走\textbf{降级}（PromoRuleID=0，不阻断下单）。
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{关单退预算}：\texttt{internal/order/cancel.go::CancelUnpaidOrder} 调 \texttt{ReleaseDiscount(reason=cancel)}，关单后预算回滚。
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{退款退预算}：\texttt{internal/refund/service.go::ApproveRefund} 调 \texttt{ReleaseDiscount(reason=refund)}，退款审批通过后预算回滚。
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{集成测试 4 例}：命中规则 / 不命中 / 多规则取最优 / 预算耗尽降级 —— \texttt{internal/order/service\_test.go::TestOrderCreate\_*} 全 PASS。
...
\end{itemize}
\srcnote{internal/order/service.go:54,101；internal/order/cancel.go:66；internal/refund/service.go:132；internal/order/service\_test.go:116,176,225,267}
\end{frame}
...
\srcnote{internal/shared/outbox/publisher.go:65--85 publisher 主循环；service/events/events.go:130--145 PromoApplied / PromoReleased payload}
\end{frame}
...
\end{itemize}
\srcnote{docs/slides/11-consistency.tex "双写问题" 章节；internal/promo/service.go:255 outbox.Insert 调用}
\end{frame}
...
\end{itemize}
\srcnote{internal/promo/repo.go:141 Stop 接口；internal/promo/handler.go:43--85 AdminCreate / AdminStop handler}
\end{frame}
...
\end{itemize}
\srcnote{internal/promo/service.go:239--260 写 outbox；internal/shared/outbox/publisher.go:65--85 至少一次语义}
\end{frame}
