...
\begin{itemize}
  ...
  \end{itemize}
  \item \textbf{叠加规则}：本期 gomall 约定\textbf{先满减、后券}。引擎只负责前半段；券抵扣由 \texttt{service/coupon.go} 在下游算。
  \item \textbf{业务取舍}：一笔订单只能用\textbf{一条}满减规则（不叠加多条满减），避免"用户拿三条阶梯都减"的羊毛漏洞。
\end{itemize}
\srcnote{repository/db/model/promo.go:9--13 与 Coupon 的边界注释；service/coupon.go:1--110 不变}
\end{frame}
...
\srcnote{repository/db/model/promo.go:32--46 PromoRule 字段表 + status / DailyBudget 字段服务于不同角色}
\end{frame}
...
release (tx) & 关单 / 退款退预算 & 10--100 & 200ms & 落 outbox，离线对账兜底 \\
...
\srcnote{service/promo.go:163--195 CalculateBestDiscount 的注释里写明 SLO；README SLO P0/P1 章节同口径}
\end{frame}
...
\end{itemize}
\srcnote{service/promo.go:65--75 PickBestPromoRule 注释里写明业务取舍}
\end{frame}
...
\srcnote{service/promo\_test.go:33--52 TestPromo\_StepThresholdPicksHighestDiscount 单测验证该业务取舍}
\end{frame}
...
\end{itemize}
\srcnote{service/promo.go:116--134 computeDiscountOnBase 同时覆盖两种类型；service/promo\_test.go:55--92 两条测试分别验证选择方向}
\end{frame}
...
\end{itemize}
\srcnote{service/promo.go:116--134 computeDiscountOnBase 实现向下取整；repository/db/model/promo.go:16--30 bps 常量定义}
\end{frame}
...
\srcnote{repository/db/model/promo.go:21--23 Scope 三档常量；repository/db/dao/promo.go:78--105 ListActiveForCart 按 scope OR 拼条件}
\end{frame}
...
\end{itemize}
\srcnote{service/promo.go:89--98 applicableSubtotal；service/promo\_test.go:104--129 TestPromo\_CategoryScopeOnlyAppliesToCategory 验证}
\end{frame}
...
\srcnote{repository/db/dao/promo.go:78--105 ListActiveForCart；service/promo.go:163--195 CalculateBestDiscount}
\end{frame}

\begin{frame}[fragile]{核心计算逻辑：30 行 PickBestPromoRule}
\begin{lstlisting}[language=Go,firstnumber=69,
  caption={\srcnote{service/promo.go:69--88 PickBestPromoRule}}]
...
\end{itemize}
\srcnote{service/promo.go:69--88 PickBestPromoRule 的注释里完整写明该业务取舍}
\end{frame}
...
\end{itemize}
\srcnote{repository/db/model/promo.go:38--40 字段定义；repository/db/dao/promo.go:110--128 AtomicConsumeBudget 单条 UPDATE 带 RowsAffected 兜底}
\end{frame}

\begin{frame}[fragile]{AtomicConsumeBudget：单条 SQL + RowsAffected 兜底}
\begin{lstlisting}[language=Go,firstnumber=110,
  caption={\srcnote{repository/db/dao/promo.go:110--128 AtomicConsumeBudget}}]
...
\end{itemize}
\srcnote{repository/db/dao/promo\_test.go:13--52 TestPromo\_AtomicConsumeBudget\_Exhaust；stressTest/REPORT.md "Redis Lua vs DB 锁" 章节}
\end{frame}
...
\srcnote{repository/db/dao/promo.go:127 RowsAffected=0 时返回 ErrPromoBudgetExhausted；pkg/e/code.go:60--63 80001--80003 业务码段}
\end{frame}

% ============================================================
\section{下单 → 关单 全链路：apply / release 的事务故事}
% ============================================================

\begin{frame}{ApplyDiscountInTx：嵌在下单事务里}
\begin{tikzpicture}[node distance=4mm and 7mm,every node/.append style={align=center,font=\scriptsize,text width=24mm,minimum height=14mm}]
  \node[node-box,node-state-normal]                  (calc){前端调 calculate\\拿到 RuleID + 减免};
  ...
  \node[node-box,node-state-transition,below=of tx]   (ins) {INSERT order\\(Money 已扣减)};
  \node[node-box,node-state-transition,below=of ins]  (cons){promo.AtomicConsumeBudget\\扣 DailyBudget};
  \node[node-box,node-ok,below=of cons]               (out) {outbox 写\\promo.applied 事件};
  \draw[arrow] (calc) -- (sub);
  \draw[arrow] (sub) -- (tx);
  \draw[arrow] (tx) -- (ins);
  \draw[arrow] (ins) -- (cons);
...
\end{tikzpicture}
\vspace{2mm}
\small calculate 是"展示"，apply 是"落库"。展示可以反复刷新（高频读），落库只在用户点提交时一次（低频写）。\textbf{二者不共享缓存，避免"看到 25 元、实际只减了 15 元"的不一致投诉}。
\srcnote{service/promo.go:239--260 ApplyDiscountInTx 嵌入业务事务；service/order.go:68 同模式调 outbox.Insert}
\end{frame}

\begin{frame}[fragile]{ApplyDiscountInTx：14 行的事务嵌入}
\begin{lstlisting}[language=Go,firstnumber=239,
  caption={\srcnote{service/promo.go:239--260 ApplyDiscountInTx}}]
...
\end{itemize}
\srcnote{service/promo.go:239--260；service/outbox/publisher.go:65--85 publisher 异步发布 outbox 行}
\end{frame}

\begin{frame}{ReleaseDiscount：关单 / 退款时退还预算}
\begin{itemize}
  \item \textbf{时机}：关单（用户主动关 / 超时自动关）或退款审批通过。
  ...
  \item \textbf{业务诚实}：\textbf{cancel 退还百分百}（用户从未享受过补贴），\textbf{refund 全单退则全退}（本期已对接 \texttt{ApproveRefund} 全单退路径；部分退按比例待退款服务支持分账后扩展）。
\end{itemize}
\srcnote{service/promo.go:261--284 ReleaseDiscount；repository/db/dao/promo.go:130--140 RestoreBudget WHERE 兜底}
\end{frame}

\begin{frame}{Release 链路示意}
\begin{tikzpicture}[node distance=4mm and 7mm,every node/.append style={align=center,font=\scriptsize,text width=24mm,minimum height=14mm}]
  \node[node-box,node-state-normal] (close) {订单关单\\(超时 / 用户主动)};
  \node[node-box,node-state-transition,right=of close] (rsv) {service.ReleaseDiscount};
  \node[node-box,node-state-transition,right=of rsv]   (tx)  {新事务};
  ...
  \draw[arrow] (close) -- (rsv);
  \draw[arrow] (rsv) -- (tx);
  \draw[arrow] (tx) -- (rest);
  \draw[arrow] (rest) -- (ev);
\end{tikzpicture}
\vspace{2mm}
\small \textbf{为什么 release 起新事务而不是嵌入关单事务}：关单流程已经有 outbox 事件 \texttt{order.cancelled} 在写，promo.released 是\textbf{次级事件}，独立事务更利于失败隔离 —— promo 退预算失败不影响关单本身。
\srcnote{service/promo.go:261--284；service/order\_cancel.go:31--55 关单事务为参考}
\end{frame}
...
\end{tabular}
\vspace{2mm}
\small 业务码 80001/80002/80003 与现有 70001/70002/60002（限流 / 熔断 / 幂等）口径一致。\textbf{客服 SOP 培训只需要拿这张表 + 复述句式}。
\srcnote{pkg/e/code.go:60--63 业务码 80001--80003；pkg/e/msg.go:54--56 客服话术原句}
\end{frame}

\begin{frame}[fragile]{业务码与提示语：3 行配置一致性}
\begin{lstlisting}[language=Go,firstnumber=60,
  caption={\srcnote{pkg/e/code.go:60--63 + pkg/e/msg.go:54--56 业务码 + 客服话术同步}}]
// pkg/e/code.go
...
\begin{itemize}
  ...
  \item \textbf{客服第 2 步}：ELK 检索 traceId，找到 \texttt{service.GetPromoSrv().CalculateBestDiscount} 的入参 / 出参；
...
\end{itemize}
\srcnote{service/promo.go:163--195 calculate 是无状态读；service/promo.go:239--260 apply 重新算 = 用户感知不一致}
\end{frame}
...
\begin{itemize}
  \item \textbf{不做用户领券}：那是 \texttt{service/coupon.go} 的职责。两套系统通过下单链路串起来。
...
\end{itemize}
\srcnote{repository/db/model/promo.go:9--13 与 Coupon 边界注释；README "业务边界（不做什么）" 章节同口径}
\end{frame}
...
\begin{itemize}
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{同步下单接入满减}：\texttt{service/order.go::OrderCreate} 已调用 \texttt{CalculateBestDiscount} + \texttt{ApplyDiscountInTx}，订单写入 \texttt{PromoRuleID / PromoDiscountCents / FinalCents} 三字段。预算耗尽走\textbf{降级}（PromoRuleID=0，不阻断下单）。
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{关单退预算}：\texttt{service/order\_cancel.go::CancelUnpaidOrder} 调 \texttt{ReleaseDiscount(reason=cancel)}，关单后预算回滚。
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{退款退预算}：\texttt{service/refund.go::ApproveRefund} 调 \texttt{ReleaseDiscount(reason=refund)}，退款审批通过后预算回滚。
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{集成测试 4 例}：命中规则 / 不命中 / 多规则取最优 / 预算耗尽降级 —— \texttt{service/order\_test.go::TestOrderCreate\_*} 全 PASS。
  \item 仍是路线图：异步下单链路 \texttt{OrderEnqueue} 未接满减（本轮范围只覆盖同步 \texttt{OrderCreate}）。
  \item 仍是路线图：DailyBudget 0 点重置 cron；promo\_token（calculate→apply 一致性校验）。
\end{itemize}
\srcnote{service/order.go:54,101；service/order\_cancel.go:66；service/refund.go:132；service/order\_test.go:116,176,225,267}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{路线图 P3}：与 Coupon 的叠加规则在 \texttt{service/order.go} 算式层固化，单测覆盖"先满减、后券"5--10 个 case。
\end{itemize}
...
\begin{itemize}
  \item \textbf{测试}：\texttt{repository/db/dao/promo\_test.go::TestPromo\_AtomicConsumeBudget\_Exhaust}
...
\end{itemize}
\srcnote{repository/db/dao/promo\_test.go:13--52 真实测试；stressTest/REPORT.md "优惠券 - Redis Lua vs DB 锁" 对照}
\end{frame}
...
\begin{tikzpicture}[node distance=4mm and 6mm,every node/.append style={align=center,font=\scriptsize,text width=22mm,minimum height=12mm}]
...
\end{tikzpicture}
\vspace{2mm}
\small 一条 \texttt{promo.applied} 事件喂三个下游消费者。三者订阅同一 routing key，互不影响 —— 这就是 outbox 模式的"事件单源 + 多订阅"红利。
\srcnote{service/outbox/publisher.go:65--85 publisher 主循环；service/events/events.go:130--145 PromoApplied / PromoReleased payload}
\end{frame}
...
\end{itemize}
\srcnote{docs/slides/11-consistency.tex "双写问题" 章节；service/promo.go:255 outbox.Insert 调用}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{修复 SOP}：\texttt{PATCH /admin/promo/rules/:id} 改大 budget（本期接口已就位），无需停活动。
\end{itemize}
\srcnote{repository/db/dao/promo.go:141 Stop 接口；api/v1/promo.go:43--85 AdminCreate / AdminStop handler}
\end{frame}
...
\end{itemize}
\srcnote{service/promo.go:239--260 写 outbox；service/outbox/publisher.go:65--85 至少一次语义}
\end{frame}
...
\begin{itemize}
  \item \textbf{Q1}：满减能不能与优惠券叠加？\textbf{A}：能。规则是先满减、后券，由 \texttt{service/order.go} 算式层固化。本期引擎只算前半段。
...
模块 & 位置 \\
\midrule
模型（PromoRule + 常量） & \texttt{repository/db/model/promo.go} \\
DAO（CRUD + 预算扣减 / 退还） & \texttt{repository/db/dao/promo.go} \\
DAO 单测（预算耗尽 / 退还 / 范围查询） & \texttt{repository/db/dao/promo\_test.go} \\
Service（最优算法 + 事务嵌入 + outbox） & \texttt{service/promo.go} \\
Service 单测（阶梯 / 满折扣 vs 满减 / 范围） & \texttt{service/promo\_test.go} \\
Service 事件 payload & \texttt{service/events/events.go::PromoApplied / PromoReleased} \\
API handler（公开 + admin） & \texttt{api/v1/promo.go} \\
路由注册（公开 / authed / admin 三组） & \texttt{routes/promo\_routes.go} \\
业务码与提示语 & \texttt{pkg/e/code.go:60--63} + \texttt{pkg/e/msg.go:54--56} \\
Migration & \texttt{repository/db/dao/migrate.go} \\
DTO & \texttt{types/promo.go} \\
\bottomrule
\end{tabular}
\vspace{2mm}
\small 全部新增文件 9 个 + 修改 4 个。引擎自身\textbf{无新增第三方依赖}，与 outbox / 业务码体系完全复用。
\end{frame}
