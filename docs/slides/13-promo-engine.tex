...
\begin{frame}[shrink]{优惠券和满减是两套独立工具，常被误当同一类}
\begin{itemize}
...
\begin{frame}[shrink]{5 角色对满减的不同诉求}
...
\begin{frame}[shrink]{业务承诺 SLO：calculate 是高频读、apply 是低频写}
\begin{tabular}{@{}lllll@{}}
\toprule
...
\begin{frame}[shrink]{用户最优 vs 平台最优 —— 一个 70 块钱的取舍}
...
\begin{frame}[shrink]{满减 vs 满折扣 —— 同价位下哪个更狠}
\begin{itemize}
  \item 同时挂两条都门槛"满 200"：
  \begin{itemize}
    \item 满 200 减 25
...
\begin{frame}[shrink]{满折扣的代价：bps 取整带来的"1 分钱"问题}
\begin{itemize}
...
\begin{frame}[shrink]{规则范围三档与业务责任分配}
\begin{tabular}{@{}llll@{}}
\toprule
...
\begin{frame}[shrink]{类目级规则：购物车里只有这个类目的金额才算门槛}
...
\begin{frame}[fragile,shrink]{核心计算逻辑：30 行 PickBestPromoRule}
...
\begin{frame}[shrink]{PickBestPromoRule 逐行讲解}
\begin{itemize}
...
\begin{frame}[shrink]{DailyBudget 是把"营销预算"和"系统兜底"绑在同一张表里}
\begin{itemize}
...
\begin{frame}[fragile,shrink]{AtomicConsumeBudget：单条 SQL + RowsAffected 兜底}
...
\begin{frame}[shrink]{为什么是一条 SQL 而不是先 SELECT 再 UPDATE}
\begin{itemize}
...
\begin{frame}[shrink]{运营怎么用 DailyBudget：三档预算策略}
...
\begin{frame}[shrink]{ApplyDiscountInTx：嵌在下单事务里}
\begin{center}
...
\begin{frame}[fragile,shrink]{ApplyDiscountInTx：14 行的事务嵌入}
...
\begin{frame}[shrink]{ApplyDiscountInTx 逐行讲解}
\begin{itemize}
...
\begin{frame}[shrink]{ReleaseDiscount：消费关单 / 退款事件，异步退还预算}
\begin{itemize}
...
\begin{frame}[shrink]{Release 链路示意}
\begin{center}
...
\begin{frame}[shrink]{客户问"为什么我满了 200 没减 25"—— 四种可能}
\scriptsize
...
\begin{frame}[fragile,shrink]{业务码与提示语：3 行配置一致性}
...
\begin{frame}[shrink]{客服真实事故脚本（演练材料）}
\begin{itemize}
...
\begin{frame}[shrink]{不做清单 —— 诚实标出本引擎的边界}
\begin{itemize}
...
\begin{frame}[shrink]{本期落地：结算集成 + 关单 / 退款 release 全闭环}
\begin{itemize}
...
\begin{frame}[shrink]{满减引擎的下一步：补 admin UI 与商家自助}
\begin{itemize}
...
\begin{frame}[shrink]{与现有营销三剑客的 RPS 对照}
\begin{tabular}{@{}lrll@{}}
\toprule
接口 & RPS & p95 & 备注 \\
\midrule
...
\begin{frame}[shrink]{真实数字之二：DAO 单测的并发安全验证}
\begin{itemize}
...
\begin{frame}[shrink]{promo 在 outbox 体系里的位置}
\begin{center}
...
\begin{frame}[shrink]{为什么不直接调下游 HTTP / RPC：双写问题}
\begin{itemize}
...
\begin{frame}[shrink]{假想故障 1：客服收到大量 "我满了没减" 投诉}
\begin{itemize}
...
\begin{frame}[shrink]{假想故障 2：财务对账每月差几百块}
\begin{itemize}
...
\begin{frame}[shrink]{Q\&A（下）—— 技术侧高频问题}
\begin{itemize}
