...
\begin{itemize}
  ...
  \item \textbf{业务诚实}：\textbf{cancel 退还百分百}（用户从未享受过补贴），\textbf{refund 全单退则全退、部分退按比例}（实现细节本期暂未对接退款服务，留接口）。
\end{itemize}
...
\begin{itemize}
  \item \textbf{已落地}：规则 CRUD（创建 / 列表 / 停用）、calculate / apply / release、80001--80003 业务码。
