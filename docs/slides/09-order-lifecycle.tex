...
\begin{frame}[shrink]{订单状态不是 type 字段，是 5 个角色的协同语言}
...
\begin{frame}[shrink]{3 态够吗：旧版本卡住的真实业务症状}
\begin{itemize}
...
\begin{frame}[shrink]{gomall v2.2 的 7 态决策：业界对齐 + 业务诉求}
{\small
\begin{tabular}{@{}llll@{}}
\toprule
gomall & 中文 & 触发事件 & 主导角色 \\
...
\begin{frame}[shrink]{帧 1 · WaitPay：C 端 / 商家 / 客服 各看到什么}
{\footnotesize
...
\begin{frame}[shrink]{帧 2 · WaitShip：商家工作流的真正起点}
{\footnotesize
\begin{tabular}{@{}lp{3.5cm}p{3cm}p{3cm}@{}}
\toprule
角色 & 看到 & 能做 & 何时切走 \\
...
\begin{frame}[shrink]{帧 3 · WaitReceive：平台 7d 自动确认的资金钟}
{\footnotesize
\begin{tabular}{@{}lp{3.5cm}p{3cm}p{3cm}@{}}
\toprule
角色 & 看到 & 能做 & 何时切走 \\
...
\begin{frame}[shrink]{帧 4 · Completed / Closed / Refunding / Refunded：三终态 + 一中间}
{\footnotesize
...
\begin{frame}[shrink]{完整状态机：用户 / 商家 / 平台 各自的边}
\begin{center}
\begin{tikzpicture}[node distance=12mm and 12mm,every node/.style={font=\scriptsize}]
  \node[node-box,fill=cMain!12] (wp) {WaitPay\\\tiny (1)};
  \node[node-box,fill=cAccent!8,draw=cAccent,dashed,right=of wp] (oh) {OnHold\\\tiny 设计示意};
  \node[node-box,fill=cMain!12,right=of oh] (ws) {WaitShip\\\tiny (2)};
  \node[node-box,fill=cMain!12,right=of ws] (wr) {WaitReceive\\\tiny (4)};
  \node[node-box,fill=cMain!12,right=of wr] (cp) {Completed\\\tiny (5) 非终态};
  ...
  \draw[arrow-alert,dashed] (wp) -- node[above,font=\tiny]{付款}(oh);
  \draw[arrow-alert,dashed] (oh) -- node[above,font=\tiny]{1h}(ws);
  \draw[arrow] (ws) -- node[above,font=\tiny]{商家发货}(wr);
  \draw[arrow] (wr) -- node[above,font=\tiny]{确认/7d}(cp);

  \draw[arrow-bad] (wp) -- node[left,font=\tiny]{30min/取消}(cl);
  \draw[arrow-alert,dashed] (oh) to[bend right=22] node[below,sloped,font=\tiny\color{cAccent}]{hold期取消}(rg.north west);
  \draw[arrow-alert] (ws.south) -- node[left,font=\tiny\color{cAccent}]{发货前退}(rg.north west);
  \draw[arrow-alert] (wr) -- node[right,font=\tiny\color{cAccent}]{用户退款}(rg);
  \draw[arrow-alert] (cp.south west) -- node[above,sloped,font=\tiny\color{cAccent}]{售后退款}(rg.north east);
...
\vspace{2mm}
\begin{itemize}
  \item \textbf{蓝边} = 正向流转（业务期望）；\textbf{橙边} = 退款；\textbf{灰边} = 关单；\textbf{虚线橙节点 OnHold} = 支付后风控 / 冷静缓冲（设计示意，当前实现 \texttt{WaitPay} 支付成功直接 $\to$ \texttt{WaitShip}）。
  \item \textbf{Completed 不是终态}：真正终态只有 Closed / Refunded；已完成在售后窗口内仍有出边 \texttt{Completed $\to$ Refunding}。
...
\begin{frame}[shrink]{30min 倒计时：体验 vs 锁库存的钢丝绳}
...
\begin{frame}[shrink]{为什么单一保险不够：RMQ 漏 + Cron 慢}
\begin{center}
...
\begin{frame}[shrink]{时序图：下单落库 + 超时延迟关单的完整交互}
\centering
...
\begin{frame}[fragile,shrink]{毒丸消息：失败关单无限 requeue 把延迟队列打爆}
...
\begin{frame}[fragile,shrink]{已修复复盘：cron 表达式陷阱与现行写法}
...
\begin{frame}[fragile,shrink]{CancelUnpaidOrder：4 个调用方共享一个幂等入口}
\begin{lstlisting}[language=Go,firstnumber=19,
...
\begin{frame}[shrink]{客诉 \#1：30min 自动关单 — 客服怎么回}
\begin{itemize}
...
\begin{frame}[shrink]{客诉 \#2：付了 3 天没发货 — 客服怎么回}
\begin{itemize}
...
\begin{frame}[shrink]{客诉 \#3：点了确认收货能撤回吗 — 客服怎么回}
\begin{itemize}
...
\begin{frame}[shrink]{客诉 \#4：用户说没收到货 — 客服怎么回}
\begin{itemize}
...
\begin{frame}[shrink]{客诉 \#5：抢购 / 支付出现 70001 / 60002 — 客服怎么回}
\begin{itemize}
...
\begin{frame}[shrink]{业务承诺总表：7 条边 7 个业务事件}
{\small
...
\begin{frame}[shrink]{订单链路 SLO：从 README 业务承诺到压测对照}
{\small
...
\begin{frame}[shrink]{业务可观测指标：5 个状态切换的 metrics}
\begin{itemize}
...
\begin{frame}[fragile,shrink]{第 1 层：状态机表是业务规则，不是数据结构}
...
\begin{frame}[fragile,shrink]{第 2 层：DAO 条件 UPDATE 是并发的最后一道闸}
...
\begin{frame}[shrink]{业务边界：deck 09 范围之外的 6 件事}
\begin{itemize}
...
\begin{frame}[shrink]{路线图 1：logistics\_order 子表 — 为什么不入主表}
\begin{itemize}
...
\begin{frame}[shrink]{路线图 2：aftersales 工单 — 售后完整生命周期}
...
\begin{frame}[shrink]{路线图 3：评价表 — 挂在哪条状态边上}
...
\begin{frame}[shrink]{现状 vs 路线图：一张图看清边界}
...
\begin{frame}[shrink]{零停机迁移：DB 不动 + 别名兼容}
\begin{itemize}
...
\begin{description}
  \item[状态机表 + 工具函数]
  \item[状态常量（旧别名已删）]
  \item[OrderStateMap 中文名]
  \item[ShipOrder service]
  \item[ConfirmReceive service]
  \item[RequestRefund service]
  \item[ApproveRefund service]
  \item[RejectRefund service]
  \item[CancelUnpaidOrder 共享入口]
  \item[满减预算退还消费侧（异步）]
  \item[CloseOrderWithCheck DAO]
  \item[ShipOrder DAO]
  \item[ConfirmReceive DAO]
  \item[RequestRefund DAO]
  \item[GetTimeoutWaitReceive DAO]
  \item[Cron 自动确认收货]
  \item[Cron 注册 (现行 @every 5m)]
  \item[RMQ 延迟队列 30min]
  \item[业务码 70001 / 70002 / 60002]
\end{description}
\end{frame}
