...
\begin{itemize}
  ...
  \item \textbf{业务影响}：DB 每分钟扫一次 \texttt{WHERE type=1 AND created\_at < now()-15min}，6M 行 order 表，每次 100 笔 $\to$ 单峰 DB 压力 60×。
...
\begin{frame}[allowframebreaks]{真实 Bug 的业务后果与修复路线图}
\begin{center}
...
\begin{itemize}\itemsep0pt
  \item \textbf{业务可见症状}：dev 环境 DB CPU 周期性飙红 + logrus 日志量异常 $\to$ 引出排查。
...
\end{itemize}
\keypoint{\footnotesize 幂等设计的隐藏 ROI：它不仅防止业务超卖，也\textbf{掩盖了表达式 bug、cron 重启重跑、网络重试} — 但代价是 60× 无效 DB 压力。}
