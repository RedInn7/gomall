...
\begin{lstlisting}[language=Go,firstnumber=19,
  caption={initialize/cron.go:19--28 注册关单 cron（含 bug）},captionpos=b]
...
\end{itemize}
\srcnote{initialize/cron.go:21 真实 bug 表达式；internal/order/cancel.go:32--38 CloseOrderWithCheck 幂等兜底}
\end{frame}
...
\srcnote{initialize/cron.go:21 待修；internal/order/cancel.go:32 + internal/order/repo.go:198 兜底链}
\end{frame}
...
\hspace{4mm}\srcnote{initialize/cron.go:21 + internal/order/cancel.go:32 + internal/order/repo.go:198}
...
\begin{description}
  ...
  \item[Cron 注册 (含 bug)]         \srcnote{initialize/cron.go:19--31}
