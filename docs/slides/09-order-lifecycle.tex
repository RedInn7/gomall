...
\end{itemize}
\srcnote{internal/order/shipping.go:36--71 ShipOrder；events/events.go OrderShipped 事件携带 tracking}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/task.go:60--90 RunAutoConfirmReceive；initialize/cron.go:42 注册 0 0 */6}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/state.go:20--26 转换表；internal/refund/service.go:46--160 三个退款方法}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/state.go:20--26 转换表；internal/order/shipping.go:36--110 + refund/service.go:46--160}
\end{frame}
...
\end{itemize}
\srcnote{repository/rabbitmq/order\_delay.go:22 OrderCancelDelay=30min；internal/order/cancel.go:54 释放 reserved}
\end{frame}
...
\end{itemize}
\srcnote{repository/rabbitmq/order\_delay.go:60--74 PublishOrderCancelDelay；initialize/cron.go:16 cron 注册；internal/order/cancel.go:18--62 共享入口}
\end{frame}
...
\end{itemize}
\srcnote{initialize/cron.go:16 真实 bug 表达式；internal/order/cancel.go:32--38 CloseOrderWithCheck 幂等兜底}
\end{frame}
...
\srcnote{initialize/cron.go:16 待修；internal/order/cancel.go:32 + internal/order/repo.go:185 兜底链}
\end{frame}

\begin{frame}[fragile]{CancelUnpaidOrder：4 个调用方共享一个幂等入口}
\begin{lstlisting}[language=Go,firstnumber=18,
  caption={internal/order/cancel.go:18--62 共享入口（节选）},captionpos=b]
...
\end{itemize}
\srcnote{internal/order/cancel.go:18--62；CloseOrderWithCheck WHERE type=1 兜底幂等}
\end{frame}
...
\end{itemize}
\srcnote{repository/rabbitmq/order\_delay.go:22 30min 平台规则；internal/order/cancel.go:46 reason=timeout 事件标记}
\end{frame}
...
\end{itemize}
\srcnote{internal/refund/service.go:36--40 refundAllowedFrom 含 WaitShip；internal/refund/service.go:46--91 RequestRefund 主流程}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/state.go:24 Completed 出边只有 Refunding；internal/refund/service.go:36--40 from 含 Completed}
\end{frame}
...
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\srcnote{internal/refund/service.go:36--40 from 列表覆盖 WaitReceive / Completed；事件 events/events.go OrderShipped 带 tracking}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/state.go:20--26；internal/order/shipping.go + refund/service.go + order/task.go 四件套}
\end{frame}
...
\begin{lstlisting}[language=Go,firstnumber=20,
  caption={internal/order/state.go:20--26 转换表},captionpos=b]
...
\end{itemize}
\srcnote{internal/order/state.go:20--26 表；:34--45 CanTransition；:56--59 IsTerminal}
\end{frame}

\begin{frame}[fragile]{第 2 层：DAO 条件 UPDATE 是并发的最后一道闸}
\begin{lstlisting}[language=Go,firstnumber=202,
  caption={internal/order/repo.go:202--210 ShipOrder DAO},captionpos=b]
...
\end{itemize}
\srcnote{internal/order/repo.go:202--210 ShipOrder；:214--222 ConfirmReceive；:226--238 RequestRefund 多 from}
\end{frame}

\begin{frame}[allowframebreaks,fragile]{第 3 层：outbox 事件解耦下游服务}
\begin{lstlisting}[language=Go,firstnumber=36,
  caption={internal/order/shipping.go:36--71 ShipOrder 主体（节选）},captionpos=b]
...
\end{itemize}
\srcnote{internal/order/shipping.go:36--71；internal/shared/outbox/publisher.go 异步派送}
\end{frame}
...
\end{itemize}
\srcnote{当前 internal/order/model.go:7--18 model 无 tracking 字段；路线图 repository/logistics/adapter\_xxx.go}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/task.go:79--85 OrderCompletedEvent.Auto；路线图：service/review/*.go 消费此事件}
\end{frame}
...
\hspace{4mm}\srcnote{initialize/cron.go:16 + internal/order/cancel.go:32 + internal/order/repo.go:185}
...
\hspace{4mm}\srcnote{internal/refund/service.go:36--40 from 含 WaitShip}
...
\hspace{4mm}\srcnote{internal/order/task.go:16 autoConfirmReceiveDays=7；internal/order/state.go:24 Completed 出边}

\vspace{1mm}
\textbf{Q5}：用户能撤回"已确认收货"吗？\\
\hspace{4mm}\small A：不能（状态机不允许 Completed $\to$ WaitReceive 回退）。但走 \texttt{Completed $\to$ Refunding} 售后路径仍可退款。\\
\hspace{4mm}\srcnote{internal/order/state.go:20--26 转换表}
\end{frame}

\begin{frame}{业务 Q\&A（下）}
\textbf{Q6}：商家发货 24h 后又想撤销发货，能吗？\\
\hspace{4mm}\small A：不能（WaitReceive $\to$ WaitShip 不存在）。商家联系客服走\textbf{用户全额退款}流程。\\
\hspace{4mm}\srcnote{internal/order/state.go:23 WaitReceive 出边只到 Completed / Refunding}
...
\hspace{4mm}\srcnote{internal/order/shipping.go:36--71 + events/events.go OrderShipped}
...
\hspace{4mm}\srcnote{internal/order/repo.go:202--210}
...
\begin{frame}{代码位置一览}
\begin{description}
  \item[状态机表 + 工具函数]   \srcnote{internal/order/state.go:20--59}
  \item[7 态常量 + 旧别名]     \srcnote{consts/order.go:6--38}
  \item[OrderStateMap 中文名]  \srcnote{consts/order.go:41--49}
  \item[ShipOrder service]    \srcnote{internal/order/shipping.go:36--71}
  \item[ConfirmReceive service] \srcnote{internal/order/shipping.go:73--110}
  \item[RequestRefund service]  \srcnote{internal/refund/service.go:46--91}
  \item[ApproveRefund service]  \srcnote{internal/refund/service.go:93--128}
  \item[RejectRefund service]   \srcnote{internal/refund/service.go:130--157}
  \item[CancelUnpaidOrder 共享入口] \srcnote{internal/order/cancel.go:18--62}
  \item[CloseOrderWithCheck DAO]    \srcnote{internal/order/repo.go:185--196}
  \item[ShipOrder DAO]              \srcnote{internal/order/repo.go:202--210}
  \item[ConfirmReceive DAO]         \srcnote{internal/order/repo.go:214--222}
  \item[RequestRefund DAO]          \srcnote{internal/order/repo.go:226--238}
  \item[GetTimeoutWaitReceive DAO]  \srcnote{internal/order/repo.go:264--277}
  \item[Cron 自动确认收货]          \srcnote{internal/order/task.go:60--90}
