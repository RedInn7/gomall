...
\begin{frame}{Saga 库存补偿：预扣在事务外，失败必须还回}
  \keypoint{Redis 预扣库存（available$\to$reserved）发生在 DB 事务\textbf{之外}——高并发下不让 Redis 扣减跟着行锁排队。代价：后续失败必须由代码主动补偿还回，否则库存被"幽灵占用"。}
  \vspace{1mm}
  \resizebox{0.74\textwidth}{0.3\textheight}{%
  ...
  \begin{itemize}\small
    ...
    \item \textbf{对账兜底}：\texttt{ScanReservedProductIDs} 用 SCAN 增量枚举 reserved 桶，定时比对 DB 捞回漏退库存。
  \end{itemize}
