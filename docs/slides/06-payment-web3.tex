...
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\srcnote{service/payment\_crypto.go:129--148 outbox 事件埋点是 BI 数据源}
...
\begin{itemize}
  ...
  \item \textbf{Web3 答案}：把"担保人"换成一段第三方审计过的代码，资金锁在合约里，合约只接受三种动作 —— 买家确认收货 → 给商家；买家退货 → 退给买家；争议 → 仲裁人裁决。
  \item \textbf{业务承诺等价}：合约的"不变式"（buyer / seller / amount 部署后不可改、Created→Funded 只能一次、终态不可逆）= 平台对外承诺的最低契约。
\end{itemize}
\vspace{1mm}
...
\begin{frame}{端到端 6 步业务流：钱 → 合约 → 订单}
\begin{tikzpicture}[node distance=4mm and 8mm,font=\scriptsize]
  \node[node-box] (w)   {1. 用户钱包\\发起付款};
  \node[node-box,right=of w]   (be)  {2. 后端\\颁 nonce};
  \node[node-box,right=of be]  (sig) {3. 钱包\\本地签名};
  \node[node-box,below=of sig,node-warn] (vfy) {4. 后端验签\\订单挂 pending};
  \node[node-box,left=of vfy] (esc) {5. 用户调合约\\fundWithOrderID};
  \node[node-box,left=of esc] (evt) {6. 链上 emit\\PaymentConfirmed};
  \node[node-box,below=of evt,node-ok] (lis) {listener 写 outbox\\订单 $\to$ Paid};
  \node[node-box,right=of lis] (ord) {商家可发货};
  \draw[arrow] (w)   -- (be);
  \draw[arrow] (be)  -- (sig);
  \draw[arrow] (sig) -- (vfy);
  \draw[arrow] (vfy) -- (esc);
...
\srcnote{service/payment\_crypto.go:48--84 IssueNonce 颁发}
\srcnote{service/payment\_crypto.go:86--160 VerifyAndPark 验签 + outbox}
...
\srcnote{service/payment\_crypto.go:156--159 立即返回 pending 状态}
...
\srcnote{service/payment\_crypto.go:104--124 nonce 消费 + 验签是事故诊断的两个关键锚点}
\end{frame}
...
\end{tabular}
\vspace{2mm}
\small \textbf{业务承诺的复用}：listener 写入的 \texttt{web3.payment.confirmed} 与法币的
...
\begin{lstlisting}[language=Go,firstnumber=49,
  caption={\srcnote{service/payment\_crypto.go:49--160 节选}}]
...
\end{tikzpicture}
\vspace{1mm}
\small \textbf{业务不变式（对外承诺）}：
\begin{itemize}
  \item \texttt{Created → Funded} 只能发生一次 → 用户付款不会被重复扣
  \item 终态不可逆 → 商家收到钱（Released）后平台无法撤回
  \item buyer / seller / arbiter / amount 部署后不可改 → 合约部署即承诺
\end{itemize}
...
\srcnote{service/payment\_crypto.go:116--124 验签后 / 写 outbox 前，是合规过滤的天然插入点}
...
跑出 \textbf{50K RPS / p95 2.33ms}（50VU × 15s 同 Idempotency-Key 累计 755{,}033 次请求 → DB 1 笔订单）。
...
\srcnote{service/payment\_crypto.go:151--154 SetWeb3Pending}
...
\hspace{4mm}\small A：钱包用户在确认 tx 时常因 gas 不够重试 → 后端 \texttt{/paydown/crypto} 收到两次同 Idempotency-Key 请求 → 幂等中间件让第二次直接 replay 第一次结果。
事务 + outbox 保证"验签通过"与"事件落地"同生共死，下游不会收到孤儿事件。\\
\srcnote{routes/router.go:132--136 paydown/crypto 套幂等中间件}
\srcnote{service/payment\_crypto.go:129--148 事务内写 outbox}
\end{frame}
...
\hspace{4mm}\small A：listener 进入退避重连（1s → 60s 封顶），期间链上事件不消费；新增订单仍可签名挂 pending（验签不依赖 RPC）；
