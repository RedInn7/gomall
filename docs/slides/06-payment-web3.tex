...
\begin{frame}[shrink]{先讲清楚 gomall 为什么开 Web3 通道}
...
\srcnote{internal/payment/routes.go:14--27 paydown（法币）与 paydown/crypto（web3）平行挂载}
\srcnote{README.md:69--80 业务边界："不真上链"是显式声明的范围}
\end{frame}

\begin{frame}[shrink]{5 角色视角：Web3 通道对每个角色意味着什么}
\begin{tabular}{@{}lll@{}}
\toprule
...
\begin{frame}[shrink]{业务价值定量：开一条 Web3 通道相当于开三个用户群}
\begin{tabular}{@{}lll@{}}
\toprule
...
\srcnote{internal/payment/routes.go:14--27 法币 + Web3 两条支付通道并行}
\srcnote{README.md:74--80 不真上链是显式业务边界}
\end{frame}

\begin{frame}[shrink]{跨境用户旅程：东南亚买家在 gomall 下单的真实链路}
...
\srcnote{pkg/web3/contracts/Escrow.sol:68--82 fund；:85--107 release/refund/dispute 状态出口}
\srcnote{internal/payment/routes.go:14--27 法币 + Web3 通道并行}
\end{frame}

\begin{frame}[shrink]{运营视角：开 Web3 通道的转化数据怎么追}
\textbf{运营要回答两个业务问题}：
\begin{itemize}
...
\begin{frame}[shrink]{Escrow 业务价值：信任锚从"银行"换成"可审计代码"}
...
\srcnote{pkg/web3/contracts/Escrow.sol:13--19 状态机；:68--82 fund/fundWithOrderID；:85--107 release/refund/dispute}
\srcnote{pkg/web3/contracts/README.md:1--36 合约职责与不变式条款}
\end{frame}

\begin{frame}[shrink]{端到端 6 步业务流：钱 $\to$ 合约 $\to$ 订单}
\centering
...
\begin{frame}[shrink]{商家收款体验：什么时候算"到账"}
...
\srcnote{pkg/web3/contracts/Escrow.sol:85--107 release / refund / dispute 三个出金口}
\srcnote{service/web3/listener.go:296--301 事件级幂等保证 refund 不会重复推进}
\end{frame}

\begin{frame}[shrink]{用户体验：链是异步的，UX 必须 ticket 化}
...
\begin{frame}[shrink]{Web3 业务码 + 客服话术（核心 4 条）}
\begin{tabular}{@{}lll@{}}
\toprule
业务码            & 含义                    & 客服话术 \\
\midrule
...
\begin{frame}[shrink]{客服话术三场景：用户最常问的三类问题}
...
\begin{frame}[shrink]{链是不可靠通道：3 类事故 + SRE 处置 SOP}
...
\begin{frame}[shrink]{listener 三项基本功：业务承诺背后的实现}
...
\begin{frame}[shrink]{容错矩阵：异常 vs 业务影响 vs 兜底}
\centering
{\footnotesize
...
\begin{frame}[shrink]{标准时序图：链上确认结算全链路（签名 $\to$ 链上 $\to$ 入账）}
\centering
\resizebox{0.92\textwidth}{!}{%
\begin{sequencediagram}
  \tikzstyle{inststyle}+=[rounded corners=2pt]
  \newthread{w}{用户钱包}
  \newinst[1]{h}{Payment Handler}
  \newinst[1]{r}{Redis}
  \newinst[1]{e}{Escrow 合约(链上)}
  \newinst[1]{l}{Web3 Listener}
  \newinst[1]{s}{结算消费者}
  \newinst[1]{o}{订单/台账}

  \begin{call}{w}{取 nonce}{h}{nonce}
    \begin{call}{h}{生成+存 nonce(5min)}{r}{ok}\end{call}
  \end{call}
  \mess[1]{w}{EIP-191 离线签名 私钥不离手}{w}
  \begin{call}{w}{提交签名 VerifyAndPark：验签 + 消费 nonce}{h}{pending}
    \begin{sdblock}{同一事务}{验签 + 写 outbox web3.payment.pending}
      \mess{h}{Redis pending 占位 30min}{r}
    \end{sdblock}
  \end{call}
  \mess[1]{w}{链上转 USDC 或 ETH，异步}{e}
  \mess{e}{emit PaymentConfirmed orderID amount}{l}
  \mess{l}{web3.payment.confirmed}{s}
  \begin{sdblock}{金额精度校验：防少付}{USDC 6 位 1:1，ETH 喂价换 wei}
    \begin{call}{s}{标记已付 + 入账 + 复式记账 + outbox order.paid（幂等 WaitPay 守卫）}{o}{已付}\end{call}
  \end{sdblock}
\end{sequencediagram}}
\par\smallskip
{\footnotesize \textbf{三道闸口}：\textbf{签名防重放}（nonce 一次性消费 + EIP-191 验签）$\to$
\textbf{链上确认兜底}（钱进合约才 emit，listener 收事件才推进，后端宕机不丢单）$\to$
\textbf{金额按代币精度校验防少付}（USDC 按 6 位小数 1:1 比对、ETH 按喂价折算 wei，少 1 单位即拒绝入账）。
\srcnote{internal/payment/service\_crypto.go:104--160 消费 nonce + 验签 + 事务写 outbox}\;
\srcnote{service/web3/listener.go:274--322 解码 + 幂等 + 写 web3.payment.confirmed}}
\end{frame}

\begin{frame}[shrink]{真实事故还原：reorg 把已发货订单的钱"撤回"了}
...
\begin{frame}[shrink]{真实事故还原：listener 漏事件怎么人工补}
...
\begin{frame}[shrink]{业务约束：为什么后端永远不能拿到私钥}
...
\begin{frame}[shrink]{签名消息模板：把"我在给 gomall 付款"钉在用户眼前}
...
\begin{frame}[fragile,shrink]{VerifyPersonalSign：EIP-191 + ecrecover 12 行}
...
\begin{frame}[fragile,shrink]{IssueNonce + VerifyAndPark：业务主流程拼图}
...
\begin{frame}[shrink]{Escrow 状态机：5 状态 / 3 角色 / 业务对应}
\begin{center}
...
\end{itemize}}
\srcnote{pkg/web3/contracts/Escrow.sol:13--19 enum；:68--82 fund；:85--107 release/refund/dispute}
\srcnote{pkg/web3/contracts/README.md:1--36 合约职责}
\end{frame}

\begin{frame}[shrink]{合约状态 $\leftrightarrow$ 订单状态：业务对账映射}
\centering
{\small
...
\srcnote{pkg/web3/contracts/README.md:31--36 合约不变式}
\srcnote{service/web3/listener.go:303--311 outbox.Insert 写入 web3.payment.confirmed}
\end{frame}

\begin{frame}[shrink]{金额口径事故：满减订单走链上付款被多扣}
\textbf{业务场景}：买家命中"满 100 减 20"活动，下单页显示实付 \$80，订单库里
\texttt{FinalCents=8000}、\texttt{PromoRuleID} 已写入。买家选了 Web3 通道付款。
\begin{itemize}
  \item \textbf{出事的地方}：Web3 路径计费写死 \texttt{order.Money $\times$ order.Num} —— 这是\textbf{折前总价} \$100，
  完全没看 \texttt{FinalCents}。outbox 事件 \texttt{web3.payment.pending} 的 \texttt{Amount} 落了 10000 分。
  \item \textbf{业务后果}：满减优惠\textbf{静默蒸发}，链上据这个金额向买家钱包要 \$100。买家明明看到 \$80，
  实际被要 \$100，多扣 \$20 —— 而且是用户自付 gas 的链上转账，扣错了退起来比法币更麻烦。
  \item \textbf{为什么只 Web3 中招}：法币 \texttt{/paydown} 路径\textbf{早就}按 \texttt{FinalCents} 折后实付扣款，
  Web3 是后开的通道，漏接了这段口径，成了\textbf{漏网的那一条}。
\end{itemize}
\vspace{1mm}
\small \textbf{工程教训}：一笔订单两条支付路径（法币 / 链上），\textbf{金额口径必须强制对齐}。
任何一条新通道复用订单数据时，都得问一句"折后实付这件事它处理了吗"。
\srcnote{internal/payment/service\_crypto.go:132--135 修复前写死折前价的位置}
\srcnote{internal/payment/service.go:79--82 法币路径早已取 FinalCents（对照基准）}
\end{frame}

\begin{frame}[fragile,shrink]{修法：两条路径统一取 FinalCents 折后实付}
\textbf{修法}：与法币路径一字不差地对齐 —— 命中满减（\texttt{PromoRuleID} 非 0）就取折后实付，
否则才回落单价 $\times$ 件数。
\begin{lstlisting}[language=Go,firstnumber=128,
  caption={\srcnote{internal/payment/service\_crypto.go:128--146 VerifyAndPark 计费口径}}]
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item \texttt{order.Money $\times$ order.Num}：无活动订单的默认口径，单价乘件数；
  \item \textbf{判据用 \texttt{PromoRuleID} 而非 \texttt{FinalCents > 0}}：满减立减到 0 / 100\% 折扣时
  \texttt{FinalCents==0} 是\textbf{合法实付}，用 \texttt{>0} 判会误判成未命中、回退全价（埋第二个坑）；
  \item \texttt{Amount: totalAmount}：链上 listener 与商家对账都读这个值，口径错则全链路错。
\end{itemize}
\srcnote{internal/payment/service\_crypto.go:130--131 判据选 PromoRuleID 的原因（代码注释）}
\srcnote{service/events/events.go:99--109 Web3PaymentPending.Amount 单位为分}
\end{frame}

\begin{frame}[fragile,shrink]{Escrow.sol 关键路径：fundWithOrderID}
\begin{lstlisting}[language=Java,firstnumber=74,
  caption={\srcnote{pkg/web3/contracts/Escrow.sol:74--82 fundWithOrderID}}]
...
\begin{frame}[shrink]{业务边界（显式声明）：Web3 通道里 gomall 不做的事}
诚实列出，对外承诺：
\begin{itemize}
...
\begin{frame}[shrink]{合规边界：链上身份 vs 实名监管}
...
\srcnote{pkg/web3/contracts/Escrow.sol:10 arbiter immutable；pkg/web3/contracts/README.md:102 多签建议}
\end{frame}

\begin{frame}[shrink]{业务承诺（SLO）：Web3 通道的对外承诺}
\begin{tabular}{@{}lll@{}}
\toprule
环节 & SLO 目标 & 当前实测 \\
\midrule
...
\begin{frame}[shrink]{各角色视角：业务侧关心的全图}
\begin{tabular}{@{}lll@{}}
\toprule
角色   & 关心               & 看哪里 \\
\midrule
...
\begin{frame}[shrink]{Arbiter 多签升级路线图}
\begin{tabular}{@{}lll@{}}
\toprule
...
\end{itemize}
\srcnote{pkg/web3/contracts/Escrow.sol:10 arbiter immutable；:57--65 constructor}
\srcnote{pkg/web3/contracts/README.md:102 arbiter 推荐多签}
\end{frame}

\begin{frame}[shrink]{从技术决策回看业务依据}
\small
\begin{tabular}{@{}ll@{}}
\toprule
技术选择                        & 业务依据 \\
...
\srcnote{internal/payment/routes.go:23--27 Web3 路由接入位}
...
\srcnote{pkg/web3/contracts/Escrow.sol:94--100 refund 入口可由 arbiter 触发}
...
\srcnote{internal/payment/routes.go:14--27 两条路由并行}
...
\srcnote{README.md:74--80 业务边界显式列出"不真上链"}
\end{frame}
...
\srcnote{internal/payment/routes.go:25--27 paydown/crypto 套幂等中间件}
...
\srcnote{pkg/web3/contracts/Escrow.sol:57--65 constructor；pkg/web3/contracts/README.md:102}
...
\srcnote{internal/payment/routes.go:14--27 两条通道并行}
\end{frame}

\begin{frame}[shrink]{代码位置一览（上）}
\small
\begin{description}
  \item[Escrow 合约 + 状态机]\srcnote{pkg/web3/contracts/Escrow.sol:7--113}
  \item[immutable 参数 / custom error]\srcnote{pkg/web3/contracts/Escrow.sol:8--11；:32--35}
  \item[fundWithOrderID 入金]\srcnote{pkg/web3/contracts/Escrow.sol:76--82}
  \item[release / refund / dispute]\srcnote{pkg/web3/contracts/Escrow.sol:85--107}
...
\begin{description}
  ...
  \item[API 路由接入位]\srcnote{internal/payment/routes.go:12--28}
  \item[crypto handler]\srcnote{internal/payment/handler\_crypto.go:14--52}
  \item[合约部署 / 多签建议]\srcnote{pkg/web3/contracts/README.md:1--103}
\end{description}
