...
\begin{description}
  \item[Escrow 合约 + 状态机]
  \item[immutable 参数 / custom error]
  \item[fundWithOrderID 入金]
  \item[release / refund / dispute]
  \item[钱包验签 EIP-191]
  \item[Nonce 颁发 + 验签业务]
  \item[签名消息模板 / chainID 防重放]
  \item[Web3 Redis 协议（nonce/pending）]
  \item[Lua 原子 GET+DEL]
\end{description}
\end{frame}
...
\small
\begin{description}
  \item[链上 listener 主循环]
  \item[catch-up 补块]
  \item[reorg 跳过]
  \item[事件级幂等 SETNX 72h]
  \item[last\_block 持久化]
  \item[退避重连]
  \item[静默降级（env 未设跳过）]
  \item[Outbox 总线写入]
  \item[API 路由接入位]
