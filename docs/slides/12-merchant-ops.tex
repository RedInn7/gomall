...
\begin{frame}[shrink]{五个角色，五个截然不同的问题}
...
\begin{frame}[shrink]{商家入驻的业务价值：先做基础再做看板的取舍}
\begin{itemize}
...
\begin{frame}[shrink]{商家三类自助看板：路线图与业务取舍}
...
\begin{frame}[shrink]{可观测分层：每一层解决一个角色的一个问题}
...
\begin{frame}[shrink]{静默降级的业务价值：商家慢 5 分钟 vs 用户搜不到}
\begin{itemize}
...
\begin{frame}[shrink]{一个商家在 gomall 里能干什么、不能干什么}
\begin{itemize}
...
\begin{frame}[shrink]{BossID 散落在五张表里}
...
\begin{frame}[shrink]{merchant Group 总览}
\begin{center}
...
\begin{frame}[shrink]{7 个接口的签名与权限矩阵}
...
\begin{frame}[fragile,shrink]{merchant.Group 注册：18 行代码}
...
\begin{frame}[shrink]{merchant.Group 草案逐行讲解}
\begin{itemize}
...
\begin{frame}[shrink]{从注册到正式上架：5 个阶段}
...
\begin{frame}[shrink]{阶段 2--3：资质上传与 OCR 字段校验}
\begin{itemize}
...
\begin{frame}[shrink]{阶段 4--5：人工审核与试运行}
\begin{itemize}
...
\begin{frame}[shrink]{商家看板首屏的 4 个象限}
...
\begin{frame}[shrink]{4 象限的数据源与计算口径}
\scriptsize
\begin{tabular}{@{}lllll@{}}
\toprule
...
\begin{frame}[shrink]{看板架构：读分离 + 三层缓存 + 异步预热}
...
\begin{frame}[shrink]{今天只有两层墙，明天要加中间一层}
...
\begin{frame}[shrink]{merchant 角色落地需要改的四处}
\begin{enumerate}
...
\begin{frame}[fragile,shrink]{RequireRole + lookupRole：30 秒缓存的取舍}
...
\begin{frame}[shrink]{客服为什么需要 trace：从"客户骂街"到"5 分钟答完"}
\begin{itemize}
...
\begin{frame}[shrink]{一次下单的 trace span 树}
\begin{tikzpicture}[
...
\begin{frame}[fragile,shrink]{Jaeger middleware：traceId 透传}
...
\begin{frame}[shrink]{Jaeger middleware 逐行讲解}
\begin{itemize}
...
\begin{frame}[fragile,shrink]{当旁路埋点反噬主链路：畸形 trace 头吞掉整条请求}
\begin{itemize}
...
\begin{frame}[shrink]{原则：旁路设施 fail-open，核心链路 fail-closed}
\begin{itemize}
...
\begin{frame}[shrink]{Jaeger 的开销：实测低于 5\%}
\begin{tabular}{@{}lrr@{}}
\toprule
场景 & RPS & p95 \\
\midrule
...
\begin{frame}[shrink]{Jaeger 与 Skywalking 同时存在的取舍}
...
\begin{frame}[shrink]{SRE 为什么需要 metrics：运营要"实时 GMV 大盘"}
\begin{itemize}
...
\begin{frame}[shrink]{要给业务接的 4 类 metrics}
\scriptsize
\begin{tabular}{@{}lllll@{}}
\toprule
...
\begin{frame}[fragile,shrink]{接入 Prometheus：30 行起步}
\begin{lstlisting}[language=Go,firstnumber=1,
  caption={}]
package metrics
...
\begin{frame}[shrink]{Prometheus 接入逐行讲解}
\begin{itemize}
...
\begin{frame}[shrink]{Grafana 看板：5 个核心面板}
\scriptsize
\begin{tabular}{@{}llll@{}}
\toprule
面板 & 数据源 & 阈值（黄 / 红） & 告警接收人 \\
...
\begin{frame}[shrink]{客服的"工具盒"：ELK 解决的三类客诉问题}
\begin{itemize}
...
\begin{frame}[shrink]{规模化版本：Filebeat $\to$ Kafka $\to$ Logstash $\to$ ES}
...
\begin{frame}[shrink]{gomall 启动期的四个 tryInit}
...
\begin{frame}[fragile,shrink]{tryInitES：8 行 recover 套路}
...
\begin{frame}[shrink]{tryInitES 逐行讲解：为什么不传 error}
\begin{itemize}
...
\begin{frame}[shrink]{业务码：现有 5 段 + 待加 80xxx 商家段}
\scriptsize
\begin{tabular}{@{}llll@{}}
\toprule
段 & 含义 & 现有 / 新增码 & 客服话术示例 \\
...
\begin{frame}[shrink]{静默降级的 5 条铁律（提炼自 cmd/main.go 现状）}
\begin{enumerate}
...
\begin{frame}[fragile,shrink]{静默降级的请求期版本：handler 内的弱依赖兜底}
\begin{lstlisting}[language=Go,firstnumber=1,
  caption={}]
...
\begin{frame}[shrink]{请求期降级套路逐行讲解}
\begin{itemize}
...
\begin{frame}[shrink]{gomall 的 4 级 SLO 表与年宕预算（与 README 对齐）}
\scriptsize
\begin{tabular}{@{}lllll@{}}
\toprule
...
\begin{frame}[shrink]{P0--P3 告警分级与响应 SLA}
\scriptsize
\begin{tabular}{@{}lllll@{}}
\toprule
...
\begin{frame}[shrink]{P0--P3 的业务后果：每一级对应的 GMV 损失}
\scriptsize
\begin{tabular}{@{}llll@{}}
\toprule
...
\begin{frame}[shrink]{SLO 燃尽预算（Error Budget Burn Rate）}
\begin{itemize}
...
\begin{frame}[shrink]{真实案例：从 trace 找到订单列表深分页问题}
\begin{itemize}
...
\begin{frame}[shrink]{业务边界：商家后台 + 可观测不做什么}
\begin{itemize}
...
\begin{frame}[shrink]{五角色共用一套底层信号 + 三件套闭环}
\begin{tikzpicture}[node distance=4mm and 8mm,every node/.append style={align=center,font=\scriptsize,text width=22mm}]
...
\begin{frame}[shrink]{面试 Q\&A（1/2）：架构与权限}
...
\begin{frame}[shrink]{面试 Q\&A（2/2）：可观测性与降级}
...
\begin{description}
  \item[BossID 散落字段]
  \item[RBAC 中间件]
  \item[Jaeger middleware]
  \item[Trace utility]
  \item[Skywalking agent]
  \item[logrus 初始化]
  \item[静默降级（启动期）]
  \item[静默降级（请求期）]
  \item[merchant.Group 草案]
  \item[merchant API 7 接口]
  \item[商家入驻申请]
  \item[Prometheus metrics]
  \item[ELK / EFK 落地]
  \item[admin promote 模板]
  \item[admin 路由组]
  \item[CircuitBreaker 配置]
  \item[压测锚点]
\end{description}
\vspace{1mm}
