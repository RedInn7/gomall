...
\srcnote{repository/db/model/order.go:12 BossID 数据基础已就位；五张表 BossID 见下一帧}
\end{frame}
...
\end{itemize}
\srcnote{repository/db/model/order.go:12、product.go:23、cart.go:10、favorite.go:12、skill\_product BossID 五处}
\end{frame}
...
\end{itemize}
\srcnote{middleware/track.go:11--29 Jaeger 中间件；pkg/utils/log/logger.go logrus；Prometheus 路线图}
\end{frame}
...
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\srcnote{cmd/main.go:37--42 调用、:54--97 四个 tryInit；README 技术亮点 \#11 静默降级哲学}
\end{frame}
...
\end{itemize}
\srcnote{repository/db/model/order.go:12、product.go:23、cart.go:10、favorite.go:12 五处 BossID；README 业务边界"没有 merchant role"}
\end{frame}
...
\end{itemize}
\srcnote{middleware/track.go:11--29 traceId 透传；客户端弹窗带 traceId 是产品侧约定}
\end{frame}
