...
\srcnote{internal/order/model.go:12 BossID 数据基础已就位；五张表 BossID 见下一帧}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/model.go:12、internal/product/model.go:23、internal/cart/model.go:10、internal/favorite/model.go:12、internal/skill/model.go BossID 五处}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/model.go:12、internal/product/model.go:23、internal/cart/model.go:10、internal/favorite/model.go:12 五处 BossID；README 业务边界"没有 merchant role"}
\end{frame}
...
\srcnote{internal/order/model.go:12、internal/product/model.go:23、internal/cart/model.go:10、internal/favorite/model.go:12}
\end{frame}
...
\end{itemize}
\srcnote{建议复用 internal/admin/service.go:46--62 PromoteToAdmin 模板做 PromoteToMerchant；试运行限额可在 RBAC 之后挂一个 trial 中间件}
\end{frame}
...
\srcnote{internal/order/model.go:12 BossID + Type；internal/product/model.go:23 BossID + Num}
\end{frame}
...
\begin{enumerate}
  \item \texttt{internal/user/model.go:27--32}：新增常量 \texttt{RoleMerchant = "merchant"}。Role 字段已有，无需 migration。
  ...
  \item \texttt{internal/admin/service.go}：补一个 \texttt{PromoteToMerchant}，参考 \texttt{PromoteToAdmin:46--62} 直接复刻；记得调 \texttt{InvalidateRoleCache}。
\end{enumerate}
\vspace{2mm}
\small 四处改动各 5--15 行，不需要任何 DB migration。整套路线图\textbf{两个 PR 就能合}。
\srcnote{routes/router.go:148--155 现成模板；internal/admin/service.go:46--62 promote 逻辑可复用}
\end{frame}
...
\begin{lstlisting}[language=Go,firstnumber=1,
  caption={\srcnote{service/search/product\_query.go 草案；与 cmd/main.go:64--73 同构}}]
...
\scriptsize
\begin{description}
  \item[BossID 散落字段]\srcnote{internal/order/model.go:12, internal/product/model.go:23, internal/cart/model.go:10, internal/favorite/model.go:12}
  ...
  \item[admin promote 模板]\srcnote{internal/admin/service.go:46--62 PromoteToAdmin}
