...
\bottomrule
\end{tabular}
\srcnote{对照行业漏斗经验值 + gomall README \S 业务承诺；service/order.go:40--102 下单入口}
\end{frame}
...
\begin{frame}{业务边界：下单\textbf{不做}什么}
\begin{itemize}
...
\srcnote{README.md \S 业务边界（不做什么）；service/order.go:46--55 当前 Order 模型字段}
\end{frame}
...
\vspace{2mm}
\begin{itemize}
  \item \texttt{ErrorProductMoreCart=20008}：重复加购同款 → 明确业务码，客服一查就知。
...
\end{itemize}
\srcnote{service/cart.go:36--49 + pkg/e/code.go:31--32 业务码 20007/20008}
\end{frame}
...
\end{itemize}
\vspace{2mm}
\textbf{业务硬约束（gomall 现状）}：\texttt{Address} 只保留 \texttt{name / phone / address} 三字段，\\
合并省事但订单一旦发起就该\textbf{冻结地址快照}，否则用户改地址簿 → 历史订单收货地址被同步改 = 客服恶梦。
\srcnote{service/address.go:34--46 当前最小模型；repository/db/model/order.go AddressID 引用关系（待加 snapshot 字段）}
\end{frame}
...
\begin{frame}{为什么订单不用 DB 自增 ID（业务侧理由）}
\begin{tabular}{@{}lll@{}}
\toprule
方案 & 业务问题 & 性能问题 \\
\midrule
...
\small \textbf{业务三连问}：
\begin{itemize}
  \item 竞争对手凌晨爬一遍订单号 \texttt{id+1} → 算出我们当日单量 → 友商情报泄露。
  \item 订单号要塞进 \textbf{短信 / 客服系统 / 退款单}，太长用户读不出来（雪花 19 位刚好）。
  \item 索引页要顺序写，UUID 把 B+ 树搅得到处分裂 = 写放大 + 内存抖动。
\end{itemize}
\srcnote{pkg/utils/snowflake/snowflake.go:10--25 InitSnowflake}
\end{frame}

\begin{frame}{雪花订单号的\textbf{业务价值帧}}
...
\end{tabular}
\vspace{3mm}
\small 客服日常话术："您报一下订单号" → 看前几位毫秒戳 → 反算下单时刻 → 立刻匹配那一刻的库存 / 限流 / 网络日志 → 30s 内定位问题。\\
对账场景：商家按"昨天的订单号区间"拉一遍 → 不用 SELECT \dots WHERE created\_at，直接 \texttt{order\_num BETWEEN}，索引走得飞快。
...
\begin{frame}{30 分钟未付窗口：商家 vs 用户的拉扯}
\begin{tikzpicture}[node distance=8mm and 12mm]
  ...
  \node[node-box,below=of w,node-alert]    (t) {超时未付\\reserved $\to$ available};
  \draw[arrow] (u) -- (c);
  \draw[arrow] (c) -- (w);
  \draw[arrow] (w) -- (p);
  \draw[arrow-alert] (w) -- (t);
\end{tikzpicture}
\vspace{2mm}
...
\srcnote{repository/cache/inventory.go:25--26 ErrStockInsufficient；service/order.go:88--92 15min TTL（注：与 30min 业务承诺待对齐）}
\end{frame}

\begin{frame}{为什么不"下单即真扣"}
\begin{itemize}
  \item \textbf{方案 A：下单立即扣 sold} —— 用户取消 / 未支付 = 库存白损失，得回滚 sold 表 = 状态机更复杂。
  \item \textbf{方案 B：支付时才扣（不预扣）} —— 用户 A 拍下 → 用户 B 同时拍下 → 两人都付钱 → 超卖。
...
任何一刻巡检脚本读三个桶相加 = 商品总库存 → 不平就告警。
\srcnote{repository/cache/inventory.go:77--125 reserve/commit/release 三脚 Lua}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{第 3 步}：投递延迟队列，超时未付款自动取消 → reserved 退回 available。
\end{itemize}
\srcnote{service/order.go:40--102 全函数；service/order.go:64--78 同事务写 order + outbox}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{tx 失败要补偿 release}：reserve 成功但 DB 失败 → 必须把扣的库存放回 available，否则\textbf{虚假占用}让其他用户也下不了单。
...
\begin{itemize}
  ...
  \begin{itemize}
    \item 写订单成功、发 MQ 前进程挂了 → \textbf{下游永远收不到 OrderCreated} → 不发货 → 客诉。
    \item 反过来：先发 MQ 再写 DB，DB 失败 → \textbf{幻影订单事件} → 发了不存在的货 → 商家亏。
  \end{itemize}
...
\begin{frame}{重复下单：用户狂点的真实场景}
\begin{itemize}
  \item \textbf{网络抖一下}：用户点"提交" → 转圈 → 没反应 → 再点一下 → 再点 → 连点 5 下。
...
\textbf{没有幂等会发生什么}：
\begin{itemize}
  \item 用户被\textbf{重复扣款} → 客服爆单 → 退款 → 信任崩塌。
  \item 库存被多次扣减 → 真实库存 = 5 但 reserved = 25 → 后来者全被拒。
  \item 商家看到 5 笔重复订单 → 不知道发不发货。
\end{itemize}
\srcnote{middleware/idempotency.go:35--112 Idempotency 中间件}
\end{frame}

\begin{frame}{业务码 60002：客服一看就懂}
\begin{tabular}{@{}lll@{}}
\toprule
状态码 & 含义        & 客服话术 \\
\midrule
...
\begin{frame}[fragile]{Idempotency 中间件：状态机三态}
...
\begin{frame}{逐行讲：三态对应三种客户体验}
\begin{description}\setlength{\itemsep}{0.5mm}
  \item[\texttt{state=0}] token 过期 / 没传 → 让客户端\textbf{重新生成 key 再来} —— 防止陈年 token 重放攻击。
  \item[\texttt{state=2}] 已经成功过了 → \textbf{回放上次的 JSON} —— 用户看到的还是那笔订单号、同样的 money，体验上"我就下了一次"。
  \item[\texttt{state=3}] 还在处理中 → 返回 60002，前端\textbf{灰掉按钮}+ 500ms 后重试 —— 阻断"看似没反应就狂点"。
  \item[\texttt{state=1}] 抢到锁，真正下单 → \texttt{responseRecorder} 拷贝响应体 → 写回 done 态 → 下次重放就走 case 2。
\end{description}
\srcnote{middleware/idempotency.go:96--110 responseRecorder + CommitIdempotencyResult}
\end{frame}

\begin{frame}{压测锚点：50,319 RPS / 755K 次 = 1 笔订单}
\begin{tabular}{@{}lrr@{}}
\toprule
场景 & 实测 & 备注 \\
\midrule
...
\srcnote{stressTest/REPORT.md \S 4 幂等中间件 755{,}033 次 → 1 笔订单}
\end{frame}
...
\vspace{3mm}
\begin{itemize}
  \item \textbf{同步}：用户等响应的同时 DB 在写；DB 抖动 → 全部超时 → 转圈 → 流失。
  \item \textbf{异步}：MQ 顶住瞬时压力，consumer 平滑落库；代价 = 用户多一次轮询。
  \item \textbf{业务承诺}：P0 接口（下单 / 支付）\textbf{不挂限流}，靠异步削峰扛峰值（README SLO 行）。
\end{itemize}
\srcnote{routes/router.go:92--95 两条路同挂 Idempotency；README.md \S 业务承诺 P0 不挂限流}
\end{frame}
...
\end{tikzpicture}
\srcnote{service/order\_async.go:119--172 enqueue；:175--188 status}
\end{frame}
...
\begin{frame}{逐行讲：ticket 与 TTL 的业务取舍}
\begin{itemize}
...
\end{itemize}
\srcnote{service/order\_async.go:23--52 常量与状态结构；:164--168 失败写 failed}
\end{frame}
...
任何一个不变量被破坏，巡检脚本应当告警：
\begin{itemize}
  \item Redis 双桶不平 → 库存对账脚本（\texttt{GetStockSnapshot}）→ \textbf{客服报"我有 5 件但显示售罄"时第一时间查}。
  \item 订单与 outbox 数量不一致 → 直接 SQL JOIN 抽查 → \textbf{商家报"看不到这单"时第一时间查}。
  \item ticket=ok 但订单不存在 → consumer 写回路径异常 → \textbf{用户报"拍下了但订单页空"时第一时间查}。
\end{itemize}
...
\srcnote{service/events/order\_created.go OrderCreated 已有 UserID/ProductID，扩展加风控字段}
\end{frame}
...
\hspace{4mm}A：用户网络抖 / 双 11 客户端重试风暴 / 脚本党 → 不会重复扣款、不会重复占库存、商家不会看到 5 笔幽灵订单。
\end{frame}
...
\begin{description}\setlength{\itemsep}{0.2mm}
  ...
  \item[同步下单主体]\srcnote{service/order.go:40--102 OrderCreate}
  \item[库存预扣/提交/释放]\srcnote{repository/cache/inventory.go:77--125 三脚 Lua}
  \item[outbox 同事务写入 + 扫表]\srcnote{repository/db/dao/outbox.go:26--78 Insert/FetchBatch/MarkSent/MarkFailed}
  \item[异步下单 + ticket 状态]\srcnote{service/order\_async.go:23--52 状态常量；:119--188 enqueue+status}
  ...
  \item[路由]\srcnote{routes/router.go:92--111 orders + carts + addresses}
\end{description}
\vspace{1mm}
