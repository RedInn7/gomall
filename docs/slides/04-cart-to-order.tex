...
\end{tabular}
\par\smallskip
{\centering\srcnote{对照行业漏斗经验值 + gomall README \S 业务承诺；internal/order/service.go:40--102 下单入口}\par}
\end{frame}
...
\srcnote{README.md \S 业务边界（不做什么）；internal/order/service.go:46--55 当前 Order 模型字段}
\end{frame}
...
\end{itemize}
\srcnote{internal/cart/service.go:36--49 + pkg/e/code.go:31--32 业务码 20007/20008}
\end{frame}
...
\srcnote{internal/address/service.go:34--46 当前最小模型；internal/order/model.go AddressID 引用关系（待加 snapshot 字段）}
\end{frame}
...
\srcnote{repository/cache/inventory.go:25--26 ErrStockInsufficient；internal/order/service.go:88--92 15min TTL（注：与 30min 业务承诺待对齐）}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/service.go:40--102 全函数；internal/order/service.go:64--78 同事务写 order + outbox}
\end{frame}

\begin{frame}[fragile]{OrderCreate 主体三步}
\begin{lstlisting}[language=Go,firstnumber=46,
  caption={\srcnote{internal/order/service.go:46--86}}]
order := &Order{
    UserID:    u.Id,
...
\srcnote{internal/shared/outbox/repo.go:44--78 FetchBatch + MarkSent + MarkFailed}
\end{frame}
...
\end{tikzpicture}}
\srcnote{internal/order/async.go:119--172 enqueue；:175--188 status}
\end{frame}

\begin{frame}[fragile]{OrderEnqueue 核心 12 行}
\begin{lstlisting}[language=Go,firstnumber=119,
  caption={\srcnote{internal/order/async.go:119--150}}]
...
\end{itemize}
\srcnote{internal/order/async.go:23--52 常量与状态结构；:164--168 失败写 failed}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/async.go:47--52 OrderTicketStatus 三状态}
\end{frame}
...
\srcnote{internal/shared/outbox/repo.go:62--78 MarkFailed 退避策略；internal/order/service.go:79--85 释放路径}
\end{frame}
...
\begin{description}\setlength{\itemsep}{0.2mm}
  \item[加购 / 地址]\srcnote{internal/cart/service.go:29--103；internal/address/service.go:27--127}
  \item[雪花生成]\srcnote{pkg/utils/snowflake/snowflake.go:10--25}
  \item[同步下单主体]\srcnote{internal/order/service.go:40--102 OrderCreate}
  \item[库存预扣/提交/释放]\srcnote{repository/cache/inventory.go:77--125 三脚 Lua}
  \item[outbox 同事务写入 + 扫表]\srcnote{internal/shared/outbox/repo.go:26--78 Insert/FetchBatch/MarkSent/MarkFailed}
  \item[异步下单 + ticket 状态]\srcnote{internal/order/async.go:23--52 状态常量；:119--188 enqueue+status}
  ...
  \item[OrderCreated 事件 / publisher]\srcnote{service/events/order\_created.go；internal/shared/outbox/publisher.go}
