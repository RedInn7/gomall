...
\end{itemize}
\srcnote{stressTest/REPORT.md \S 1 基线 64k RPS；internal/order/async.go:1--188 异步削峰方案}
\end{frame}
...
\end{itemize}
\srcnote{routes/router.go:79--84 红包；:136--142 秒杀；internal/skill/service.go 秒杀逻辑}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/async.go:1--188；internal/order/consumer.go；repository/rabbitmq/order\_async.go}
\end{frame}
...
\srcnote{internal/order/async.go OrderEnqueue + 前端轮询协议}
\end{frame}
...
\srcnote{internal/order/handler.go：EnqueueOrderHandler + OrderStatusHandler; internal/order/async.go}
\end{frame}

\begin{frame}[fragile]{OrderEnqueue：核心 12 行}
\begin{lstlisting}[language=Go,firstnumber=119,basicstyle=\ttfamily\tiny,
  caption={\srcnote{internal/order/async.go:119--172}}]
...
\hspace{4mm}\small A：不会。库存先 ReserveStock（Redis Lua 原子），任一失败都 ReleaseReservation 回滚；ticket TTL=1h 兜底；MQ 用 RabbitMQ 持久化 + 手动 ack。代码 \texttt{internal/order/async.go:119--172}。运营 SLO：99.95\% 可用 = 年宕 4h22min，异步路径 p95 入队 $<$ 10ms、轮询 $<$ 3s 拿结果。
...
\begin{description}
  ...
  \item[OrderEnqueue] \srcnote{internal/order/async.go:119--172}
