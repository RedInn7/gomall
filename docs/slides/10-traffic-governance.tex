...
\end{itemize}
\srcnote{routes/router.go:41 TokenBucket(rate.Limit(100), 200)；middleware/ratelimit.go:35--78}
\end{frame}
...
\end{itemize}
\srcnote{middleware/ratelimit.go:35--78 golang.org/x/time/rate 实现}
\end{frame}

\begin{frame}{业务困局：限流器自己把整机内存吃光}
\begin{itemize}
...
\end{itemize}
\srcnote{middleware/ratelimit.go:19--24 TTL 常量；26--29 entry 带 lastSeen；51--65 janitor}
\end{frame}

\begin{frame}[fragile]{TokenBucket 中间件实现 (逐行)}
\begin{lstlisting}[language=Go,firstnumber=35,basicstyle=\ttfamily\tiny,
  caption={\srcnote{middleware/ratelimit.go:35--78}}]
...
\end{itemize}
\srcnote{internal/payment/routes.go:14--21 paydown 接 CB；middleware/circuitbreaker.go:45--140}
\end{frame}
...
\srcnote{middleware/circuitbreaker.go:77--130 allow + report；123 halfOpenSuccess 判据}
\end{frame}

\begin{frame}{业务困局：半开期一个偶发成功就放全量流量}
\begin{itemize}
...
\end{itemize}
\srcnote{middleware/circuitbreaker.go:36 halfOpenSuccess 字段；120--128 report 判据}
\end{frame}

\begin{frame}[fragile]{CircuitBreaker.allow 状态转移 (逐行)}
\begin{lstlisting}[language=Go,firstnumber=77,basicstyle=\ttfamily\tiny,
  caption={\srcnote{middleware/circuitbreaker.go:77--108 allow}}]
...
\begin{frame}[fragile]{report：必须全部探测成功才回 Closed}
\begin{lstlisting}[language=Go,firstnumber=110,basicstyle=\ttfamily\tiny,
  caption={\srcnote{middleware/circuitbreaker.go:110--130 report}}]
...
\end{itemize}
\srcnote{internal/payment/routes.go:14--21；middleware/circuitbreaker.go:58--66；pkg/e/code.go:58}
\end{frame}
...
\end{itemize}
\srcnote{internal/payment/routes.go:14--21；middleware/circuitbreaker.go:58--66}
\end{frame}
...
\end{itemize}
\srcnote{middleware/circuitbreaker.go:45--140 单实例 CB; internal/payment/routes.go:14--21 仅 CB-A 接入}
\end{frame}
...
\end{itemize}
\srcnote{middleware/circuitbreaker.go:70 失败判定 = 5xx 或 c.Errors; 兜底路径需要在 handler 内吞掉 5xx 改成 200+stale}
\end{frame}
...
\srcnote{middleware/ratelimit.go:35--78 token-bucket; repository/cache/ratelimit.go:18--34 sliding-log}
\end{frame}
...
\begin{description}
  ...
  \item[TokenBucket] \srcnote{middleware/ratelimit.go:35--78（含 51--65 janitor 回收空闲限流器）}
  \item[SlidingWindow] \srcnote{middleware/ratelimit.go:60--107 + repository/cache/ratelimit.go:18--34}
  \item[CircuitBreaker] \srcnote{middleware/circuitbreaker.go:45--140（半开回 Closed 需全部探测成功，123）}
