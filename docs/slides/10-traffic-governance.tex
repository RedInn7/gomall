...
\end{itemize}
\srcnote{stressTest/REPORT.md \S 1 基线 64k RPS；internal/order/async.go:1--187 异步削峰方案}
\end{frame}
...
\end{itemize}
\srcnote{README.md SLO 表；routes/router.go:41 全局令牌桶；internal/payment/routes.go:14--21；internal/skill/routes.go:17--24}
\end{frame}
...
\srcnote{routes/router.go:41 全局令牌桶；其余挂载点见 internal/\{product,redpacket,skill,payment,order\}/routes.go}
\end{frame}
...
\srcnote{routes/router.go:41 全局令牌桶；internal/product/routes.go:15--16；internal/redpacket/routes.go:20--25；internal/payment/routes.go:14--21；internal/skill/routes.go:17--24}
\end{frame}
...
\end{itemize}
\srcnote{routes/router.go:41 TokenBucket(rate.Limit(100), 200)；middleware/ratelimit.go:23--50}
\end{frame}
...
\end{itemize}
\srcnote{routes/router.go:41；stressTest/REPORT.md \S 1 单机 64{,}254 RPS}
\end{frame}
...
\end{itemize}
\srcnote{middleware/ratelimit.go:23--50 golang.org/x/time/rate 实现}
\end{frame}

\begin{frame}[fragile]{TokenBucket 中间件实现 (逐行)}
\begin{lstlisting}[language=Go,firstnumber=23,basicstyle=\ttfamily\tiny,
  caption={\srcnote{middleware/ratelimit.go:23--50}}]
...
\end{itemize}
\srcnote{internal/redpacket/routes.go:20--25 红包；internal/skill/routes.go:17--24 秒杀；internal/skill/service.go 秒杀逻辑}
\end{frame}
...
\end{itemize}
\srcnote{internal/skill/routes.go:17--24 Scope:seckill Limit:3 Window:1s ByUser:true}
\end{frame}
...
\end{itemize}
\srcnote{repository/cache/ratelimit.go:18--34 Lua 脚本; middleware/ratelimit.go:70--106 调用方}
\end{frame}
...
\end{itemize}
\srcnote{internal/redpacket/routes.go:20--25 红包抢包；internal/skill/routes.go:17--24 秒杀；幂等链 internal/order/routes.go:12, 14 与 internal/payment/routes.go:20, 26}
\end{frame}
...
\end{itemize}
\srcnote{internal/payment/routes.go:14--21 paydown 接 CB；middleware/circuitbreaker.go:44--136}
\end{frame}
...
\end{itemize}
\srcnote{internal/payment/routes.go:14--21；middleware/circuitbreaker.go 三态机；deck 11 Outbox 配套}
\end{frame}
...
\end{itemize}
\srcnote{internal/payment/routes.go:14--21；middleware/circuitbreaker.go:57--65；pkg/e/code.go:58}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/async.go:1--187；internal/order/consumer.go；repository/rabbitmq/order\_async.go}
\end{frame}
...
\begin{lstlisting}[language=Go,firstnumber=118,basicstyle=\ttfamily\tiny,
  caption={\srcnote{internal/order/async.go:118--171}}]
...
\end{itemize}
\srcnote{middleware/httpcache.go:47--100 HTTPCache; internal/product/routes.go:15--16 等挂载点}
\end{frame}
...
\end{itemize}
\srcnote{middleware/httpcache.go:78--98 ETag 协商；internal/product/routes.go:15 product/list max-age=30}
\end{frame}
...
\srcnote{internal/product/routes.go:15--16、internal/category/routes.go:13、internal/carousel/routes.go:13 各路由的 maxAge 参数}
\end{frame}
...
\end{itemize}
\srcnote{internal/redpacket/routes.go:20--25；internal/skill/routes.go:17--24；pkg/e/msg.go:51}
\end{frame}
...
\end{itemize}
\srcnote{internal/payment/routes.go:14--21；middleware/circuitbreaker.go:57--65}
\end{frame}
...
\end{itemize}
\srcnote{routes/router.go:41 (L2); internal/skill/routes.go:17--24 (L3)}
\end{frame}
...
\end{itemize}
\srcnote{middleware/circuitbreaker.go:44--136 单实例 CB; internal/payment/routes.go:14--21 仅 CB-A 接入}
\end{frame}
...
\srcnote{middleware/ratelimit.go:23--50 token-bucket; repository/cache/ratelimit.go:18--34 sliding-log}
\end{frame}
...
\footnotesize
\begin{description}
  \item[router 挂载点] \srcnote{routes/router.go:41 全局令牌桶；internal/product/routes.go:15--16；internal/redpacket/routes.go:20--25；internal/payment/routes.go:14--21；internal/skill/routes.go:17--24}
  \item[TokenBucket] \srcnote{middleware/ratelimit.go:23--50}
  \item[SlidingWindow] \srcnote{middleware/ratelimit.go:60--107 + repository/cache/ratelimit.go:18--34}
  \item[CircuitBreaker] \srcnote{middleware/circuitbreaker.go:44--136}
  \item[HTTPCache] \srcnote{middleware/httpcache.go:47--100}
  \item[OrderEnqueue] \srcnote{internal/order/async.go:118--171}
