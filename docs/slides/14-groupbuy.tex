...
\end{itemize}
\srcnote{consts/order.go:15 OrderWaitGroup=8；service/order\_state.go:27 WaitGroup $\to$ WaitShip / Closed}
\end{frame}
...
func runGroupbuyExpireSweep() {
    ...
    ids, _ := dao.NewGroupbuyDao(ctx).ExpireOpenGroupsBefore(time.Now(), 200)
...
\end{itemize}
\srcnote{initialize/cron.go:62--72 @every 5m 注册；initialize/cron.go:93--110 runGroupbuyExpireSweep；repository/db/dao/groupbuy.go::ExpireOpenGroupsBefore}
\end{frame}
...
\end{itemize}
\srcnote{initialize/cron.go:62--72 注册；initialize/cron.go:93--110 sweep；repository/db/dao/groupbuy.go::ExpireOpenGroupsBefore}
\end{frame}
...
\hspace{4mm}\srcnote{initialize/cron.go:62 GroupbuyExpire @every 5m；service/groupbuy.go::JoinGroup 末尾 mark 失败只记录日志}
