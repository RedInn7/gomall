...
\begin{frame}[shrink]{拼团不是促销活动，是一种履约前置的销售模型}
\begin{itemize}
  \item 普通下单：用户决策 \textbf{$\to$} 单人付款 \textbf{$\to$} 商家发货。每一笔订单都是\textbf{孤立事件}。
  \item 拼团下单：用户决策 \textbf{$\to$} 多人凑齐 \textbf{$\to$} 一起发货。订单之间\textbf{绑成一组}，要么集体推进、要么集体退款。
  \item 业务关键：拼团把\textbf{用户社交关系}变成商家\textbf{库存利用率的杠杆} —— 商家用更低的价格换确定性销量。
  \item 5 角色看到 5 件不同的事，这就是为什么团购必须是独立 deck。
\end{itemize}
\srcnote{internal/groupbuy/service.go::CreateGroup / JoinGroup / MarkGroupSuccess / ExpireGroup 四个入口}
\end{frame}

\begin{frame}[shrink]{拼团的 5 角色视图}
\scriptsize
...
\begin{frame}[shrink]{业务承诺：拼团接口的 SLO}
\begin{tabular}{@{}llll@{}}
\toprule
接口 & 等级 & 可用率 & p99 延迟 \\
\midrule
...
\begin{frame}[shrink]{业务边界：拼团这次做什么、不做什么}
...
\end{itemize}
\srcnote{internal/groupbuy/handler.go 仅有 3 个 handler；README 业务边界一节列出"未做清单"}
\end{frame}
...
\begin{frame}[shrink]{业务侧主流程：1 个团长 + N-1 个团员的协同时序}
\begin{tikzpicture}[node distance=4mm and 8mm,every node/.style={font=\scriptsize}]
  ...
  \node[node-box,right=of s3,node-ok] (s4a) {\textbf{4a. 成团}\\WaitGroup$\to$WaitShip};
  \node[node-box,below=8mm of s3,node-bad] (s4b) {\textbf{4b. 散团}\\WaitGroup$\to$Closed};
  \draw[arrow] (s1) -- (s2);
  \draw[arrow] (s2) -- (s3);
  \draw[arrow] (s3) -- (s4a);
...
\end{itemize}
\srcnote{consts/order.go:15 OrderWaitGroup=8；internal/order/state.go:27 WaitGroup $\to$ WaitShip / Closed}
\end{frame}

\begin{frame}[shrink]{为什么需要 WaitGroup 过渡态：3 态够不够}
\begin{itemize}
...
业务侧的状态划分 = \textbf{角色间需要协同的最小事件}。3 + group\_id 是把字段当状态用，必爆。
\end{tcolorbox}
\end{center}
\srcnote{consts/order.go:15 OrderWaitGroup 注释；internal/order/state.go:21--28 转换表新增的两条出边}
\end{frame}

\begin{frame}[fragile,shrink]{节点 1 · 团长发起：业务诉求 + 关键代码}
\begin{itemize}
  \item 业务侧：团长选 1 个商品、设成团人数 N、设拼团价（低于正价）、设 TTL（默认 24h）。
  \item 系统侧：建团 + 团长订单 + 1 份预扣库存 + outbox 同事务原子提交。
  \item 失败兜底：tx 回滚 $\to$ 释放刚扣的 reserved（Saga），不会留下"团没建但库存被锁"的脏数据。
\end{itemize}
\begin{lstlisting}[language=Go,caption={CreateGroup 关键路径 internal/groupbuy/service.go}]
...
err := dao.NewDBClient(ctx).Transaction(func(tx *gorm.DB) error {
    if e := NewGroupbuyDaoByDB(tx).CreateGroup(g); e != nil { return e }
    if e := orderpkg.NewOrderDaoByDB(tx).CreateOrder(leaderOrder); e != nil { return e }
...
\end{lstlisting}
\srcnote{internal/groupbuy/service.go:77--158 CreateGroup；与 internal/order/service.go::OrderCreate 同一模式}
\end{frame}

\begin{frame}[fragile,shrink]{节点 2 · 成员加入：单 SQL 抢名额 + uniqueIndex 兜底}
\begin{itemize}
  \item 业务侧：用户扫团长分享链接 \texttt{GET /groupbuy/:id} $\to$ 看落地页 $\to$ \texttt{POST /groupbuy/:id/join}。
...
\end{itemize}
\begin{lstlisting}[language=Go,caption={JoinGroupAtomic 单 SQL internal/groupbuy/repo.go}]
...
\end{lstlisting}
\srcnote{internal/groupbuy/repo.go:55--73 JoinGroupAtomic；uniqueIndex 见 internal/groupbuy/model.go:46--47}
\end{frame}

\begin{frame}[shrink]{节点 3 · 凑齐成团：reserved $\to$ sold 的 Saga 时序}
\begin{tikzpicture}[node distance=4mm and 8mm,every node/.style={font=\scriptsize}]
  ...
  \node[node-box,below=8mm of j3] (j5) {tx: UPDATE order\\WaitGroup$\to$WaitShip};
...
\vspace{2mm}
\begin{itemize}
  \item 成团触发在 \textbf{第 N 个人 join 后}，调 \texttt{MarkGroupSuccess} —— 失败仅 ERROR log，cron \texttt{@every 5m} 兜底（已落地，\texttt{initialize/cron.go:64}）。
...
\end{itemize}
\srcnote{internal/groupbuy/service.go::MarkGroupSuccess；repository/cache/inventory.go::CommitReservation}
\end{frame}

\begin{frame}[fragile,shrink]{节点 4 · 24h 散团：协同式 Saga 一气呵成}
\begin{itemize}
  \item 触发口径：cron 5min 扫一次 \texttt{ExpireOpenGroupsBefore(now)}，把超过 24h 仍 open 的团逐个 \texttt{ExpireGroup}。
  \item 散团操作（tx 内）：MarkGroupExpired + 所有成员订单 \texttt{WaitGroup $\to$ Closed} + member.status=refunded + outbox。
...
\end{itemize}
\begin{lstlisting}[language=Go,caption={ExpireGroup Saga 散团 internal/groupbuy/service.go}]
err = dao.NewDBClient(ctx).Transaction(func(tx *gorm.DB) error {
    ok, e := NewGroupbuyDaoByDB(tx).MarkGroupExpired(groupID)
    if !ok { return nil } // 幂等 no-op
    for _, m := range members {
        tx.Model(&orderpkg.Order{}).Where("id=? AND type=?",
...
\end{lstlisting}
\srcnote{internal/groupbuy/service.go::ExpireGroup；internal/groupbuy/repo.go::MarkGroupExpired \& ExpireOpenGroupsBefore}
\end{frame}
...
\begin{frame}[shrink]{库存两桶模型在拼团里的承载}
\begin{center}
\begin{tikzpicture}[node distance=7mm and 18mm,every node/.style={font=\scriptsize}]
  ...
  \node[font=\tiny,below=3mm of avail,align=center] (a1) {ReserveStock(1)\\每人 join 扣 1};
  \node[font=\tiny,below=3mm of res,align=center] (a2) {Commit：成团\\Release：散团};
  \node[font=\tiny,below=3mm of sold,align=center] (a3) {真扣到 sold\\商家发货依据};

  \draw[arrow]      ([yshift=2pt]avail.east) -- node[above,font=\scriptsize\sffamily]{reserve} ([yshift=2pt]res.west);
  \draw[arrow-bad]  ([yshift=-2pt]res.west) -- node[below,font=\scriptsize\sffamily]{release} ([yshift=-2pt]avail.east);
  \draw[arrow]      (res.east) -- node[above,font=\scriptsize\sffamily]{commit} (sold.west);
\end{tikzpicture}
\end{center}
\vspace{2mm}
\begin{itemize}
...
\end{itemize}
\srcnote{repository/cache/inventory.go:29--65 三段 Lua；internal/groupbuy/service.go::CreateGroup / JoinGroup / ExpireGroup 复用入口}
\end{frame}

\begin{frame}[shrink]{真实数字：reserved 桶为什么能扛拼团峰值}
\begin{itemize}
...
\begin{frame}[shrink]{帧：散团 vs 成团的库存数学差异}
\begin{tabular}{@{}lllll@{}}
\toprule
...
满团 + 失败 commit & 100  & 3        & 3 $\to$ \textbf{3} & 等下次 cron 重 commit \\
\bottomrule
\end{tabular}
\vspace{2mm}
\begin{itemize}
  \item 散团的真实代价不是退款（钱包做），而是\textbf{库存在 reserved 桶里被冻结 24h}，期间这部分库存无法卖给其他用户。
  \item 商家定价取舍：拼团价定得低 $\to$ 散团率高 $\to$ 库存空转高 $\to$ 商家亏。所以业内拼团商品都是\textbf{低毛利长尾 SKU}，散团代价低。
\end{itemize}
\srcnote{repository/cache/inventory.go:78--95 ReserveStock 返回 -1/-2 错误码；internal/groupbuy/service.go::MarkGroupSuccess 末尾的 CommitReservation}
\end{frame}
...
\begin{frame}[shrink]{C 端视角：发起 / 加入 / 看团 三个动作}
\begin{itemize}
  \item \textbf{发起}（已登录）：选品 $\to$ 设 N $\to$ 设价 $\to$ 立刻锁定 1 份库存。团长承担"凑不到人就退"的小成本。
  \item \textbf{加入}（已登录）：扫链接 $\to$ 看落地页 $\to$ 点"加入" $\to$ 立刻锁 1 份库存 + 锁价。
  \item \textbf{看团}（\emph{免登录}）：分享链接公开可看，未登录用户可以看到"还差 2 人" + 倒计时 $\to$ 引导注册。
  \item 业务杠杆：免登录看团是裂变的关键 —— 团长把链接转 5 个群，每个群有 30 人看，转化率 0.5\% = 0.75 人加入，1 个团长能凑齐 3 人团的概率 ~80\%。
\end{itemize}
\srcnote{internal/groupbuy/routes.go:14 GET /groupbuy/:id 挂在 public 组；POST 都挂 authed 组}
\end{frame}

\begin{frame}[shrink]{商家视角：拼团对库存利用率的真实改变}
\begin{itemize}
  \item 普通销售：日销 100 件 / 库存 1000 件 / 周转 10 天 / 滞销 SKU 占库存 30\%。
  \item 拼团销售：日销 100 件正价 + 50 件拼团价（散团率 30\%）$\to$ 日均真销 100 + 35 = 135 件 / 周转 7.4 天。
...
\end{itemize}
\srcnote{internal/groupbuy/model.go:33 PriceCents 字段；与 internal/product/model.go 中 discount\_price 不交叉}
\end{frame}

\begin{frame}[shrink]{社交裂变 vs 库存利用率：两边怎么平衡}
...
\vspace{2mm}
\begin{itemize}
  \item N 越大 $\to$ 用户越爱裂变（拉到 5 个朋友 vs 拉到 2 个）；但散团率指数上升（拉不齐的概率高）。
  \item 折扣越深 $\to$ 用户越想拉 $\to$ 但商家毛利越低；行业经验：8 折是甜点，5 折就要怀疑商家清库存。
  \item gomall 把这两个参数都开放给\textbf{运营 / 商家自助}决定，不写死，code 只保证状态机一致。
\end{itemize}
\srcnote{internal/groupbuy/handler.go::GroupbuyCreateReq.TargetCount / PriceCents 由商家入参；不在 service 写死}
\end{frame}
...
\begin{frame}[shrink]{客服 SOP：散团那一刻用户冲过来怎么应对}
\begin{itemize}
...
\bottomrule
\end{tabular}
\srcnote{pkg/e/code.go:66--69 业务码定义；pkg/e/msg.go:58--61 客服话术}
\end{frame}

\begin{frame}[shrink]{客服话术映射 = 业务码的真正使命}
\begin{itemize}
...
\srcnote{pkg/e/code.go:66--69；pkg/e/msg.go:58--61；service/events/events.go::GroupbuyExpired 是钱包消费入口}
\end{frame}

\begin{frame}[fragile,shrink]{帧：错误码翻译路径（handler $\to$ service $\to$ DB）}
\begin{lstlisting}[language=Go,caption={internal/groupbuy/handler.go::groupbuyErrorResponse}]
...
\begin{itemize}
  ...
  \item 客服系统拉 code $\to$ 命中 \texttt{pkg/e/msg.go::MsgFlags[81001]} $\to$ 自动展开成"该团已满员，请发起新团或加入其他团"。
\end{itemize}
\srcnote{internal/groupbuy/handler.go::groupbuyErrorResponse；internal/groupbuy/service.go::Err* sentinel errors}
\end{frame}
...
\begin{frame}[shrink]{黑产视角：拼团有哪些薅羊毛玩法}
\begin{itemize}
  \item 玩法 1：\textbf{脚本党刷团}。批量注册账号 $\to$ 都加入同一个团 $\to$ 拼团价拿货 $\to$ 货到立刻转卖。
...
\begin{frame}[shrink]{脚本党刷团：分布式滑动窗口 + IP 黑名单}
\begin{itemize}
  \item gomall 当前：\textbf{TokenBucket} 全局 100 RPS / 200 burst 挡基础脚本（routes/router.go:41）。
...
\end{itemize}
\srcnote{middleware/ratelimit.go::TokenBucket / SlidingWindow；routes/router.go:41 全局令牌桶}
\end{frame}

\begin{frame}[shrink]{同账号多团 / 虚假退团：DB 层 uniqueIndex + 状态机兜底}
\begin{itemize}
...
\end{itemize}
\srcnote{internal/groupbuy/model.go:46--47 uniqueIndex；internal/groupbuy/handler.go 只暴露 create / join / show 三个动作，无 cancel}
\end{frame}

\begin{frame}[fragile,shrink]{帧：JoinGroupAtomic 是怎么挡满团 + 超时 + 重复加入的}
\begin{lstlisting}[language=Go,caption={单 SQL 三防 internal/groupbuy/repo.go}]
res := d.DB.Model(&GroupbuyGroup{}).
    Where("id=? AND status=? AND current_count<target_count AND expire_at>?",
        groupID, GroupbuyStatusOpen, time.Now()).
    UpdateColumn("current_count", gorm.Expr("current_count + 1"))
...
\end{itemize}
\srcnote{internal/groupbuy/repo.go:55--73 JoinGroupAtomic；internal/groupbuy/model.go:46--47 uniqueIndex 定义}
\end{frame}
...
\begin{frame}[shrink]{SRE 关注：散团链路出问题怎么发现}
\begin{itemize}
  \item 散团是 Saga 链路：\textbf{DB tx 推状态} + \textbf{Redis 释放库存} + \textbf{outbox 写事件} + \textbf{下游钱包消费退款}。
  \item 任一步失败的影响：
  \begin{itemize}
    \item DB tx 失败 $\to$ 团仍 open，cron 下次扫到重试，自动恢复。
    \item Redis 释放失败 $\to$ 库存泄漏（reserved 桶有但 group 已 expired），需要对账 cron 兜底。
    \item outbox 写入在 tx 内 = 不会出现"散了但下游没消息"。
    \item 下游消费失败 $\to$ outbox publisher 指数退避重试，dead 后人工捞。
  \end{itemize}
\end{itemize}
\srcnote{internal/groupbuy/service.go::ExpireGroup 末尾的 ReleaseReservation 在 tx 外；internal/shared/outbox/publisher.go 重试机制}
\end{frame}

\begin{frame}[shrink]{SRE 监控指标清单}
\footnotesize
\begin{tabular}{@{}p{0.32\linewidth}p{0.22\linewidth}p{0.40\linewidth}@{}}
\toprule
指标 & 期望值 & 异常含义 \\
\midrule
...
\end{itemize}
\srcnote{internal/groupbuy/service.go 整套散团链路；与 internal/shared/outbox/publisher.go 共用一套指标}
\end{frame}

\begin{frame}[fragile,shrink]{帧：Outbox 在拼团里的 4 个事件路由}
...
\end{itemize}
\srcnote{service/events/events.go::GroupbuyCreated/Joined/Success/Expired；internal/groupbuy/service.go 4 处 outbox Insert}
\end{frame}

\begin{frame}[fragile,shrink]{帧：cron 散团兜底（已落地）}
\begin{lstlisting}[language=Go,caption={initialize/cron.go 散团兜底 @every 5m}]
...
func runGroupbuyExpireSweep() {
    ...
    ids, _ := groupbuy.NewGroupbuyDao(ctx).ExpireOpenGroupsBefore(time.Now(), 200)
...
\end{lstlisting}
\begin{itemize}
  \item \textcolor{cOK}{\textbf{[已落地]}} 与 \texttt{RunOrderTimeoutCheck} / \texttt{RunAutoConfirmReceive} 同模式，cron + 幂等 SQL 双兜底。
  \item \textbf{绕开 60×/min 陷阱}：用 \texttt{@every 5m} 而不是 \texttt{0 */5 * * * *} 这类 6 段表达式 —— 项目内已知 \texttt{* */5 * * * *} 误写成"每分钟跑 12 次"的陷阱（README 技术亮点 \#4），新 job 一律用 \texttt{@every} 写法绕开。
  \item 单团失败仅 ERROR log，不中断后续 —— 留给下一 tick 补，这是协同式 Saga 心法。
\end{itemize}
\srcnote{initialize/cron.go:64--74 @every 5m 注册；initialize/cron.go:93--112 runGroupbuyExpireSweep；internal/groupbuy/repo.go::ExpireOpenGroupsBefore}
\end{frame}
...
\begin{frame}[shrink]{文件清单：拼团域横切多少层}
\begin{center}
\footnotesize
...
test          & internal/groupbuy/groupbuy\_test.go \\
\bottomrule
\end{tabular}
\end{center}
\vspace{1mm}

{\footnotesize 横切 11 个层 = 完整业务域，与红包 / 优惠券 / 秒杀同等地位。}
\srcnote{本帧每一行都是真实文件路径，可在仓库 grep 验证}
\end{frame}

\begin{frame}[shrink]{测试覆盖：什么单测可以跑，什么留给集成}
\begin{itemize}
  \item \textbf{单测覆盖（internal/groupbuy/groupbuy\_test.go）}：
  \begin{itemize}
  ...
  \begin{itemize}
    \item 团长发起 $\to$ 2 个成员加入 $\to$ 凑齐 3 人 $\to$ 订单转 WaitShip
    \item 24h 超时散团 $\to$ 所有订单 Closed + 库存归还
    \item 满团再加返 81001 / 重复 join 返 81003
    \item 并发 50VU 同时 join 一个 N=3 团 $\to$ 只放 3 个进、其余 81001
  \end{itemize}
\end{itemize}
\srcnote{internal/groupbuy/groupbuy\_test.go 单测；docs/architecture/feature-matrix.md 路线图把集成跑通}
\end{frame}

\begin{frame}[fragile,shrink]{帧：状态机自检脚本（防回退保护）}
\begin{lstlisting}[language=Go,caption={internal/groupbuy/groupbuy\_test.go 状态机检查}]
...
\begin{itemize}
  ...
  \item 9 条原有 \texttt{TestOrderState\_*} 测试同时跑通 $\to$ 拼团接入不破坏老链路。
\end{itemize}
\srcnote{internal/groupbuy/groupbuy\_test.go::TestGroupbuy\_StateMachine\_*；internal/order/order\_state\_test.go 原 9 条}
\end{frame}

\begin{frame}[shrink]{本期落地：cron 兜底注册 + 规避 60×/min 陷阱}
\begin{itemize}
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{散团 cron 注册}：\texttt{initialize/cron.go} 加 \texttt{@every 5m} job，调 \texttt{ExpireOpenGroupsBefore(now, 200)} 拉超时团 id，逐个 \texttt{ExpireGroup}。单团失败仅 ERROR log，不中断后续。
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{规避 60×/min 表达式陷阱}：项目内已知 \texttt{* */5 * * * *} 误读成"每分钟跑 12 次"（README 技术亮点 \#4 / deck 09 复盘），新 job 一律用 \texttt{@every 5m} 写法，物理上不可能写错为 60×。
  \item \textcolor{cOK}{\textbf{[已落地]}} \textbf{协同式 Saga 心法}：\texttt{runGroupbuyExpireSweep} 单团失败 continue，下一 tick 自动重试；\texttt{ExpireGroup} 内部用条件 UPDATE（\texttt{WHERE status=Open}）做幂等兜底。
  \item 仍是路线图：散团 SLA \textbf{24h ± 5min} 端到端线上验证 —— cron 已落地，但真实流量下的窗口口径需观测后确认。
\end{itemize}
\srcnote{initialize/cron.go:64--74 注册；initialize/cron.go:93--112 sweep；internal/groupbuy/repo.go::ExpireOpenGroupsBefore}
\end{frame}

\begin{frame}[shrink]{业务边界与路线图}
\begin{itemize}
  \item \textbf{本期完成}：状态机 / DAO / service / handler / events / 单测 + \textcolor{cOK}{\textbf{cron 散团兜底 @every 5m}}，core 闭环就绪。
  \item \textbf{路线图（按优先级）}：
  \begin{enumerate}
    ...
    \item 散团 SLA \textbf{24h ± 5min} 端到端线上验证（cron 已落地，待真实流量验证窗口口径）
  \end{enumerate}
  \item 不做（明确边界）：多 SKU 组合团 / 砍价 / 团购返利 / 老带新奖励。
\end{itemize}
\srcnote{initialize/cron.go:64--74 @every 5m 已注册；docs/architecture/feature-matrix.md 未做清单更新位置}
\end{frame}
...
\hspace{4mm}\srcnote{internal/groupbuy/model.go:33 PriceCents；internal/product/model.go 主表 discount\_price 不变}

\vspace{1mm}
\textbf{Q2}：N 人团里第 N 人加入失败，前 N-1 人怎么办？\\
\hspace{4mm}\small A：前 N-1 人仍 WaitGroup，cron 散团 SLA 24h 兜底。第 N 人前端 81001 重试或换团。\\
\hspace{4mm}\srcnote{internal/groupbuy/service.go::JoinGroup 末尾 isSuccess=false 分支不影响其他成员}

\vspace{1mm}
\textbf{Q3}：拼团商品支付环节走法币 vs Web3 有区别吗？\\
\hspace{4mm}\small A：本期拼团默认\textbf{加入即扣}（与法币 paydown 不同步）。Web3 路径未对齐，路线图。\\
\hspace{4mm}\srcnote{internal/groupbuy/service.go::JoinGroup 在 tx 内立刻 reserveStock，不依赖 paydown 流程}
...
\hspace{4mm}\srcnote{internal/groupbuy/service.go::MarkGroupSuccess tx 内 for-range members 推 UPDATE}

\vspace{1mm}
\textbf{Q5}：用户 join 团但还没付款？\\
\hspace{4mm}\small A：拼团的设计是\textbf{加入即锁价 + 锁库存}，不留"WaitPay"过渡。这是与普通下单最大的差异。\\
\hspace{4mm}\srcnote{internal/groupbuy/service.go::JoinGroup leaderOrder.Type=OrderWaitGroup，不是 OrderWaitPay}
...
\begin{frame}[shrink]{Q\&A（下）}
\small
\textbf{Q7}：MarkGroupSuccess 失败了怎么办？团一直挂着？\\
\hspace{4mm}\small A：cron \texttt{@every 5m} 已落地，扫超时仍 Open 的团逐个 \texttt{ExpireGroup}（散团兜底）；成团方向的 \texttt{MarkGroupSuccessIfFull} 反向兜底留待 P1 扩展。当前 join 内 mark 失败仅日志。\\
\hspace{4mm}\srcnote{initialize/cron.go:64 GroupbuyExpire @every 5m；internal/groupbuy/service.go::JoinGroup 末尾 mark 失败只记录日志}
...
\hspace{4mm}\srcnote{internal/shared/outbox/publisher.go；internal/groupbuy/service.go 4 处 Insert outbox}
\end{frame}

\begin{frame}[shrink]{代码位置一览}
\begin{description}
  \item[订单状态新增 WaitGroup=8]      \srcnote{consts/order.go:15}
  \item[状态机 WaitGroup 出边]         \srcnote{internal/order/state.go:27}
  \item[业务码 81001-81004]            \srcnote{pkg/e/code.go:66--69}
  \item[客服话术]                      \srcnote{pkg/e/msg.go:58--61}
  \item[Model GroupbuyGroup / Member] \srcnote{internal/groupbuy/model.go}
  \item[migrate 注册]                  \srcnote{internal/migrate/migrate.go:41}
  \item[DAO 5 方法]                    \srcnote{internal/groupbuy/repo.go}
  \item[JoinGroupAtomic 单 SQL]        \srcnote{internal/groupbuy/repo.go::JoinGroupAtomic}
  \item[Service CreateGroup]           \srcnote{internal/groupbuy/service.go::CreateGroup}
  \item[Service JoinGroup]             \srcnote{internal/groupbuy/service.go::JoinGroup}
  \item[Service MarkGroupSuccess]      \srcnote{internal/groupbuy/service.go::MarkGroupSuccess}
  \item[Service ExpireGroup (Saga)]    \srcnote{internal/groupbuy/service.go::ExpireGroup}
  \item[事件 4 件套]                   \srcnote{service/events/events.go::Groupbuy*}
  \item[API handler 3 个]              \srcnote{internal/groupbuy/handler.go}
  \item[Route 注册]                    \srcnote{internal/groupbuy/routes.go}
  \item[单测]                          \srcnote{internal/groupbuy/groupbuy\_test.go}
  \item[库存复用 Lua]                  \srcnote{repository/cache/inventory.go::Reserve/Commit/Release}
  \item[Outbox Insert 模式]            \srcnote{internal/shared/outbox/repo.go::Insert}
\end{description}
\end{frame}
