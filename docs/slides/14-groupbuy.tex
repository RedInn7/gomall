...
\end{tcolorbox}
\end{center}
\srcnote{consts/order.go:15 OrderWaitGroup 注释；internal/order/state.go:20-28 转换表新增的两条出边}
\end{frame}
...
\end{lstlisting}
\srcnote{internal/groupbuy/service.go::CreateGroup 100-160；与 internal/order/service.go::OrderCreate 同一模式}
\end{frame}
...
\end{lstlisting}
\srcnote{internal/groupbuy/repo.go::JoinGroupAtomic 62-79；uniqueIndex 见 internal/groupbuy/model.go:43}
\end{frame}
...
\vspace{2mm}
\begin{itemize}
  \item 成团触发在 \textbf{第 N 个人 join 后}，调 \texttt{MarkGroupSuccess} —— 失败仅 ERROR log，cron \texttt{@every 5m} 兜底（已落地，\texttt{initialize/cron.go:62}）。
...
\end{itemize}
\srcnote{repository/cache/inventory.go:30-66 三段 Lua；internal/groupbuy/service.go::CreateGroup / JoinGroup / ExpireGroup 复用入口}
\end{frame}
...
\end{itemize}
\srcnote{repository/cache/inventory.go:79-95 ReserveStock 返回 -1/-2 错误码；internal/groupbuy/service.go::MarkGroupSuccess 末尾的 CommitReservation}
\end{frame}
...
\end{itemize}
\srcnote{internal/groupbuy/model.go:43 uniqueIndex；internal/groupbuy/handler.go 只暴露 create / join / show 三个动作，无 cancel}
\end{frame}
...
\end{itemize}
\srcnote{internal/groupbuy/repo.go::JoinGroupAtomic 62-79；internal/groupbuy/model.go:43 uniqueIndex 定义}
\end{frame}
...
\end{itemize}
\srcnote{initialize/cron.go:62--72 @every 5m 注册；initialize/cron.go:93--110 runGroupbuyExpireSweep；internal/groupbuy/repo.go::ExpireOpenGroupsBefore}
\end{frame}
...
\end{itemize}
\srcnote{initialize/cron.go:62--72 注册；initialize/cron.go:93--110 sweep；internal/groupbuy/repo.go::ExpireOpenGroupsBefore}
\end{frame}
...
\end{itemize}
\srcnote{initialize/cron.go:62--72 @every 5m 已注册；docs/architecture/feature-matrix.md 未做清单更新位置}
\end{frame}
...
\hspace{4mm}\srcnote{initialize/cron.go:62 GroupbuyExpire @every 5m；internal/groupbuy/service.go::JoinGroup 末尾 mark 失败只记录日志}
