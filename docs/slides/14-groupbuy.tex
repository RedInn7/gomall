...
\begin{description}
  \item[订单状态新增 WaitGroup=8]
  \item[状态机 WaitGroup 出边]
  \item[业务码 81001-81004]
  \item[客服话术]
  \item[Model GroupbuyGroup / Member]
  \item[migrate 注册]
  \item[DAO 5 方法]
  \item[JoinGroupAtomic 单 SQL]
  \item[Service CreateGroup]
  \item[Service JoinGroup]
  \item[Service MarkGroupSuccess]
  \item[Service ExpireGroup (Saga)]
  \item[事件 4 件套]
  \item[API handler 3 个]
  \item[Route 注册]
  \item[单测]
  \item[库存复用 Lua]
  \item[Outbox Insert 模式]
\end{description}
\end{frame}
