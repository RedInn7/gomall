...
\end{tabular}
\vspace{2mm}
\small SLO 是\textbf{业务和技术之间的契约}：超了赔运营预算，没超就接受延迟。商品列表 24.5 RPS / p95 2.5s \textbf{不算事故}（P3 上限是 1s 但当前 QPS 远低于阈值），但已经进入"应该排期修"的状态。
...
\srcnote{service/product.go:42--68 / repository/cache/product.go:55--58 TryProductLock}
\end{frame}
...
\end{tabular}
\vspace{2mm}
\small \textbf{为什么不接受 10 分钟旧价}：限时秒杀按分钟计价值；用户看到老价 = 漏发优惠 = 客诉 + 平台担责。\\
\textbf{为什么 500ms 可接受}：和产品 / 运营当面对齐："改价后 1 秒内可能仍显示老价，刷新即可"——写进客服话术。\\
\textbf{为什么不做强一致}：商家批量改 1000 个 SKU，每个卡 200ms = 后台转 3 分钟，完全不能用。
\srcnote{service/product.go:267--292 ProductUpdate + repository/cache/product.go:64--74 DoubleDeleteAsync}
\end{frame}
...
\srcnote{对应代码：service/product.go:50--58 SETNX 单飞 (击穿) / 规划 bloom.go (穿透) / 规划 jitter (雪崩)}
\end{frame}
...
\end{enumerate}
\srcnote{stressTest/REPORT.md \S 3 商品列表反模式 + repository/db/dao/product.go:58--64}
\end{frame}
...
\small
\begin{tabular}{@{}p{34mm}p{22mm}p{52mm}@{}}
\toprule
客诉描述 & 责任方 & 客服回复 + 升级路径 \\
\midrule
...
\small
\begin{tabular}{@{}p{16mm}p{60mm}p{32mm}@{}}
\toprule
角色 & 关键指标 & 报警阈值 \\
\midrule
...
\end{tabular}
\vspace{2mm}
\small \textbf{要点}：同一个 cache 命中率，SRE 看的是"我系统健不健康"，运营看的是"我商家上架速度被压了没"，是\textbf{同一份原始数据的不同业务视角}。Prometheus 一套指标支撑多个仪表盘。
...
\begin{description}
  ...
  \item[Outbox 联动 ES]\srcnote{service/product.go:257--265 emitProductChanged + :155/253/289 三处调用点}
