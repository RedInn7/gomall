...
\title{商品展示的业务依据}
\subtitle{转化漏斗最宽一跳、首屏 1 秒、三层缓存与深分页 COUNT 取舍}
...
\begin{frame}[shrink]{商品展示是什么角色的痛点}
...
\begin{frame}[shrink]{用户旅程视图：详情页是漏斗的腰}
...
\end{itemize}
\srcnote{internal/product/routes.go:15--17、internal/carousel/routes.go:13、internal/category/routes.go:13 公开 GET 挂 HTTPCache}
\end{frame}

\begin{frame}[shrink]{业务侧 SLO：详情页背的指标}
\footnotesize
\begin{tabular}{@{}lllr@{}}
\toprule
指标 & 业务等级 & 目标 & 实测 \\
...
列表/类目 p99 延迟     & P3 信息类 & \textless 1s    & 列表 2.5s (含全表 COUNT) \\
可用率                 & P1        & 99.9\%          & N/A \\
...
SLO 是\textbf{业务和技术之间的契约}：超了赔运营预算，没超就接受延迟。商品列表 24.5 RPS / p95 2.5s 在 P3 信息类口径下\textbf{未触发事故}（当前 QPS 远低于阈值），其延迟来自 \texttt{COUNT(*)} 全表计数这一已知特性；提速模板（游标分页）已在订单列表落地，可直接平移。
\srcnote{README.md \S 业务承诺(SLO) P1/P3 行 + stressTest/REPORT.md 链路压测表}
\end{frame}

\begin{frame}[shrink]{业务边界：这一节不解决什么}
\begin{itemize}
...
\begin{itemize}
  ...
  \item \texttt{product/list} 跑 24.5 RPS / p95 2.5s，全表 \texttt{COUNT(*)} 的代价从业务侧怎么解读、怎么平移到游标分页。
\end{itemize}
\srcnote{对应代码 internal/product/routes.go:15--17 / middleware/httpcache.go / repository/cache/product.go / internal/product/service.go}
\end{frame}

\begin{frame}[shrink]{首屏延迟与转化率：业务侧的硬性约束}
\begin{tabular}{@{}lll@{}}
\toprule
首屏延迟 & 跳出率上升 & 业内常见处理 \\
\midrule
...
\begin{frame}[shrink]{命中缓存 vs 回源 DB 的业务收益}
\begin{tabular}{@{}lrrr@{}}
\toprule
...
\texttt{/product/list} 的延迟全部来自 \texttt{COUNT(*)} 全表计数，正是下文要量化拆解的对象。
...
\begin{frame}[shrink]{业务场景：商家上架 1 万 SKU 后 DB 顶不住的剧本}
...
\srcnote{internal/product/service.go:41--81 / repository/cache/product.go:102--104 TryProductLock}
\end{frame}

\begin{frame}[shrink]{Cache Aside 读路径}
...
\end{itemize}
\srcnote{internal/product/service.go:41--81 ProductShow 五段式}
\end{frame}

\begin{frame}[fragile,shrink]{读路径关键代码}
\begin{lstlisting}[language=Go,firstnumber=43,
  caption={\srcnote{internal/product/service.go:43--80 ProductShow}}]
...
\begin{frame}[shrink]{读路径五步的业务含义}
\begin{description}
  \item[L1 命中] 99\% 的请求走这里。Redis $\sim$ 1ms，对业务侧等价于"没有数据库"。
  \item[L1' 空值] 命中 \texttt{\textbackslash x00null} 空值标记，确认商品不存在，\textbf{直接}按 not found 返回，挡住穿透——细节见"穿透"一节。
  \item[L2 SETNX] 缓存失效/被双删的瞬间，跨进程互斥只放一个请求回源，业务侧叫\textbf{缓存击穿}。
  \item[L3 单飞] \texttt{LoadProductOnce} 用进程内 \texttt{singleflight} 再合并同实例的并发回源，叠加在 Redis 锁之上防\textbf{惊群}——细节见"惊群"一节。
  \item[L3' 回写空值 / L4 回填] not-found 写短 TTL 空值标记；命中则回填，TTL 带随机抖动防\textbf{雪崩}。
\end{description}
\srcnote{repository/cache/product.go:16 ProductDetailTTL=10min / :64--71 GetProductDetail / :94--99 LoadProductOnce}
\end{frame}
...
\begin{frame}[shrink]{业务取舍：用户多久能看到改价}
\footnotesize
...
\srcnote{internal/product/service.go:284--315 ProductUpdate + repository/cache/product.go:110--120 DoubleDeleteAsync}
\end{frame}

\begin{frame}[shrink]{为什么不能"写库 + 写缓存"}
\begin{itemize}
...
\begin{frame}[shrink]{延迟双删：第二刀挡哪个窗口}
...
\end{tikzpicture}
\srcnote{repository/cache/product.go:110--120 DoubleDeleteAsync (interval=500ms)}
\end{frame}

\begin{frame}[fragile,shrink]{写路径关键代码：删-写-延迟删}
\begin{lstlisting}[language=Go,firstnumber=290,
  caption={\srcnote{internal/product/service.go:284--315 ProductUpdate}}]
...
\begin{frame}[shrink]{业务侧为什么接受"500ms 旧值"}
\begin{itemize}
...
\srcnote{internal/product/service.go:284--315; repository/cache/product.go:110--120}
\end{frame}
...
\begin{frame}[shrink]{为什么轮播图 24h 可以、价格 5 分钟都不行}
...
\srcnote{internal/product/routes.go:15--17 公开 GET 挂 cache / 下单写接口 internal/order/routes.go:12 走 authed 中间件链不挂 HTTPCache}
\end{frame}

\begin{frame}[shrink]{四层缓存漏斗}
...
\srcnote{internal/product/routes.go:15--16、internal/carousel/routes.go:13、internal/category/routes.go:13 挂载点}
\end{frame}

\begin{frame}[shrink]{每层命中率怎么算}
\begin{center}
\footnotesize
...
\begin{frame}[shrink]{命中率链式公式与业务含义}
...
\begin{frame}[shrink]{CDN 选型与命中率掉档的运维剧本}
\begin{tabular}{@{}llll@{}}
\toprule
方案 & 节点 / 成本 & 命中率 KPI & 适用 \\
\midrule
...
\begin{frame}[shrink]{ETag 304 流程}
\begin{tikzpicture}[node distance=10mm]
...
\begin{frame}[fragile,shrink]{ETag 中间件关键代码}
...
\begin{frame}[shrink]{ETag 中间件逐行讲解 + 为什么 POST 不挂}
\begin{itemize}
...
\end{itemize}
\srcnote{middleware/httpcache.go:47--100 + 方法判断 :54--58}
\end{frame}

\begin{frame}[shrink]{TTL 怎么定：业务节奏决定}
\begin{tabular}{@{}lrll@{}}
\toprule
接口 & max-age & 业务理由 & 风险 \\
\midrule
...
\srcnote{internal/product/routes.go:15 (30s) / :16 (60s) / internal/category/routes.go:13 (300s) / internal/carousel/routes.go:13 (300s)}
\end{frame}

% ============================================================
\section{四态故障：击穿 / 穿透 / 雪崩 / 惊群}
% ============================================================

\begin{frame}[shrink]{四态故障的真实事故剧本}
\textbf{为什么这一节重要}：缓存四态故障是详情页能不能上 99.9\% 可用率的天花板。
\vspace{1mm}
\begin{description}
  ...
  \item[惊群事故] 即便 SETNX 挡住了跨进程并发，单个实例里仍有上千 goroutine 同时 miss、同时去抢锁、抢不到全 sleep 50ms 再抢；DB 连接被同一商品的重复回源占满。
\end{description}
\vspace{1mm}
\small 四个事故对客户都是\textbf{"为什么平时秒开的页面突然 5 秒打不开"}，对 SRE 是\textbf{四种不同的处置 SOP}。区分清楚是基本功。
\srcnote{internal/product/service.go:51 SETNX(击穿) / :70 空值(穿透) / repository/cache/product.go:79 抖动(雪崩) / :94 单飞(惊群)}
\end{frame}

\begin{frame}[shrink]{四种缓存故障的区别与防护}
\footnotesize
\begin{tabular}{@{}llll@{}}
\toprule
名称 & 触发条件 & 后果 & gomall 防护 \\
\midrule
\textbf{击穿} & 单个热 key 过期，瞬间 N 个请求回源 & 单点 DB 被打爆 & SETNX 互斥锁 \\
\textbf{穿透} & 查\textbf{不存在}的 key，cache 永远 miss & 每次都打 DB，CPU 100\% & 短 TTL 空值标记 \\
\textbf{雪崩} & 大量 key 同一时刻过期 & 整片回源，连接池打满 & TTL 随机抖动 \\
\textbf{惊群} & 同实例大量 goroutine 同时回源 & DB 连接被重复回源占满 & \texttt{singleflight} 合并 \\
\bottomrule
\end{tabular}
\vspace{2mm}
\small 四态共同点：cache miss 引起 DB 流量\textbf{阶跃}上涨。区别在\textbf{key 的范围}（单 key / 不存在的 key / 多 key）与\textbf{并发的层次}（跨进程 / 同进程）。
四种防护现在都\textbf{已落地在代码里}，不再是路线图。下面分别讲。
\srcnote{repository/cache/product.go:22 ProductTTLJitter / :29 productNullValue / :48 productLoadGroup / :103 SetNX}
\end{frame}

\begin{frame}[shrink]{击穿：SETNX 单飞回源}
\begin{tikzpicture}[node distance=10mm]
...
\end{tikzpicture}
\srcnote{internal/product/service.go:51--61 SETNX 抢锁 + repository/cache/product.go:102--108 TryProductLock / UnlockProduct}
\end{frame}

\begin{frame}[shrink]{穿透：短 TTL 空值标记挡住"不存在"的 id}
\begin{itemize}
  \item 攻击场景：脚本爬虫扫 \texttt{/product/show?id=99999999}，DB 返回 \texttt{record not found}。若不回填，下一次请求继续打 DB——穿透。
  \item \textbf{gomall 的修法（已落地）}：回源 not-found 时写一个\textbf{空值标记} \texttt{product:detail:\{id\} = "\textbackslash x00null"}，TTL 仅 \texttt{ProductNullTTL = 60s}。后续同 id 请求命中标记直接按 not found 短路，不再打 DB。
  \item \textbf{占位内容选 \texttt{\textbackslash x00null}}：以 \texttt{\textbackslash x00} 开头，和正常详情 JSON 的首字符 \texttt{\{} 天然区分，读缓存时一比即知是空值还是真实对象。
  \item \textbf{TTL 取 60s 而非 10min}：既挡住高频穿透，又保证商品后续\textbf{真上架}时最多 60s 自愈——空值标记不能长命。
  \item \textbf{为什么不用布隆过滤器}：BF 要维护全量合法 id、新建商品增量 \texttt{BF.ADD}、有 0.01\% 误判，运维成本高。空值标记零额外组件、零误判，对当前 QPS 量级是更划算的工程取舍。
\end{itemize}
\srcnote{repository/cache/product.go:29 productNullValue / :64--71 GetProductDetail 命中分支 / :83--85 SetProductNotFound}
\end{frame}

\begin{frame}[fragile,shrink]{空值缓存关键代码}
\begin{lstlisting}[language=Go,firstnumber=29,
  caption={\srcnote{repository/cache/product.go:29 + :64--85 空值标记三件套}}]
...
func GetProductDetail(ctx context.Context, id uint, dst interface{}) error {
    ...
    if err == redis.Nil { return ErrProductCacheMiss }
    if err != nil { return err }
    if raw == productNullValue { return ErrProductNotFound }
...
\begin{frame}[fragile,shrink]{雪崩：TTL 随机抖动打散过期时刻}
\begin{itemize}
  \item 触发场景：商家批量上架 1000 个 SKU，全部用默认 \texttt{ProductDetailTTL = 10min}。10 分钟后\textbf{同一秒}集中过期，回源瞬时打 1000 次 DB。
  \item \textbf{gomall 的修法（已落地）}：写缓存时不用固定 TTL，而是在 10min 上叠加 \texttt{[0, ProductTTLJitter)} 的随机偏移（\texttt{ProductTTLJitter = 90s}），把同批写入的过期时刻打散到 90s 窗口里。
  \item \textbf{兜底}：DB 拒连时整链路可降级返回上一版 cache（哪怕过期），客服话术接受"价格延迟 1 分钟"。
\end{itemize}
\begin{lstlisting}[language=Go,firstnumber=51,
  caption={\srcnote{repository/cache/product.go:51--56 + :74--80 写缓存带抖动}}]
func withProductTTLJitter(base time.Duration) time.Duration {
    if ProductTTLJitter <= 0 { return base }
...
\begin{frame}[shrink]{惊群：singleflight 合并同实例并发回源}
\begin{tikzpicture}[node distance=8mm and 14mm]
  \node[node-box,minimum width=14mm] (g1) {goroutine 1};
  \node[node-box,minimum width=14mm,below=4mm of g1] (g2) {goroutine 2};
  \node[node-box,minimum width=14mm,below=4mm of g2] (g3) {goroutine N};
  \node[node-box,node-warn,minimum width=24mm,right=22mm of g2] (sf) {\texttt{singleflight}\\Do(key)};
  \node[node-box,node-bad,minimum width=20mm,right=of sf] (db) {MySQL\\回源 1 次};
  \draw[arrow] (g1.east) -- (sf.north west);
  \draw[arrow] (g2.east) -- (sf.west);
  \draw[arrow] (g3.east) -- (sf.south west);
  \draw[arrow] (sf.east) -- node[above,font=\tiny]{只 1 个执行} (db.west);
  \draw[arrow-async,bend left=20] (db.north) to node[above,font=\tiny]{结果广播给 N 个等待者} (sf.north);
\end{tikzpicture}
\vspace{1mm}
\begin{itemize}
  \item SETNX 是\textbf{跨进程}互斥；singleflight 是\textbf{同进程}合并。两者叠加：100 个实例 $\times$ 1000 goroutine 抢同一商品，最终只有 1 次回源打到 DB。
  \item \texttt{LoadProductOnce(id, load)} 以 \texttt{product:detail:\{id\}} 为 key，\texttt{load} 只被其中一个 goroutine 实际执行，其余等待并\textbf{共享同一结果}（含 error），不会各自重复打库。
\end{itemize}
\srcnote{repository/cache/product.go:48 productLoadGroup / :94--99 LoadProductOnce / internal/product/service.go:64--66 调用点}
\end{frame}

\begin{frame}[fragile,shrink]{惊群防护关键代码}
\begin{lstlisting}[language=Go,firstnumber=92,
  caption={\srcnote{repository/cache/product.go:92--99 LoadProductOnce}}]
...
\end{lstlisting}
\vspace{1mm}
\small \texttt{Do} 的第三个返回值 \texttt{shared} 表示该结果是否被多个调用方共享；这里业务侧不关心是谁真正回源的，只关心\textbf{结果一致}，故丢弃。
\end{frame}

\begin{frame}[shrink]{第二刀间隔的取舍}
\begin{tabular}{@{}lll@{}}
\toprule
第二刀间隔 & 覆盖率 & 副作用 \\
\midrule
...
\srcnote{repository/cache/product.go:18 ProductDelayInterval = 500ms}
\end{frame}

\begin{frame}[shrink]{热 key：探测 + 三种拆分策略}
\small
...
% ============================================================
\section{深分页 COUNT(*) 的工程取舍}
% ============================================================

\begin{frame}[shrink]{业务侧视角：list 延迟从哪来、怎么平移到游标分页}
\begin{itemize}
  \item C 端用户：列表页是\textbf{发现商品}的主入口，比详情更上游；list 快慢直接影响用户进不进得了详情。
  \item 商家：上架新 SKU 后\textbf{30s 内可见}（HTTP TTL），深分页到第 5 页的可达性影响商家流量分配的公平性。
  \item 运营：类目页 "全部 1{,}245 件" 的 total 是\textbf{产品需求} —— 但用户从来不在乎是 1245 还是约 1200，精确 total 往往是过度需求。
  \item SRE：\texttt{product/list} 24.5 RPS / p95 2.5s 是\textbf{1M 行表上 \texttt{COUNT(*)} 全表计数的固有代价}，不是 bug；提速路径在于改产品需求或换实现，两者都有成熟模板。
\end{itemize}
\vspace{1mm}
\textbf{业务取舍排序}：
\begin{enumerate}
...
\begin{frame}[shrink]{对照数字：24.5 RPS / p95 2.5s 的来源}
\begin{tabular}{@{}lrr@{}}
\toprule
接口 & 实测 RPS & p95 \\
\midrule
...
\texttt{/product/list} (含 COUNT) & \textbf{24.5}    & \textbf{2.50s} \\
\bottomrule
\end{tabular}
\vspace{2mm}
...
\begin{frame}[shrink]{扫表示意：1M 行的 COUNT 怎么炸的}
\begin{tikzpicture}
...
\begin{frame}[fragile,shrink]{COUNT(*) 代码与其代价}
...
\end{lstlisting}
\vspace{2mm}
\textbf{service 调用点}：\srcnote{internal/product/service.go:210--215 ProductList 同时调 ListProductByCondition + CountProductByCondition}
\end{frame}

\begin{frame}[shrink]{三种修法对业务的代价}
\begin{tabular}{@{}lll@{}}
\toprule
方案 & 代价 & 业务可见变化 \\
\midrule
...
\begin{frame}[shrink]{物化视图 / 无限滚动：选哪个}
\begin{tabular}{@{}lllr@{}}
\toprule
方案 & 一致性 & 复杂度 & 查询耗时 \\
\midrule
...
\begin{frame}[shrink]{容量规划：1k / 10k / 100k QPS 链路资源估算}
\begin{tabular}{@{}lrrr@{}}
\toprule
...
\begin{frame}[shrink]{客服话术手册：商品展示侧 4 类典型客诉}
\begin{center}
\footnotesize
...
\begin{frame}[shrink]{各角色仪表盘：每人看什么}
\footnotesize
...
\begin{frame}[shrink]{业务码：60002 的真实含义}
\footnotesize
\begin{tabular}{@{}lll@{}}
\toprule
状态码 & 含义       & 客服话术 \\
...
\begin{frame}[shrink]{客诉拆解 + 业务侧最终目标}
\textbf{"为什么显示老价格"三种场景}：
\begin{description}
...
\srcnote{internal/product/service.go:300--312 双删 + emitProductChanged 三联}
\end{frame}

\begin{frame}[shrink]{商品展示路线图：从 P1 信息类到 P0 流量入口}
\begin{tabular}{@{}p{20mm}p{42mm}p{36mm}@{}}
\toprule
阶段 & 已做 & 路线图 \\
\midrule
M1 当前 & Cache Aside / SETNX / 双删 / HTTPCache / outbox 联动 ES / 四态防护（空值 + 抖动 + singleflight） & — \\
...
\begin{frame}[shrink]{面试 Q\&A（一）：业务取舍篇}
\small
...
\begin{frame}[shrink]{面试 Q\&A（二）：角色与边界篇}
\small
...
\vspace{1mm}
\textbf{Q8}：缓存击穿、穿透、雪崩、惊群四个名词怎么区分？gomall 各用什么挡？\\
A：击穿=单 key 过期瞬时并发回源（SETNX 跨进程互斥）—— 双 11 主推品 \texttt{DEL} 误操作，1 分钟丢 200 万 GMV；穿透=查不存在的 key 一直 miss（短 TTL 空值标记 \texttt{\textbackslash x00null}）—— 黑产扫 id 打 DB CPU 100\%；雪崩=多 key 同时过期（TTL 叠加 90s 随机抖动）—— 商家批量上架后 T+10min 集中过期；惊群=同实例大量 goroutine 同时回源（\texttt{singleflight} 合并）—— 叠加在 SETNX 之上把同进程重复回源也收成 1 次。四种防护都已落地，非路线图。
\end{frame}

\begin{frame}[shrink]{面试 Q\&A（三）：SLO 与监控篇}
\small
\textbf{Q9}：商品详情背的 SLO 是什么？什么时候算事故？\\
A：P1 信息类 99.9\% / p99 \textless 200ms，实测 p95 3ms 有 60 倍余量。事故触发：p99 \textgreater 200ms 持续 5min，或可用率跌破 99.9\%（年宕 8.76h 预算）。商品列表 24.5 RPS / p95 2.5s 当前 \textbf{不算事故}（P3 上限 1s 但 QPS 远低于阈值），延迟来自全表 \texttt{COUNT(*)}，QPS 增长前可平移游标分页模板提速。
...
\scriptsize
\begin{description}
  \item[读路径 Cache Aside]\srcnote{internal/product/service.go:41--81 ProductShow}
  \item[写路径 双删]\srcnote{internal/product/service.go:284--315 ProductUpdate}
  \item[Cache 包装]\srcnote{repository/cache/product.go:59--120 Get/Set/Del/DoubleDeleteAsync}
  \item[SETNX 回源锁]\srcnote{repository/cache/product.go:101--108 TryProductLock / UnlockProduct}
  \item[Double Delete 异步]\srcnote{repository/cache/product.go:110--120 DoubleDeleteAsync 500ms}
  \item[穿透：空值标记]\srcnote{repository/cache/product.go:29 productNullValue / :64--71 Get / :83--85 SetProductNotFound}
  \item[雪崩：TTL 抖动]\srcnote{repository/cache/product.go:22 ProductTTLJitter / :51--56 withProductTTLJitter / :74--80 SetProductDetail}
  \item[惊群：singleflight]\srcnote{repository/cache/product.go:48 productLoadGroup / :94--99 LoadProductOnce}
  ...
  \item[路由挂载]\srcnote{internal/product/routes.go:15--16 (list 30s / show 60s) + internal/carousel/routes.go:13 / internal/category/routes.go:13 (300s)}
  \item[全表 COUNT 取舍]\srcnote{internal/product/repo.go:58--64 CountProductByCondition}
  \item[热 key 探测 / 多级 cache]\srcnote{规划 middleware/hotkey.go / Redis cluster ro-replica L2}
  \item[业务码]\srcnote{pkg/e/code.go:53--58 (60001/60002/70001/70002)}
  \item[Outbox 联动 ES]\srcnote{internal/product/service.go:271--278 emitProductChanged + :169/266/312 三处调用点}
