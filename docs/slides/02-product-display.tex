...
列表/类目 p99 延迟     & P3 信息类 & \textless 1s    & 优化前 2.5s $\to$ 总数已上 60s 缓存 \\
可用率                 & P1        & 99.9\%          & N/A \\
...
SLO 是\textbf{业务和技术之间的契约}：超了赔运营预算，没超就接受延迟。商品列表优化前 24.5 RPS / p95 2.5s 未触发事故（P3 口径、QPS 低于阈值），延迟来自 \texttt{COUNT(*)} 全表计数；第 0 刀已落地——\textbf{总数 60s 缓存 + singleflight}，每类目每分钟至多数一遍，回源只剩分页 SELECT；游标分页模板（订单列表已落地）是下一步。
\end{frame}
...
\begin{itemize}
  ...
  \item \texttt{product/list} 优化前 24.5 RPS / p95 2.5s，全表 \texttt{COUNT(*)} 的代价从业务侧怎么解读、第 0 刀（60s 计数缓存）怎么砍。
\end{itemize}
\end{frame}
...
\texttt{/product/list} 优化前的延迟全部来自 \texttt{COUNT(*)} 全表计数，正是下文要量化拆解的对象（第 0 刀 60s 计数缓存已摊薄）。
\end{frame}
...
\begin{itemize}
  ...
  \item SRE：\texttt{product/list} 优化前 24.5 RPS / p95 2.5s 是\textbf{1M 行表上 \texttt{COUNT(*)} 全表计数的固有代价}，不是 bug——但"每翻一页数一遍"可以摊薄。
\end{itemize}
\vspace{1mm}
\textbf{第 0 刀（已落地）}：总数 60s 缓存 + singleflight——不改前端、不改语义，COUNT 摊到每类目每分钟 $\le$1 次；唯一不需要跨团队对齐的选项。\\
\textbf{业务取舍排序（下一步）}：
\begin{enumerate}
...
\texttt{/product/list} (含 COUNT，优化前) & \textbf{24.5}    & \textbf{2.50s} \\
\bottomrule
\end{tabular}
\vspace{2mm}
\small 同样是商品域的 GET 接口，吞吐差 \textbf{2{,}500 倍}，p95 差 \textbf{800 倍}。\\
区别只有一个：\texttt{ShowProduct} 走主键，\texttt{ListProducts} 多了一个 \texttt{SELECT COUNT(*)}。计数缓存上线后 COUNT 每类目每 60s 至多一次，miss 之外回源只剩分页 SELECT（新口径未复测，保留优化前数字作对照）。
\end{frame}
...
\end{lstlisting}
\vspace{2mm}
\textbf{service 调用点（第 0 刀落地后）}：
\begin{lstlisting}[language=Go]
// internal/product/service.go —— 总数走 60s 缓存 + singleflight
total, err = cache.ProductCountCached(ctx, req.CategoryID, func() (int64, error) {
    return productDao.CountProductByCondition(condition) // 每类目每分钟至多真数一遍
})
\end{lstlisting}
\end{frame}
...
方案 & 代价 & 业务可见变化 \\
\midrule
缓存 COUNT 60s（\textbf{已落地}） & total 滞后 $\le$60s & 无——与上架 SLA 同口径 \\
...
\end{tabular}
\vspace{2mm}
\small 物化视图临界点：\textbf{QPS \textgreater 1k \&\& 类目稳定}。gomall 优化前实测 24.5 RPS，远没到——第 0 刀计数缓存 + 建索引就够。\\
...
A：P1 信息类 99.9\% / p99 \textless 200ms，实测 p95 3ms 有 60 倍余量。事故触发：p99 \textgreater 200ms 持续 5min，或可用率跌破 99.9\%（年宕 8.76h 预算）。商品列表优化前 24.5 RPS / p95 2.5s \textbf{不算事故}（P3 上限 1s 但 QPS 远低于阈值）；现总数已上 60s 缓存 + singleflight，回源只剩分页 SELECT，深分页再平移游标分页模板。
...
A：第 0 刀 60s 计数缓存已落地（后端独立决定、零对齐成本）；QPS \textless 1k 加索引（后端独立决定）；要"约 1000 件"文案用近似计数（要拉产品对文案）；类目稳定且 QPS \textgreater 1k 上物化视图（架构师拍板）；无限滚动 = 最优解但要前端 PM + 产品三方对齐。\textbf{业务取舍优先级 \textgreater 技术选型}。
...
\scriptsize
\begin{description}
  \item[读路径 Cache Aside]
  \item[写路径 双删]
  \item[Cache 包装]
  \item[SETNX 回源锁]
  \item[Double Delete 异步]
  \item[穿透：空值标记]
  \item[雪崩：TTL 抖动]
  \item[惊群：singleflight]
  \item[HTTP cache 中间件]
  \item[ETag 304 比对]
  \item[路由挂载]
  \item[全表 COUNT 取舍]
  \item[热 key 探测 / 多级 cache]
  \item[业务码]
  \item[Outbox 联动 ES]
  \item[业务全景 / SLO]
  \item[压测数字]
