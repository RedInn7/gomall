...
\srcnote{internal/product/service.go:40--66 / repository/cache/product.go:55--58 TryProductLock}
\end{frame}
...
\end{itemize}
\srcnote{internal/product/service.go:40--66 ProductShow 三段式}
\end{frame}

\begin{frame}[fragile]{读路径关键代码}
\begin{lstlisting}[language=Go,firstnumber=41,
  caption={\srcnote{internal/product/service.go:41--65 ProductShow}}]
cached := &ProductResp{}
...
\srcnote{internal/product/service.go:269--290 ProductUpdate + repository/cache/product.go:64--74 DoubleDeleteAsync}
\end{frame}
...
\begin{lstlisting}[language=Go,firstnumber=270,
  caption={\srcnote{internal/product/service.go:269--290 ProductUpdate}}]
...
\srcnote{internal/product/service.go:269--290; repository/cache/product.go:64--74}
\end{frame}
...
\srcnote{对应代码：internal/product/service.go:48--56 SETNX 单飞 (击穿) / 规划 bloom.go (穿透) / 规划 jitter (雪崩)}
\end{frame}
...
\srcnote{对应 internal/product/service.go:48--56 SETNX + 后续准备引入的布隆过滤器和 TTL jitter}
\end{frame}
...
\end{tikzpicture}
\srcnote{internal/product/service.go:48--56 + repository/cache/product.go:56--62}
\end{frame}
...
\end{lstlisting}
\vspace{2mm}
\textbf{service 调用点}：\srcnote{internal/product/service.go:193--198 ProductList 同时调 ListProductByCondition + CountProductByCondition}
\end{frame}
...
\srcnote{internal/product/service.go:280--287 双删 + emitProductChanged 三联}
\end{frame}
...
\scriptsize
\begin{description}
  \item[读路径 Cache Aside]\srcnote{internal/product/service.go:40--66 ProductShow}
  \item[写路径 双删]\srcnote{internal/product/service.go:269--290 ProductUpdate}
  ...
  \item[Outbox 联动 ES]\srcnote{internal/product/service.go:256--263 emitProductChanged + :153/251/287 三处调用点}
