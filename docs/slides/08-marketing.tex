...
\end{itemize}
\srcnote{coupon.CouponBatch.Type/Threshold/Amount 字段；internal/coupon/service.go:30--57 CreateBatch}
\end{frame}
...
\end{itemize}
\srcnote{internal/skill/service.go:122--134 SkillProduct；商品独立 skill.SkillProduct 表}
\end{frame}
...
\srcnote{internal/skill/service.go:122--134；routes/router.go:142--152 秒杀路由}
\end{frame}
...
\end{itemize}
\srcnote{internal/redpacket/service.go:42--103 Create + Claim 全流程}
\end{frame}
...
\hspace{4mm}\small \textbf{代码}：\texttt{internal/coupon/service.go:65--99 Claim}（gomall 暂未实装 ROI 仪表盘）。
...
\begin{description}\small
  ...
  \item[coupon 业务层 Claim]\srcnote{internal/coupon/service.go:65--99}
  \item[coupon 库存预热（创建批次）]\srcnote{internal/coupon/service.go:30--57}
  ...
  \item[秒杀业务]\srcnote{internal/skill/service.go:122--134}
...
\begin{description}\small
  ...
  \item[拼手气红包模型（gomall 已实现）]\srcnote{internal/redpacket/service.go:42--103 Create + Claim}
