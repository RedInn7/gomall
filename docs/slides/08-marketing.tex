...
\begin{frame}[fragile]{coupon 回滚必须原子：两条独立命令留下的资损口子}
...
\begin{frame}[fragile]{修法：单条 Lua 原子归还 + 返回 error 供补偿}
...
\begin{frame}[fragile]{秒杀详情/下单读了一个从没写过的 key：接口必然失败}
...
\begin{frame}[fragile]{修法：预热补写单品 key + 读侧未命中回源回填}
