...
\begin{description}\small
  \item[coupon Lua（原子扣减 + 单人限领）]
  \item[coupon 错误码翻译]
  \item[coupon 回滚 Lua（原子归还 + 返回 error）]
  \item[coupon 业务层 Claim]
  \item[coupon 库存预热（创建批次）]
  \item[coupon DB 锁路径（clause.Locking + 随机券码）]
  \item[coupon 路由（无 SlidingWindow，业务取舍）]
  \item[红包二倍均值法拆分]
  \item[红包预拆 RPUSH Lua]
  \item[红包抢取 Lua（HEXISTS + LPOP + HSET）]
  \item[红包 Saga 回滚 Lua]
  \item[红包 Service 调用 Saga 点]
  \item[红包过期回收]
  \item[红包路由（SlidingWindow + Idempotency）]
  \item[秒杀业务（详情/下单读单品 key + 回源回填）]
  \item[秒杀路由（SlidingWindow 3/s/用户）]
\end{description}
\end{frame}

\begin{frame}[shrink]{代码位置一览（二）：限流、业务码、压测}
\begin{description}\small
  \item[SlidingWindow Lua（ZSet 滑窗）]
  \item[全局 TokenBucket（100 RPS / IP）]
  \item[业务码 70001 / 70002 / 60002 / 50001]
  \item[Go error（红包 / 优惠券）]
  \item[压测：秒杀 + 滑动窗口（46/45 误差 2.2\%）]
  \item[压测：优惠券 Redis Lua vs DB（51K / 50K RPS）]
  \item[拼手气红包模型（gomall 已实现）]
  \item[业务边界（未做项）]
\end{description}
\vspace{2mm}
