...
\begin{frame}[shrink]{为什么营销是一个独立 deck：业务视角的三条线}
\begin{itemize}
  \item gomall 主链路是\textbf{下单 $\to$ 支付 $\to$ 履约}，营销是\textbf{并行链路}：在用户开始下单决策\textbf{之前}动手。
  \item 三条业务线，目的截然不同：
  \begin{itemize}
...
\end{itemize}
\srcnote{internal/coupon/service.go, internal/redpacket/service.go, internal/skill/service.go 三个 service 入口}
\end{frame}

\begin{frame}[shrink]{营销活动的 5 角色视图}
\scriptsize
\begin{tabular}{@{}lp{0.36\linewidth}p{0.50\linewidth}@{}}
\toprule
角色 & 营销活动对他意味着什么 & 他最怕看到什么 \\
\midrule
C 端用户   & "抢券、秒杀、抽红包"的兴奋时刻 & 抢券失败 / 红包金额很少 / 秒杀页面卡死 \\
商家       & 我的活动能引流多少？带来多少新客？ & 平台羊毛党把券都领走，真实用户没拿到 \\
运营       & 预算$\to$GMV 的 ROI 算不算得过来 & 100 万发券，只换来 5 万 GMV 增量（ROI < 1） \\
客服       & 大促当天投诉量翻 5 倍，话术够不够用 & 用户问"为什么我抢不到红包"，没有标准回复 \\
SRE        & 流量峰值是平常 50×，集群扛得住吗 & 0 点限流误伤真用户 / Redis Lua 超时 \\
\bottomrule
\end{tabular}
\smallskip
\keypoint{\footnotesize \textbf{核心矛盾}：同一个活动，5 个角色的成功定义完全不同 —— 运营要"参与率高"，SRE 要"流量平稳"，客服要"话术清晰"，商家要"真客户拉到"，C 端用户要"抢得到 + 公平"。本 deck 的每一帧都会回答："这个设计满足了谁，牺牲了谁。"}
\srcnote{README 业务全景 5 角色表 + SLO P2 营销秒杀等级}
\end{frame}

\begin{frame}[shrink]{业务承诺 SLO：营销是 P2，但峰值远超平日}
\begin{tabular}{@{}llll@{}}
\toprule
活动 & 业务等级 & 可用率 & p99 延迟 \\
\midrule
...
\begin{frame}[shrink]{业务边界：这份 deck 覆盖什么、不覆盖什么}
\begin{tikzpicture}[node distance=4mm and 6mm]
  ...
  \node[node-box,right=of out2,node-ok] (out3) {满减引擎 (deck 13)};
  \node[node-box,below=of out1,node-bad] (out4) {会员等级};
  \node[node-box,right=of out4,node-bad] (out5) {积分体系};
  \node[node-box,right=of out5,node-ok] (out6) {拼团 (deck 14)};
\end{tikzpicture}
\vspace{2mm}
\small \textbf{做了的}（绿）：三个核心 Lua 原子机制 + Saga 兜底 + 限流防黑产 + 客服话术映射；\textbf{满减引擎}与\textbf{拼团}已另行落地，各有专属 deck（13 / 14），本 deck 不展开。\\
\textbf{没做的}（红）：\textbf{AB 测试} / \textbf{用户分群} / \textbf{商家自助活动配置} / \textbf{积分} / \textbf{会员等级} / \textbf{直播带货}。\\
...
\begin{frame}[shrink]{优惠券的业务目的：把犹豫用户推过决策线}
\begin{itemize}
  \item \textbf{业务公式}：商品定价 100 元，用户犹豫 $\to$ 发 20 元券 $\to$ 净付 80 元 $\to$ 下单。\\
...
\end{itemize}
\srcnote{coupon.CouponBatch.Type/Threshold/Amount 字段；internal/coupon/service.go:27--54 CreateBatch}
\end{frame}

\begin{frame}[shrink]{活动 ROI 业务化帧：100 万发券能换多少 GMV}
...
\srcnote{README 业务承诺 + internal/coupon/service.go Claim 流程；ROI 算式为行业常识、gomall 未实装}
\end{frame}

\begin{frame}[shrink]{coupon 接口的中间件链：业务诉求决定每一层}
\begin{center}
\begin{tikzpicture}[node distance=10mm and 9mm]
  ...
  \draw[arrow-bad] (tx.west) -- node[above,midway,font=\tiny\color{cBad}]{DB FAIL} (rb.east);
\end{tikzpicture}
\end{center}
\vspace{2mm}
...
\srcnote{routes/router.go:41 全局桶；internal/coupon/routes.go:10--13 coupon 路由（无 SlidingWindow）}
\end{frame}

\begin{frame}[fragile,shrink]{coupon Lua：库存 + 单人限领 二维原子}
...
\end{lstlisting}
\vspace{-1mm}
\scriptsize \textbf{业务码映射}：\texttt{1 $\to$ 200 成功}；\texttt{-1 $\to$ "已抢光" 终态}；\texttt{-2 $\to$ "每人限领 N 张"}。Lua 返回 \texttt{int}，Go 侧 switch 翻译成 \texttt{ErrCouponSoldOut / ErrCouponPerUserExceed}（无独立业务码，复用 ERROR=500 + 文案）。
\srcnote{repository/cache/coupon.go:67--85 ClaimCouponAtomic 翻译错误码}
\end{frame}

\begin{frame}[fragile,shrink]{coupon 回滚必须原子：两条独立命令留下的资损口子}
\small \textbf{业务场景}：Lua 已扣库存 + 记单人，但下一步 DB \texttt{PersistClaim} 落库失败，必须把这两个计数\textbf{一起还回去}。早期回滚用两条独立命令、且\textbf{吞掉错误}：
\begin{lstlisting}[language=Go,firstnumber=1,
  caption={旧实现：库存与已领标记分两次写、错误被丢弃}]
RedisClient.Incr(ctx, stockKey)        // 还库存
RedisClient.Decr(ctx, userClaimedKey)  // 撤单人标记 —— 两条独立、无返回
\end{lstlisting}
\vspace{-1mm}
\textbf{业务后果}：两条命令之间崩溃或第二条失败，会出现\textbf{半还状态}——
\begin{itemize}\itemsep0pt
  \item 库存还了、单人标记没撤：用户\textbf{白白少领一张}（明明没领到却被记了一次）。
  \item 单人标记撤了、库存没还：这一张\textbf{库存幽灵消耗}，别人永远抢不到。
  \item 错误被吞 $\to$ 运营无从察觉，活动结束盘点才发现\textbf{发券数对不上}。
\end{itemize}
\srcnote{repository/cache/coupon.go:87--93 RollbackCouponStock}
\end{frame}

\begin{frame}[fragile,shrink]{修法：单条 Lua 原子归还 + 返回 error 供补偿}
\begin{lstlisting}[language=LuaRedis,firstnumber=50,
  caption={\srcnote{repository/cache/coupon.go:50--54 rollbackScript}}]
redis.call('INCR', KEYS[1])   -- 归还库存
redis.call('DECR', KEYS[2])   -- 撤销用户已领标记
return 1
\end{lstlisting}
\vspace{-1mm}
\small 两个动作放进\textbf{同一个 Lua 单元}，Redis 单线程执行保证\textbf{同进同退}，不再有半还状态。这与领券 \texttt{claimScript} 是同一套纪律——\textbf{扣减原子，回滚也必须原子}。
\vspace{1mm}
\begin{itemize}\itemsep0pt
  \item \textbf{返回 error 不再静默}：\texttt{RollbackCouponStock} 把脚本错误向上抛；业务层回滚失败时打 \texttt{Errorf} 级告警，带上 \texttt{userId / batchId} 便于人工补偿对账。
  \item \textbf{对账闭环}：库存计数与单人标记永远一致 $\to$ 盘点时\textbf{发券数 = DB 行数 = Redis 消耗}三者对得上。
\end{itemize}
\srcnote{repository/cache/coupon.go:89--93 返回 error；internal/coupon/service.go:91--94 回滚失败 Errorf 告警}
\end{frame}

\begin{frame}[shrink]{coupon 压测数字：业务侧怎么解读这两组数据}
...
\small \textbf{零超发是底线}：1000 goroutine 抢 100 张券 $\to$ 成功数恰好 100，DB 行数 100，Redis 库存归 0，三者一致。\\
\textbf{业务侧再次确认}：这个不是技术指标，这是"\textbf{不被资损}"的底线 —— 多发出去一张券就是真金白银的损失。\\
\textbf{踩坑回填}：DB 路径的 FOR UPDATE 曾因裸拼 SQL 退化成无锁空操作（并发下照样超发），现已改用 gorm \texttt{clause.Locking} 真实行锁；券码也补了 \texttt{crypto/rand} 随机段，同一用户同一秒连领不再撞唯一索引。
\srcnote{stressTest/REPORT.md 链路压测表 第 5--6 行；repository/cache/coupon.go:30--44；internal/coupon/repo.go:44--90 ClaimWithDBLock、:149--157 generateCouponCode}
\end{frame}
...
\begin{frame}[shrink]{秒杀的业务本质：用"亏本商品"换"全场曝光"}
\begin{itemize}
  \item \textbf{业务公式}：iPhone 标价 8000 元，秒杀价 999 元 $\to$ 100 台 $\to$ 平台亏\textbf{70 万}。\\
...
\end{itemize}
\srcnote{internal/skill/service.go:144--152 GetSkillGoods / SkillProduct；商品独立 skill.SkillProduct 表}
\end{frame}

\begin{frame}[fragile,shrink]{秒杀详情/下单读了一个从没写过的 key：接口必然失败}
\small \textbf{业务现象}：秒杀详情与下单接口\textbf{每次都报错}，用户点开页面就是空白。根因是\textbf{读写 key 对不上}——预热只写了\textbf{列表 key}，详情/下单却按 \texttt{product\_id} 读\textbf{单品 key}，而这个单品 key \textbf{从没被写入}。
\begin{lstlisting}[language=Go,firstnumber=77,
  caption={读单品 key：缓存里永远是 redis.Nil}]
key := fmt.Sprintf(cache.SkillProductKey, productId)  // skill:product:<id>
resp, err := rc.Get(ctx, key).Result()                // 旧代码到此必空
\end{lstlisting}
\vspace{-1mm}
\textbf{业务后果}：秒杀是引流主入口，详情打不开 = 用户\textbf{连"抢不到"的机会都没有} = 活动当场作废，比"手慢没抢到"严重得多。
\srcnote{repository/cache/key.go:12 SkillProductKey；internal/skill/service.go:75--85 loadSkillProduct 读取点}
\end{frame}

\begin{frame}[fragile,shrink]{修法：预热补写单品 key + 读侧未命中回源回填}
\small \textbf{双管齐下}，不再依赖单一预热顺序：
\begin{itemize}\itemsep0pt
  \item \textbf{写侧}：\texttt{InitSkillGoods} 在 LPush 列表的同时，按 \texttt{product\_id} 多写一份\textbf{单品 key}（带 30min TTL）。
  \item \textbf{读侧}：详情/下单未命中时\textbf{回源 DB} \texttt{GetByProductId} 并回填缓存，彻底摆脱"必须先预热"的隐式依赖。
\end{itemize}
\begin{lstlisting}[language=Go,firstnumber=61,
  caption={\srcnote{internal/skill/service.go:61--63 预热补写单品 key}}]
errx = cache.RedisClient.Set(ctx,
    fmt.Sprintf(cache.SkillProductKey, spList[i].ProductId),
    jsonString, cache.SkillProductTTL).Err()
\end{lstlisting}
\vspace{-1mm}
\scriptsize 读侧回填见 \texttt{loadSkillProduct}：\texttt{Get} 命中直接返回；\texttt{redis.Nil} 则查 DB、回写同一 key 再返回；其余错误才向上抛。\texttt{GetSkillGoods}（详情）与 \texttt{SkillProduct}（下单）共用这条读路径。
\srcnote{internal/skill/service.go:75--103 loadSkillProduct 回源回填；internal/skill/repo.go:40--45 GetByProductId}
\end{frame}

\begin{frame}[shrink]{秒杀 vs 普通下单的根本差异：业务侧规则}
\scriptsize
...
\srcnote{internal/skill/service.go:144--152；internal/skill/routes.go:12--25 秒杀路由}
\end{frame}

\begin{frame}[shrink]{SlidingWindow Lua 与"阈值 = 3"的业务依据}
...
\begin{frame}[shrink]{秒杀压测实测：30VU × 15s 误差 2.2\%}
\textbf{场景}：30 个并发用户用\textbf{同一 token}打 \texttt{/skill\_product/skill}，配置 \texttt{Limit=3 Window=1s ByUser=true}（复现"\textbf{一个账号被脚本疯狂调用}"的黑产画像）。
...
\begin{frame}[shrink]{秒杀客诉链路：从"我抢不到"到客服话术}
\begin{center}
\begin{tikzpicture}[node distance=14mm and 12mm]
  ...
  \draw[arrow-bad] (sw.south) -- node[left,midway,font=\tiny\color{cBad}]{70001} (cs1.north east);
  \draw[arrow-bad] (lua.south west) -- node[left,midway,font=\tiny\color{cBad}]{stock=0} (cs2.north);
  \draw[arrow-bad] (lua.south) -- node[right,midway,font=\tiny\color{cBad}]{owned$\geq$N} (cs3.north);
\end{tikzpicture}
\end{center}
\vspace{2mm}
...
\begin{frame}[shrink]{抢红包的业务目的：留存而非促单}
\begin{itemize}
  \item \textbf{业务公式}：发 1000 个 1 元红包 $\to$ 1000 人参与 $\to$ 抢到的"开心"+ 没抢到的"下次还来"。\textbf{钱花得不多，留存效果惊人}。
...
\end{itemize}
\srcnote{internal/redpacket/service.go:41--175 Create + Claim 全流程}
\end{frame}

\begin{frame}[shrink]{公平性事故的真实代价：业务案例}
...
\begin{frame}[fragile,shrink]{二倍均值法：业务约束如何映射成代码}
\small \textbf{业务约束 $\to$ 算法选择}：
\begin{itemize}\itemsep0pt
  \item "总额精确" $\to$ 最后一份 \texttt{out[count-1] = remain} \textbf{兜底吃剩}。
  \item "每份 $\geq$ 1 分" $\to$ 随机区间 \texttt{[1, hi]}，\texttt{maxThisRound = remain - (剩余份数)} 保证后续每份至少 1。
  \item "随机有惊喜" $\to$ 上界 \texttt{2*avg-1}，期望 = avg，方差由 avg 控制。
  \item "位置不敏感" $\to$ 最后 \texttt{Shuffle} 打散数组，否则用户能推断"第一个抢的最大"。
\end{itemize}
\begin{lstlisting}[language=Go,firstnumber=107,
  caption={\srcnote{repository/cache/redpacket.go:107--140 SplitRedPacket 主体}}]
...
\begin{frame}[shrink]{200 元 / 10 份分布示意 + 用户感知}
...
\textbf{产品价值}：每份不等 $\to$ 每个人都\textbf{有故事讲} $\to$ 社交传播 $\to$ 下次活动\textbf{自发参与}。\\
\textbf{对比反面}：等额拆分 = 100 元 / 100 份 = 每人 1 元 $\to$ 没人晒图 $\to$ 留存效果归零。
\srcnote{repository/cache/redpacket.go:113--141；行业经验：拼手气红包 vs 等额红包的留存对比}
\end{frame}

\begin{frame}[fragile,shrink]{抢红包 Lua：业务规则 8 行落地}
...
\end{lstlisting}
\vspace{-1mm}
\scriptsize \textbf{业务码翻译}（Go 侧 \texttt{repository/cache/redpacket.go:168--190}）：\\
\texttt{-1 $\to$ ErrRedPacketDrained} $\to$ 客服话术 "\texttt{红包已被抢完}"（README 称作 \texttt{RP-EMPTY} 文案，代码侧是 Go error）\\
\texttt{-2 $\to$ ErrRedPacketAlreadyTaken} $\to$ 客服话术 "\texttt{您已领取过本红包}"\\
\texttt{> 0 $\to$ 200 + amount} $\to$ 客户端展示动画
\end{frame}

\begin{frame}[shrink]{抢红包 Saga：资金链路的"最后一道防线"}
...
\begin{itemize}\itemsep0pt
  ...
  \item \textbf{回滚}：金额 LPUSH 回队头 + HDEL 撤销 user 标记 $\to$ 下一个用户能抢到 $\to$ 用户也可重试。
\end{itemize}
\textbf{业务承诺}：\textbf{发出去多少钱，必须回到用户钱包}。Saga 是这条业务承诺的代码兑现。\\
\textbf{对应代码}：\texttt{rollbackRedPacketClaimScript}（两个动作放一个 Lua 原子单元）。
\srcnote{repository/cache/redpacket.go:79--90 rollback；internal/redpacket/service.go:155--162 调用点}
\end{frame}
...
\begin{frame}[shrink]{三种活动：业务目的 vs 代码骨架}
\scriptsize
...
\keypoint{业务语义决定结构选型：金额\textbf{同质 $\to$ 计数器}；\textbf{异质 $\to$ 队列}；\textbf{时间窗口 $\to$ ZSet}。\\
...
\begin{frame}[shrink]{三段 Lua 的共同设计纪律：业务诉求驱动}
\begin{enumerate}\itemsep1pt
  \item \textbf{失败也要原子}：判断不通过\textbf{什么都不写} $\to$ 防止"扣了库存但没记上单人"的资损。
  \item \textbf{回滚也是 Lua}：DB 失败时补偿动作走同一种原子机制 $\to$ 防止 Go 侧两次 Redis 调用之间的崩溃。
  \item \textbf{TTL 永远要设}：尤其领取标记类 key $\to$ 防止用户停留态 key 无限堆积撑爆 Redis。
  \item \textbf{返回值留语义}：用 \texttt{>0 / 0 / 负数} 区分 $\to$ 业务层 \texttt{switch} 翻译成\textbf{客服话术}。
  \item \textbf{进程内 rand}：发包拆分时不用 \texttt{math/rand} 全局源 $\to$ 防止热点路径并发争锁。
\end{enumerate}
\vspace{2mm}
...
\begin{frame}[shrink]{黑产对营销 ROI 的真实破坏：业务后果帧}
...
黑产手法 & 业务后果 \\
\midrule
\textbf{脚本扫券}      & 万号自动领券 $\to$ \textbf{真用户领不到}，运营预算\textbf{打 7 折}吃掉 \\
\textbf{机器人秒杀}    & 1 秒打 50 次 $\to$ 真人 1 秒最多 2 次，\textbf{真人 0\% 抢到} \\
\textbf{撞库刷红包}    & 用泄露密码盗号抢红包 $\to$ 用户钱包资金\textbf{凭空消失}，投诉爆炸 \\
\textbf{券倒卖}        & 黄牛领券后\textbf{二手平台 50\% 折价转卖} $\to$ 平台拉新归零 \\
\bottomrule
\end{tabular}
\vspace{2mm}
...
\begin{frame}[shrink]{gomall 拦的是哪一类黑产：能力分层}
\scriptsize
...
L2 网关   & IP TokenBucket 100/IP & \textbf{已接}（router.go:41 全局） & 单机脚本 / 自动化爬虫 \\
...
\end{itemize}
\srcnote{routes/router.go:41 全局桶；internal/redpacket/routes.go:20--25 红包 SlidingWindow；internal/skill/routes.go:17--24 秒杀 SlidingWindow}
\end{frame}

\begin{frame}[shrink]{coupon 路由的故意"漏洞"：业务取舍而非疏忽}
...
\srcnote{internal/coupon/routes.go:10--13 coupon 路由无 SlidingWindow；repository/cache/coupon.go:37 PerUser 校验}
\end{frame}
...
\begin{frame}[shrink]{大促当天的客诉流量画像}
...
"我抢到红包但用不了" & 流程未完成 & 查 outbox 表 $\to$ 重投 red\_packet.claimed 事件\\
\bottomrule
\end{tabular}
\vspace{2mm}
...
\begin{frame}[shrink]{客服话术红线：业务码到 UX 文案的映射}
\begin{tabular}{@{}lll@{}}
\toprule
...
\begin{frame}[shrink]{"我抢到红包但金额很少"：公平性解释 SOP}
...
\begin{frame}[allowframebreaks]{大促前 7 天准备清单：5 角色协同}
\scriptsize
...
SRE & Redis 库存\textbf{预热}（活动券 / 秒杀商品 / 红包预拆 SET 入库）/ 限流阈值\textbf{预设到大促值}（3/s $\to$ 评估是否拉到 5/s）/ 监控大盘\textbf{重点指标}：70001 数 / Lua RTT p99 / Redis CPU \\
客服 & \textbf{话术下发}（上一节那张映射表）/ 客诉演练彩排 / 常见 FAQ 上线 \\
...
\footnotesize\textbf{T-1 复盘}：\textbf{压测 30VU × 15s 跑一遍} 验证限流和库存预热 —— 看是否 46/45 误差 < 5\%。
...
\begin{itemize}\itemsep0pt
  ...
  \item 风控\textbf{实时识别}单账号 / 单 IP 异常 $\to$ 触发临时拉黑。
\end{itemize}
\srcnote{stressTest/REPORT.md \S 5；README "运行" / docker-up / make env-up 准备依赖}
\end{frame}

\begin{frame}[shrink]{活动结束盘点：4 个维度复盘}
\scriptsize
...
\begin{itemize}\itemsep0pt
  ...
  \item \textbf{话术迭代}：客诉热点 $\to$ 下次话术增补。
  \item \textbf{阈值校准}：本次 SlidingWindow 限流数 / 通过数 $\to$ 下次活动参考。
\end{itemize}
...
\begin{frame}[shrink]{三个活动的各角色视图汇总}
\scriptsize
\begin{tabular}{@{}p{0.16\linewidth}p{0.42\linewidth}p{0.38\linewidth}@{}}
\toprule
角色  & 关心指标 & 数据来源 \\
\midrule
\textbf{运营}     & 领取率 / 核销率 / GMV / 拉新数 / ROI & \texttt{coupon\_batch.finished} + 订单表 JOIN \\
\textbf{风控}     & 70001 占比 / 单账号高频 / 核销率异常 & 业务码统计 + 用户行为日志 \\
\textbf{客服}     & 投诉单 $\to$ 业务码反查 / 话术覆盖率 & status 字段 + Jaeger trace 反查 \\
...
\begin{frame}[shrink]{大促当天监控大盘的 6 个核心图}
...
\begin{itemize}\itemsep0pt
  \item \textbf{业务预警}：GMV 增长曲线 vs 预期 $\to$ 偏离 > 20\% 运营介入。
  \item \textbf{安全预警}：70001 飙升 + 核销率骤降 $\to$ 黑产攻击，风控降级。
  \item \textbf{系统预警}：Redis CPU > 80\% 或 Lua p99 > 100ms $\to$ SRE 准备熔断。
\end{itemize}
\textbf{联动开关}：监控大盘旁边\textbf{常备 3 个红色按钮}——
\begin{enumerate}\itemsep0pt
  \item \textbf{临时拉高限流}（3/s $\to$ 1/s）—— 黑产攻击降级。
...
\begin{frame}[shrink]{Q\&A（上）：业务取舍与原子性}
\textbf{Q1}：优惠券为什么不挂用户 SlidingWindow？\\
\hspace{4mm}\small A：业务侧\textbf{鼓励多领}（沉没成本$\to$转化），Lua 已管 PerUser；信任 IP 桶 + 运营核销率监控兜底。\\
\hspace{4mm}\small \textbf{代码}：\texttt{routes/router.go:41}；\texttt{internal/coupon/routes.go:10--13}；\texttt{repository/cache/coupon.go:37}。

\vspace{2mm}
\textbf{Q2}：100 万发券的 ROI 怎么算？\\
\hspace{4mm}\small A：发券 $\to$ 领取率 60\% $\to$ 核销率 20\% $\to$ 带动 GMV。让利 12 万换 90 万 GMV，ROI = 7.5。黑产吃 30\% 流量 $\to$ ROI 跌到 3.0。\\
\hspace{4mm}\small \textbf{代码}：\texttt{internal/coupon/service.go:62--96 Claim}（gomall 暂未实装 ROI 仪表盘）。
...
\hspace{4mm}\small \textbf{代码}：\texttt{repository/cache/redpacket.go:79--90}、\texttt{internal/redpacket/service.go:155--162}。

\vspace{2mm}
\textbf{Q4}：秒杀为什么不挂熔断？\\
\hspace{4mm}\small A：熔断 = "全场买不到" = 活动作废，比"部分用户买不到"严重 10 倍。\\
\hspace{4mm}\small \textbf{代码}：\texttt{internal/skill/routes.go:12--25}（无 CircuitBreaker）。
\end{frame}

\begin{frame}[shrink]{Q\&A（下）：黑产、客服与公平性}
\textbf{Q5}：3/s/用户 这个阈值是怎么定的？\\
\hspace{4mm}\small A：真人 300--500ms 点一次，3 留 50\% 余量；压测 46/45 误差 2.2\%；取 1 客诉爆，取 10 形同没限。\\
\hspace{4mm}\small \textbf{代码}：\texttt{internal/skill/routes.go:17--24}；\texttt{stressTest/REPORT.md \S 5}。
...
\hspace{4mm}\small \textbf{代码}：\texttt{routes/router.go:41}；\texttt{internal/coupon/routes.go:10--13}。

\vspace{2mm}
\textbf{Q8}：抢券失败、抢红包失败、秒杀失败，客服怎么区分？\\
\hspace{4mm}\small A：按业务码：70001 限流$\to$"稍后再试"；sold-out$\to$"已抢完"；exceed$\to$"已领过"。\textbf{绝不能}说"系统繁忙"。\\
\hspace{4mm}\small \textbf{代码}：\texttt{pkg/e/code.go:57--58}；\texttt{repository/cache/coupon.go:56--59}；\texttt{redpacket.go:22--26}。
\end{frame}

\begin{frame}[shrink]{代码位置一览（一）：业务实现}
\begin{description}\small
  \item[coupon Lua（原子扣减 + 单人限领）]\srcnote{repository/cache/coupon.go:30--44}
  \item[coupon 错误码翻译]\srcnote{repository/cache/coupon.go:56--85}
  \item[coupon 回滚 Lua（原子归还 + 返回 error）]\srcnote{repository/cache/coupon.go:50--54、:87--93}
  \item[coupon 业务层 Claim]\srcnote{internal/coupon/service.go:62--96}
  \item[coupon 库存预热（创建批次）]\srcnote{internal/coupon/service.go:27--54}
  \item[coupon DB 锁路径（clause.Locking + 随机券码）]\srcnote{internal/coupon/repo.go:44--90、:149--157}
  \item[coupon 路由（无 SlidingWindow，业务取舍）]\srcnote{internal/coupon/routes.go:10--13}
  ...
  \item[红包 Service 调用 Saga 点]\srcnote{internal/redpacket/service.go:155--162}
  \item[红包过期回收]\srcnote{repository/cache/redpacket.go:65--77 ReleaseRedPacketLeft}
  \item[红包路由（SlidingWindow + Idempotency）]\srcnote{internal/redpacket/routes.go:17--27}
  \item[秒杀业务（详情/下单读单品 key + 回源回填）]\srcnote{internal/skill/service.go:75--103、:144--152}
  \item[秒杀路由（SlidingWindow 3/s/用户）]\srcnote{internal/skill/routes.go:17--24}
\end{description}
\end{frame}

\begin{frame}[shrink]{代码位置一览（二）：限流、业务码、压测}
\begin{description}\small
  \item[SlidingWindow Lua（ZSet 滑窗）]\srcnote{repository/cache/ratelimit.go:18--34}
  \item[全局 TokenBucket（100 RPS / IP）]\srcnote{routes/router.go:41 middleware/ratelimit.go}
  \item[业务码 70001 / 70002 / 60002 / 50001]\srcnote{pkg/e/code.go:49--58}
  \item[Go error（红包 / 优惠券）]\srcnote{repository/cache/coupon.go:56--59；redpacket.go:22--26}
  ...
  \item[拼手气红包模型（gomall 已实现）]\srcnote{internal/redpacket/service.go:41--175 Create + Claim}
