...
15 分钟   & 多数人够用         & 高峰期仍有 5--8\% 误关，弱网感受不友好 \\
...
\vspace{1mm}
\small 30min 是工业界经验值（京东 30min / 天猫 30min--1h / 拼多多 15min）。改这个值要谈\textbf{5 个角色}：商家 / 运营 / 风控 / 客服 / 财务——代码侧只落到 \texttt{OrderCancelDelay} 一行。
...
\end{tikzpicture}
\begin{itemize}
  \item \textbf{只 RMQ}：进程挂 / 丢消息 / nack 进 DLX = 订单永远停 UnPaid。
  \item \textbf{只 Cron}：5min 间隔 + 单批 100 上限，库存释放 lag 偏大。
  \item \textbf{双保险}：RMQ 30min 准点 + Cron 15min 窗口 / 5min 节奏。
  \item \textbf{Cron 用 15min 而非 30min}：RMQ 正常时订单 30min 已被关掉、Cron 不抢功；RMQ 挂时 Cron 立刻接管，最多多锁 15--20min。
\end{itemize}
\srcnote{order\_delay.go:24--56；order\_task.go:14--48,:16 GetTimeoutOrders(15,100)}
\end{frame}
...
func (dao *OrderDao) CloseOrderWithCheck(orderNum uint64) (bool, error) {
    res := dao.DB.Model(&model.Order{}).
        Where("order_num=? and type=?", orderNum, consts.UnPaid).
...
\end{lstlisting}
\begin{itemize}
  \item \texttt{WHERE type=UnPaid}：只有 UnPaid 会被改成 Cancelled，已 Cancelled / Paid 直接 RowsAffected=0。
  \item 第二次调用 RowsAffected=0 $\to$ closed=false $\to$ 直接 return，不释放库存 / 不写 outbox。
\end{itemize}
\srcnote{order.go:185--196 幂等 SQL；order\_cancel.go:30--57 closed 旗标}
\end{frame}
...
\begin{itemize}
  \item \texttt{order.cancelled} 下游绝对\textbf{不能}包含"通知商家发货"——商家收到"发货" $\to$ 真发货 $\to$ 用户没付款 $\to$ 商家亏运费 + 货损。
  \item 合理下游：ES 删索引 / 库存对账 / 客服工单关闭 / 推送"订单已取消"。
  \item \textbf{关键不变量}：\texttt{order.cancelled} 永远只对应 type=UnPaid（已 Paid 走 \texttt{order.refunded}）。
\end{itemize}
\keypoint{关单是触发一组下游业务动作的金融事件——谁该消费、谁不该，事件设计时定死。}
\vspace{1mm}
\small \textbf{对账兜底}：凌晨低峰跑 \texttt{SUM(reserved)} vs \texttt{SUM(UnPaid.Num)} 偏差告警，是双保险都漏时的最后一道闸（运维 backlog 第 1 项，待落地）。
...
位置 & 数值 & 用途 \\
\midrule
\texttt{order\_delay.go:22}      & 30min & RMQ TTL（业务关单时机）\\
\texttt{order\_task.go:16}       & 15min & Cron 扫表窗口（兜底安全余量）\\
...
\vspace{1mm}
\begin{itemize}
  \item \texttt{routes/router.go:81} 的 \texttt{orders/delete} 直接调 OrderDelete $\to$ DeleteOrderById（GORM 软删除）。
  \item \textbf{漏了 release reservation 和 outbox 写}——Redis 桶仍占库存，下游 ES 不知道订单消失。
  \item 建议要么对 UnPaid 改走 CancelUnpaidOrder，要么禁掉这个 endpoint。
\end{itemize}
\srcnote{routes/router.go:81；service/order.go:168--192；repository/db/dao/order.go:149--154}
\end{frame}

\begin{frame}[fragile]{遗留 3：Cron 表达式 bug——意图 5min 一次, 实际 60 次}
\begin{lstlisting}[language=Go,firstnumber=14,
  caption={initialize/cron.go:14--16 触发表达式},captionpos=b]
c := cron.New(cron.WithSeconds())
orderService := new(service.OrderTaskService)
_, err := c.AddFunc("* */5 * * * *", func() { /* ... */ })
\end{lstlisting}
\begin{itemize}
  \item 6 字段 cron \texttt{秒 分 时 日 月 周}：\texttt{* */5 * * * *} = 第 0/5/10/\dots 分钟里\textbf{每秒都触发}，5min 窗口内打 60 次。
  \item 业务意图 "每 5 分钟跑一次"，正确写法 \texttt{0 */5 * * * *}（秒位 = 0）。
  \item 影响：DB 被无谓打 60 倍 + 日志被淹没。功能层面靠 CloseOrderWithCheck 幂等没产生 bug。
\end{itemize}
...
\begin{itemize}
  ...
  \item CancelUnpaidOrder 幂等不是"代码优雅"，是因为\textbf{真的有 4 个调用方}并发触发。
  \item 关单是金融事件——下游消费者必须设计成\textbf{不能给商家发货通知}。
  \item 三处遗留（口径 / orders/delete / cron 表达式）说明业务承诺到代码的最后一公里需要持续打磨。
\end{itemize}
\keypoint{关单治理的核心是\textbf{把"30 分钟"落到 6 条承诺 + 双保险 + 幂等 SQL 上}。}
\end{frame}
...
\hspace{4mm}\small A：幂等关单 SQL 兜住了功能正确性，但 DB QPS 被无谓放大 60 倍；监控里能看出 SELECT type=UnPaid 异常高频。改 \texttt{0 */5 * * * *} 即可。

\vspace{2mm}
\textbf{Q6}：为什么不直接用 Redis ZSet 替代 RMQ？\\
\hspace{4mm}\small A：order.go:88--92 的 ZSet 只用作内部状态。换 ZSet 替代 RMQ 要自己写消费循环 + ack + 重试 + DLX——RMQ 已经打包好了，当前业务量性价比更高。

\vspace{2mm}
\textbf{Q7}：对账脚本什么时候上？\\
\hspace{4mm}\small A：运维 backlog 第 1 项。当前靠商家反馈"库存对不上"才查 GetStockSnapshot，不是好状态。

\vspace{2mm}
\textbf{Q8}：双保险能保证 100\% 不漏单吗？\\
\hspace{4mm}\small A：不能。outbox 提交但 publisher 挂掉这条漏，仍要靠\textbf{对账}做最终兜底。
\end{frame}
