...
\begin{tikzpicture}[node distance=4mm and 8mm,every node/.style={font=\scriptsize}]
  ...
  \node[node-box,right=of fin] (ship) {商家发货\\deck 09};
...
\end{itemize}
\srcnote{routes/preorder\_routes.go 路由四件套；service/preorder.go::PayDeposit/PayFinal 主入口}
\end{frame}
...
\begin{itemize}\small
  ...
  \item \textbf{平台}：\textbf{GMV 前置}。原本 11.11 当天爆发的 GMV 提前到 10.20-11.10 平摊，下单链路负载从 10x 平均 → 3x 平均。
...
\end{itemize}
\srcnote{repository/db/model/preorder.go 字段定义；service/preorder.go::PayDeposit 窗口校验}
\end{frame}
...
\begin{tikzpicture}[node distance=6mm and 11mm]
  ...
  \draw[arrow] (n1) -- node[below,font=\tiny]{定金期内 Cancel} (n4);
\end{tikzpicture}
\vspace{1mm}
\begin{itemize}\small
...
\end{itemize}
\srcnote{repository/db/model/preorder.go \texttt{PreorderStageXxx} 四个常量；service/preorder.go::forfeitOne 推进逻辑}
\end{frame}
...
\begin{itemize}\small
  ...
  \item 选 B 原因：
    ...
    \end{itemize}
\end{itemize}
\srcnote{consts/order.go 7 态机；service/order\_state.go::orderStateTransitions}
\end{frame}
...
\end{itemize}
\srcnote{repository/db/model/preorder.go:18--26；repository/db/model/order.go PreorderStage 字段}
\end{frame}
...
\begin{lstlisting}[language=Go]
// service/preorder.go::PayDeposit
...
err = dao.NewDBClient(ctx).Transaction(func(tx *gorm.DB) error {
    dao.NewOrderDaoByDB(tx).CreateOrder(order)
    debitUser(tx, u.Id, req.BossID, req.Key, pp.DepositCents)
    dao.NewPreorderDaoByDB(tx).MarkDepositPaid(tx, order.ID, now)
...
\end{lstlisting}
\srcnote{service/preorder.go::PayDeposit 完整 70 行；repository/cache/inventory.go::ReserveStock Lua}
\end{frame}

\begin{frame}[fragile]{PayFinal：尾款扣款 + 真扣库存 + 转 WaitShip}
\begin{lstlisting}[language=Go]
// service/preorder.go::PayFinal
// 1) 业务校验
...
err = baseDao.DB.Transaction(func(tx *gorm.DB) error {
    debitUser(tx, u.Id, order.BossID, req.Key, pp.FinalCents)
    // product.Num -= order.Num  (沿用 service/payment.go 口径)
    productDao.UpdateProduct(...)
    dao.NewPreorderDaoByDB(tx).MarkFinalPaid(tx, order.ID, now)
    return outboxDao.Insert("order", "PreorderFinalPaid", ...)
})
// 3) 事务成功后清 Redis reserved；失败不回滚 DB（DB 是业务真相）
cache.CommitReservation(ctx, order.ProductID, int64(order.Num))
\end{lstlisting}
\srcnote{service/preorder.go::PayFinal；service/payment.go::PayDown 同口径 AES 扣款}
\end{frame}

\begin{frame}[fragile]{cron 没收：单一不可靠靠两层兜底}
\begin{lstlisting}[language=Go]
// service/preorder.go::ForfeitDepositsForUnpaidFinals
...
err := baseDao.DB.Transaction(func(tx *gorm.DB) error {
    ...
    ok, _ := dao.NewPreorderDaoByDB(tx).ForfeitDeposit(tx, order.ID)
...
\end{lstlisting}
\vspace{0.5mm}
\srcnote{service/preorder.go::ForfeitDepositsForUnpaidFinals；repository/db/dao/preorder.go::ListUnpaidFinalBefore JOIN}
\end{frame}
...
\end{itemize}
\srcnote{repository/db/dao/preorder.go::MarkDepositPaid/MarkFinalPaid/ForfeitDeposit；deck 09 同口径}
\end{frame}
...
\end{itemize}
\srcnote{service/events/events.go::PreorderXxx 四个 struct；service/outbox/publisher.go 投递}
\end{frame}
...
\begin{itemize}\small
  ...
  \item \textbf{业务码 82004} \texttt{ErrPreorderForfeitedDeposit} 出现在：
    \begin{itemize}\footnotesize
      \item 预售结束后用户尝试取消 → 返回 82004
      \item 尾款期内用户尝试取消 → 返回 82004
      \item cron 没收后用户查询订单 → 客服界面显示 82004 + 中文话术
    \end{itemize}
\end{itemize}
\keypoint{业务承诺写得越清楚，客诉转化为"按规则赔付"的比例越高 — 这是\textbf{客服效率指标}。}
...
\end{itemize}
\srcnote{types/preorder.go::PreorderShowResp 字段；前端实现路线图 docs/architecture/feature-matrix.md}
\end{frame}
...
\vspace{2mm}
\begin{itemize}\small
  \item \textbf{C 端最大焦虑}：忘付尾款 → 定金没了。解决：尾款期开始前 \textbf{24h / 4h / 1h 三次 push 提醒}。
  \item \textbf{锁价心理}：用户付 200 元定金的核心动机是"现在锁住 2000 元这个价"，营销文案要强化"双 11 当天补尾款 = 现在的价"。
  \item \textbf{用户故事}：付定金后想退 — 定金期可全退；尾款期"我先看看其它家" → 不退；失效期忘付 → 定金没了。
\end{itemize}
\srcnote{service/preorder.go::CancelPreorderInDepositWindow 取消路径；events.PreorderCancelled 通知钱包}
\end{frame}
...
\begin{itemize}\small
  \item \textbf{GMV 前置比例}：原本 11.11 当天 100\% 爆发 → 预售期 \textbf{30\%-40\%} GMV 提前落账（定金 + 尾款都算 GMV）。
...
\begin{itemize}\small
  ...
  \item \textbf{真实故障假设}：
    \begin{itemize}\footnotesize
      \item cron 进程崩 / 进程未起 → 失效订单滞留 \texttt{stage=1}（用户能"补付"窗口实际拖长） → 监控滞后 60min 报警。
      \item outbox 事件丢 → 商家不知道哪些定金被没收 → 工作台 \texttt{deposit\_paid} 计数虚高 → 错下货。
      \item 数据库时钟漂移 → \texttt{final\_end\_at} 判定不准 → \textbf{服务端时钟必须 NTP 强制同步}。
    \end{itemize}
\end{itemize}
\srcnote{service/preorder.go::nowFn 时钟函数指针；service/preorder.go::forfeitOne 单笔事务}
\end{frame}
...
\begin{itemize}\small
  ...
  \item 业务码透出方式：\texttt{service.CodeOf(err)} 提取，\texttt{api/v1/preorder.go::respondPreorderErr} 标准化返回。
\end{itemize}
\srcnote{pkg/e/code.go:75--80 82xxx 段；api/v1/preorder.go::respondPreorderErr 业务码透出}
\end{frame}
...
\begin{itemize}\small
  ...
  \item 客服后台日均统计每个业务码出现次数 → 反推产品 / 运营优化方向：
    \begin{itemize}\footnotesize
      \item 82001 多 → 用户分不清窗口期，加日历提醒
      \item 82004 多 → 失效率高，复盘商品价格 / 营销定位
    \end{itemize}
\end{itemize}
...
\begin{itemize}\small
  ...
  \item \textbf{cron 1h}：尾款期一般是数天 - 周，没收延迟 1h 业务可接受；缩短到分钟级需独立 cron 进程，路线图阶段。
\end{itemize}
...
\end{itemize}
\srcnote{stressTest/REPORT.md §1 基线；service/payment.go::PayDown AES 链路实测复用}
\end{frame}
...
\begin{itemize}\small
  ...
  \item \textbf{对 gomall 的启示}：
    \begin{itemize}\footnotesize
      \item 定金链路当前 SLO 余量充足（45K RPS 容量 vs 80K 峰值需求）→ 异步削峰路径（deck 10）需要扩到 preorder
      \item 尾款链路峰值远超基线 → \textbf{需要类似 \texttt{/orders/enqueue} 的异步路径}（路线图）
      \item Redis reserved 桶在双 11 前 1-2 天会涨到日常 50-100 倍，要给 Redis 扩容预留容量
    \end{itemize}
\end{itemize}
\srcnote{stressTest/REPORT.md；deck 10 异步削峰章节；service/order\_async.go 异步路径复用方向}
\end{frame}
...
\end{itemize}
\srcnote{service/preorder.go::PayDeposit 失败路径释放；service/order\_cancel.go 同口径 release}
\end{frame}
...
\begin{itemize}\small
  ...
  \item \textbf{解决}：两者都走 \texttt{WHERE preorder\_stage=1 AND type=WaitPay} 条件 UPDATE，\textbf{数据库行锁兜底}。
    \begin{itemize}\footnotesize
      \item 用户付尾款先 commit → stage 2 → cron \texttt{ForfeitDeposit} \texttt{RowsAffected=0} → \texttt{forfeitOne return nil}
      \item cron 先 commit → stage 3 → 用户 \texttt{MarkFinalPaid} \texttt{RowsAffected=0} → 返回"订单状态已变更"
    \end{itemize}
  \item \textbf{业务结论}：\textbf{先到先得，绝不双扣}。用户兜底场景应在前端引导"提前 5min 内不再开放支付"以避免边缘 case。
\end{itemize}
\srcnote{repository/db/dao/preorder.go::MarkFinalPaid/ForfeitDeposit 双向条件 UPDATE 兜底}
\end{frame}

\begin{frame}{业务全景：4 个 outbox 事件接到哪}
\begin{tikzpicture}[node distance=4mm and 10mm,every node/.style={font=\scriptsize}]
  \node[node-box] (svc) {preorder service};
  \node[node-box,right=of svc] (outbox) {outbox\_event 表};
  \node[node-box,right=of outbox] (pub) {publisher\\1s 轮询};
  \node[node-box,right=of pub] (mq) {RabbitMQ\\domain.events};
  \node[node-box,above=of mq,node-ok] (wallet) {钱包服务\\入账 / 退款};
  \node[node-box,below=of mq,node-warn] (merchant) {商家工作台\\备货 / 发货};
  \node[node-box,right=of mq] (data) {数据宽表\\转化漏斗 / 报表};
...
\end{itemize}
\srcnote{service/outbox/publisher.go 投递；events/events.go::Preorder* 四个事件}
\end{frame}
...
尾款异步路径 & 11.11 0 点峰值削峰 & 复用 service/order\_async.go 路径 \\
...
\begin{itemize}\small
  \item \textbf{1 单 1 件硬约束}：当前 PayDeposit 强制 \texttt{Num=1}，多件下单需走普通链路。理由：预售场景下用户决策粒度天然是"单件商品"，多件并发付款会引入 SKU + 库存配对复杂度。
  \item \textbf{cron 表达式}：本 deck 未引入 preorder cron（路线图阶段加 \texttt{initialize/cron.go} 的 \texttt{ForfeitDepositsForUnpaidFinals} 调用）。当前\textbf{未自动触发}，需手动调用 service 入口测试。
  \item \textbf{没收资金归属未定}：\texttt{PayDeposit} 时定金已划给商家，\texttt{Forfeited} 事件只标状态、不动钱包。"定金应归平台还是商家还是分账"留给 wallet 服务消费 \texttt{preorder.forfeited} 时决定。
...
\end{itemize}
\srcnote{service/preorder.go::PayDeposit Num=1；service/preorder.go::nowFn 测试钩子}
\end{frame}
...
\hspace{4mm}\small A：旧消费者只读 \texttt{type}，扩 \texttt{type} 要改一票下游；\texttt{stage} 做正交子状态，对旧消费者透明 — 见 deck 09 17 例状态转换测试零改动。\srcnote{repository/db/model/order.go:24}

\vspace{2mm}
\textbf{Q2}：定金期内取消，怎么保证"定金真的退回"？\\
\hspace{4mm}\small A：\texttt{CancelPreorderInDepositWindow} 在事务内调 \texttt{refundUser}（与 \texttt{debitUser} 反向 AES），同事务写 outbox \texttt{preorder.cancelled} 给钱包二次记账，双保险。\srcnote{service/preorder.go::CancelPreorderInDepositWindow}

\vspace{2mm}
\textbf{Q3}：用户在 \texttt{final\_end\_at - 1s} 付尾款，cron 同时触发，谁赢？\\
\hspace{4mm}\small A：先 commit 者赢 — 双方走条件 UPDATE \texttt{WHERE stage=1 AND type=WaitPay}，数据库行锁兜底。后到者 \texttt{RowsAffected=0}，业务上自然降级（用户付款 → 用户报错 / cron → 静默跳过）。\srcnote{repository/db/dao/preorder.go::MarkFinalPaid/ForfeitDeposit}
...
\hspace{4mm}\small A：法律明确支持（《民法典》第 587 条定金罚则）。\textbf{合规要点}：必须在用户支付定金\textbf{前}明确告知，字号不得小于其它条款。gomall 通过 \texttt{ShowPreorder} 返回的字段强制前端拼《预售须知》弹窗。\srcnote{types/preorder.go::PreorderShowResp 字段给前端}
...
\begin{description}\footnotesize
  \item[预售配置表]\srcnote{repository/db/model/preorder.go:18--26}
  \item[Order 字段扩展（PreorderStage / DepositPaidAt / FinalPaidAt）]\srcnote{repository/db/model/order.go}
  \item[DAO（GetPreorder / Mark* / Forfeit / List*）]\srcnote{repository/db/dao/preorder.go}
  \item[PayDeposit 主入口（窗口 + Reserve + Tx + outbox）]\srcnote{service/preorder.go::PayDeposit}
  \item[PayFinal 主入口（扣尾款 + 真扣库存 + 状态机）]\srcnote{service/preorder.go::PayFinal}
  \item[CancelInDepositWindow（全退路径）]\srcnote{service/preorder.go::CancelPreorderInDepositWindow}
  \item[ForfeitDepositsForUnpaidFinals（cron 入口）]\srcnote{service/preorder.go::ForfeitDepositsForUnpaidFinals}
  \item[ShowPreorder（公开查询）]\srcnote{service/preorder.go::ShowPreorder}
  \item[HTTP handler 四件套]\srcnote{api/v1/preorder.go}
  ...
  \item[单测覆盖 8 例]\srcnote{service/preorder\_test.go}
\end{description}
