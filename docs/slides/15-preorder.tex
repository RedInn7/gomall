...
\begin{frame}[shrink]{预售在电商业务全景中的位置}
\begin{center}
...
\begin{frame}[shrink]{业务诉求：为什么要拆"定金 + 尾款"}
\begin{itemize}\small
...
\begin{frame}[shrink]{五角色眼中的同一笔预售单}
\small
...
\begin{frame}[shrink]{业务边界：本 deck \textbf{不做} 什么}
\begin{itemize}\small
...
\begin{frame}[shrink]{三窗口时间轴：每个窗口能做 / 不能做什么}
...
\begin{frame}[shrink]{状态机：order.type + preorder\_stage 双维度}
\begin{center}
...
\begin{frame}[shrink]{为什么 type 维持 WaitPay？兼容性决策}
\begin{itemize}\small
...
\begin{frame}[fragile,shrink]{数据模型：product\_preorder 与 order 字段扩展}
...
\begin{frame}[fragile,shrink]{PayDeposit 的 4 步原子操作}
\begin{lstlisting}[language=Go]
...
\begin{frame}[fragile,shrink]{PayFinal：尾款扣款 + 真扣库存 + 转 WaitShip}
\begin{lstlisting}[language=Go]
...
\begin{frame}[fragile,shrink]{cron 没收：单一不可靠靠两层兜底}
\begin{lstlisting}[language=Go]
...
\begin{frame}[shrink]{DAO 层：条件 UPDATE = 状态机 + 幂等}
...
\begin{frame}[shrink]{Outbox 四事件：业务全景的真实事件流}
{\footnotesize
...
\begin{frame}[shrink]{定金不退：业务承诺 + 法律依据}
\begin{itemize}\small
...
\begin{frame}[shrink]{合规帧：用户支付定金前必看 5 行字}
...
\begin{frame}[shrink]{C 端用户视角：3 个时间窗口看到什么}
{\footnotesize
...
\begin{frame}[shrink]{商家视角：备货决策的业务化}
\begin{itemize}\small
...
\begin{frame}[shrink]{运营视角：GMV 前置 + 转化漏斗}
\begin{itemize}\small
...
\begin{frame}[shrink]{客服视角：三大投诉与 SOP}
{\footnotesize
...
\begin{frame}[shrink]{SRE 视角：cron 没收的可观测性}
\begin{itemize}\small
...
\begin{frame}[shrink]{82001-82004 业务码对照表}
{\footnotesize
...
\begin{frame}[fragile,shrink]{业务码客服话术（中文版）}
\begin{lstlisting}[language=Go]
// pkg/e/msg.go
...
\begin{frame}[shrink]{SLO：每个接口的业务承诺}
{\footnotesize
\begin{tabular}{@{}llrr@{}}
\toprule
接口 & 业务等级 & p99 延迟 & 可用率 \\
\midrule
\texttt{POST /preorder/:id/deposit} & P0 交易 & < 200ms & 99.95\% \\
\texttt{POST /preorder/:id/final}   & P0 交易 & < 500ms & 99.95\% \\
\texttt{POST /preorder/:id/cancel}  & P1 售后 & < 1s   & 99.9\%  \\
\texttt{GET  /preorder/:id}         & P2 信息 & < 100ms & 99.9\%  \\
\midrule
...
\vspace{2mm}
\begin{itemize}\small
  \item \textbf{通配符统一 \texttt{:id}}：语义随接口而定 —— show / deposit 的 \texttt{:id} 是 productID，final / cancel 的 \texttt{:id} 是 orderID（见 \texttt{internal/preorder/routes.go:9--14} 接口契约注释）。
...
\begin{frame}[shrink]{压测参考：复用一次性下单的基线数据}
\begin{itemize}\small
  ...
  \item 预售\texttt{/preorder/:id/deposit} 链路结构等同 \texttt{orders/create + 1 次 AES 扣款}：
    \begin{itemize}\footnotesize
...
\begin{frame}[shrink]{真实数字：双 11 历年预售规模参考}
\begin{itemize}\small
  \item 行业历史数据（业务侧公开报告）：
    \begin{itemize}\footnotesize
...
\begin{frame}[shrink]{Saga 视角：4 个故障路径的兜底}
{\footnotesize
\begin{tabular}{@{}lll@{}}
\toprule
失败点 & 表现 & 兜底动作 \\
...
\begin{frame}[shrink]{真实事故假设：用户卡点付款 vs cron 没收的竞态}
\begin{tikzpicture}[node distance=4mm and 8mm,every node/.style={font=\scriptsize}]
...
\begin{frame}[shrink]{业务全景：4 个 outbox 事件接到哪}
\begin{center}
...
\begin{frame}[shrink]{本期落地：cron 没收兜底 + 规避 60×/min 陷阱}
\begin{itemize}\small
...
\begin{frame}[shrink]{路线图：本期之后做什么}
{\footnotesize
...
\hspace{4mm}\small A：旧消费者只读 \texttt{type}，扩 \texttt{type} 要改一票下游；\texttt{stage} 做正交子状态，对旧消费者透明 — 见 deck 09 17 例状态转换测试零改动。

\vspace{2mm}
\textbf{Q2}：定金期内取消，怎么保证"定金真的退回"？\\
\hspace{4mm}\small A：\texttt{CancelPreorderInDepositWindow} 在事务内调 \texttt{refundUser}（与 \texttt{debitUser} 反向 AES），同事务写 outbox \texttt{preorder.cancelled} 给钱包二次记账，双保险。

\vspace{2mm}
\textbf{Q3}：用户在 \texttt{final\_end\_at - 1s} 付尾款，cron 同时触发，谁赢？\\
\hspace{4mm}\small A：先 commit 者赢 — 双方走条件 UPDATE \texttt{WHERE stage=1 AND type=WaitPay}，数据库行锁兜底。后到者 \texttt{RowsAffected=0}，业务上自然降级（用户付款 $\to$ 用户报错 / cron $\to$ 静默跳过）。

\vspace{2mm}
\textbf{Q4}：定金 + 尾款是\textbf{两次扣款} vs \textbf{冻结 + 扣款}，为什么选两次？\\
\hspace{4mm}\small A：冻结需要支付通道（支付宝 / 微信）支持"预授权"接口，gomall 当前\textbf{不真接}第三方支付，AES 模拟扣款。两次扣款链路更简单、更与现有 \texttt{PayDown} 一致。生产接通预授权后再切。
\end{frame}

\begin{frame}[shrink]{面试 Q\&A（下）}
\textbf{Q5}：cron 1 小时一次是不是太慢？\\
\hspace{4mm}\small A：尾款期一般是 1-7 天，1h 没收延迟业务可接受。缩到 1min 需要独立 cron 进程 + 防扫表压力，对业务\textbf{无明显收益}。SLO 写 1h 是诚实边界。

\vspace{2mm}
\textbf{Q6}：商家凭定金销量备货，转化率 75\% 是怎么算出来的？\\
\hspace{4mm}\small A：是\textbf{历史经验值}（双 11 行业平均 70\%-90\%）。gomall 当前没有真实历史数据，路线图阶段先按 75\% 给商家做"建议下单量"，3-5 期之后用商家自己的历史 \texttt{conversion\_rate} 替换。

\vspace{2mm}
\textbf{Q7}：定金不退是不是侵犯消费者权益？\\
\hspace{4mm}\small A：法律明确支持（《民法典》第 587 条定金罚则）。\textbf{合规要点}：必须在用户支付定金\textbf{前}明确告知，字号不得小于其它条款。gomall 通过 \texttt{ShowPreorder} 返回的字段强制前端拼《预售须知》弹窗。

\vspace{2mm}
\textbf{Q8}：定金已经划给商家了，cron 没收时为什么不再扣商家？\\
\hspace{4mm}\small A：业务上\textbf{没收的"归属"是独立决策}。当前版本只标 \texttt{preorder\_stage=Forfeited} + 发事件，钱包 / 财务下游决定"归商家 / 归平台 / 分账"。这是\textbf{业务争议点}，故意不写死。
\end{frame}

\begin{frame}[shrink]{代码位置一览}
\begin{description}\footnotesize
  \item[预售配置表]
  \item[Order 字段扩展（PreorderStage / DepositPaidAt / FinalPaidAt）]
  \item[DAO（GetPreorder / Mark* / Forfeit / List*）]
  \item[PayDeposit 主入口（窗口 + Reserve + Tx + outbox）]
  \item[PayFinal 主入口（扣尾款 + 真扣库存 + 状态机）]
  \item[CancelInDepositWindow（全退路径）]
  \item[ForfeitDepositsForUnpaidFinals（cron 入口）]
  \item[ShowPreorder（公开查询）]
  \item[HTTP handler 四件套]
  \item[路由注册（统一 :id 通配符）]
  \item[业务码 82001-82004]
  \item[outbox 事件 struct]
  \item[单测覆盖 8 例]
\end{description}
