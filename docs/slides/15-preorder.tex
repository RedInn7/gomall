...
\end{itemize}
\srcnote{internal/preorder/routes.go 路由四件套；internal/preorder/service.go::PayDeposit/PayFinal 主入口}
\end{frame}
...
\begin{itemize}\small
  ...
  \item 选 B 原因：
    ...
    \end{itemize}
\end{itemize}
\srcnote{consts/order.go 订单状态枚举（含拼团 WaitGroup 共 8 态）；internal/order/state.go::orderStateTransitions}
\end{frame}
...
\end{itemize}
\srcnote{internal/preorder/model.go:19--28；internal/order/model.go:23--25 PreorderStage 字段}
\end{frame}
...
\end{lstlisting}
\srcnote{internal/preorder/service.go:78--166 PayDeposit 完整实现；repository/cache/inventory.go::ReserveStock Lua}
\end{frame}
...
\end{center}
\vspace{1mm}
\srcnote{pkg/e/code.go:75 ErrPreorderForfeitedDeposit；pkg/e/msg.go 82004 中文话术}
\end{frame}
...
\end{itemize}
\srcnote{pkg/e/code.go:72--75 82xxx 段；internal/preorder/handler.go::respondPreorderErr 业务码透出}
\end{frame}
...
\end{itemize}
\srcnote{initialize/cron.go:79--89 @every 1h 注册；initialize/cron.go:116--121 runPreorderForfeitSweep；internal/preorder/service.go:348--364 ForfeitDepositsForUnpaidFinals}
\end{frame}
...
\hspace{4mm}\small A：旧消费者只读 \texttt{type}，扩 \texttt{type} 要改一票下游；\texttt{stage} 做正交子状态，对旧消费者透明 — 见 deck 09 17 例状态转换测试零改动。\srcnote{internal/order/model.go:23--25}
...
\begin{description}\footnotesize
  \item[预售配置表]\srcnote{internal/preorder/model.go:19--28}
  ...
  \item[路由注册（统一 :id 通配符）]\srcnote{internal/preorder/routes.go}
  \item[业务码 82001-82004]\srcnote{pkg/e/code.go:72--75; pkg/e/msg.go}
