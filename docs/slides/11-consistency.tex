...
\begin{frame}[shrink]{一句话：一致性是"用户看见的事实对齐"}
\begin{itemize}
...
\begin{frame}[shrink]{业务能容忍的不是"零延迟"，是"看得见的窗口"}
...
\begin{frame}[shrink]{gomall 真正在意的三对一致性}
\begin{tabular}{@{}lll@{}}
\toprule
...
\begin{frame}[shrink]{商家 / 运营 / 客服 / SRE 视角}
\begin{itemize}
...
\begin{frame}[shrink]{反方向也会炸：先 publish 再 commit}
\textbf{反向失败链路}（更糟）：
\begin{enumerate}
...
\begin{frame}[shrink]{为什么 gomall 不能用 2PC / XA}
\begin{itemize}
...
\begin{frame}[shrink]{Outbox 的核心一句话}
...
\begin{frame}[fragile,shrink]{outbox\_event 表：客服查得到的事实流}
...
\begin{frame}[fragile,shrink]{CancelUnpaidOrder：业务变更 + 事件 同事务}
...
\begin{frame}[shrink]{Outbox 怎么消除两个"不对称失败"}
\begin{itemize}
...
\begin{frame}[shrink]{Transactional Outbox 可靠投递时序}
\centering
...
\begin{frame}[shrink]{默认参数：每个数值背后是业务约束}
...
\begin{frame}[fragile,shrink]{publisher.loop / drainBatch：业务流水线}
...
\begin{frame}[shrink]{业务码状态机：pending / sent / dead}
...
\begin{frame}[fragile,shrink]{MarkFailed: 指数退避 + 死信落地}
...
\begin{frame}[shrink]{死信处理：让"失败可见"而不是"自动忽略"}
\begin{itemize}
...
\begin{frame}[shrink]{协同式 Saga：5 步链路 + 任一失败补偿}
\begin{tabular}{@{}lll@{}}
\toprule
模式 & 实现 & 业务取舍 \\
\midrule
...
\begin{frame}[shrink]{Saga 失败：CancelUnpaidOrder 就是一次补偿}
...
\begin{frame}[shrink]{补偿失败的容错：用户感受是什么}
\begin{itemize}
...
\begin{frame}[shrink]{改价生效窗口：三方期望对齐}
...
\begin{frame}[fragile,shrink]{双删 + 击穿保护：源码}
...
\begin{frame}[shrink]{击穿保护：SETNX 单飞回源}
...
\begin{frame}[shrink]{压测里量出来的两组真实数字}
\begin{tabular}{@{}lr@{}}
\toprule
压测场景 & 实测 \\
\midrule
...
\begin{frame}[shrink]{Outbox 解决"丢失"，幂等解决"重复"}
\begin{center}
\resizebox{0.95\textwidth}{!}{%
\begin{tikzpicture}[node distance=8mm and 12mm,font=\scriptsize]
  \node[node-box] (a) {应用};
...
\begin{frame}[fragile,shrink]{困局一：幂等记录卡在 processing 永不可重试}
...
\begin{frame}[shrink]{困局二：毒丸消息无限 requeue 打爆队列}
...
\begin{frame}[shrink]{毒丸消息超阈值进 DLQ 的流转}
\begin{center}
...
\begin{frame}[fragile,shrink]{困局三：消费者断连后静默停摆 + 生产 fail-fast}
...
\begin{frame}[shrink]{事故一：未支付订单取消导致虚高库存}
...
\begin{frame}[shrink]{事故二：搜索索引腐烂 12 小时无人知}
...
\begin{frame}[shrink]{事故三：InitLog 顺序错让全站 503}
...
\begin{frame}[shrink]{修复后的启动顺序（业务因果）}
...
\begin{frame}[shrink]{两条可复用的业务侧教训}
\begin{enumerate}
...
\begin{frame}[shrink]{三档一致性窗口：写在 SLO 里}
...
\begin{frame}[shrink]{业务边界：本一致性方案不做什么}
...
\begin{frame}[shrink]{业务侧总结：一致性约束一张表}
\begin{tabular}{@{}llll@{}}
\toprule
