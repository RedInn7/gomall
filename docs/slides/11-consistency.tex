...
\srcnote{README.md \S 业务承诺（SLO）；internal/shared/outbox/publisher.go:23 PollInterval=1s}
\end{frame}
...
\srcnote{internal/order/service.go / internal/order/cancel.go:14--63 / internal/product/service.go ProductUpdate}
\end{frame}
...
\end{itemize}
\srcnote{internal/shared/outbox/model.go:24--35 outbox\_event 字段就是客服查询面}
\end{frame}
...
\srcnote{internal/order/service.go::CreateOrder / internal/order/cancel.go::CancelUnpaidOrder}
\end{frame}

\begin{frame}[fragile]{outbox\_event 表：客服查得到的事实流}
\begin{lstlisting}[language=Go,firstnumber=24,
  caption={\srcnote{internal/shared/outbox/model.go:24--35}}]
...
\begin{lstlisting}[language=Go,firstnumber=31,
  caption={\srcnote{internal/order/cancel.go:31--51}}]
...
\srcnote{internal/shared/outbox/publisher.go:51--62 主循环；:65--86 单批投递}
\end{frame}
...
\end{itemize}}
\srcnote{internal/shared/outbox/publisher.go:14--32 \texttt{PublisherConfig.withDefaults}；internal/shared/outbox/model.go:14 \texttt{OutboxDefaultMaxAttempts=10}}
\end{frame}

\begin{frame}[fragile]{publisher.loop / drainBatch：业务流水线}
\begin{lstlisting}[language=Go,firstnumber=51,
  caption={\srcnote{internal/shared/outbox/publisher.go:51--86}}]
...
\end{itemize}
\srcnote{internal/shared/outbox/repo.go:62--78 \texttt{MarkFailed} / internal/shared/outbox/model.go:9--14}
\end{frame}

\begin{frame}[fragile]{MarkFailed: 指数退避 + 死信落地}
\begin{lstlisting}[language=Go,firstnumber=62,
  caption={\srcnote{internal/shared/outbox/repo.go:62--78}}]
...
\end{itemize}}
\srcnote{internal/order/cancel.go:14--63 完整闭包}
\end{frame}
...
\end{itemize}
\srcnote{internal/order/cancel.go:55--62 释放 reservation 仅 log，不回滚 DB}
\end{frame}
...
\hspace{4mm}\small A：commit 与 publish 不在一个事务，commit 成功 publish 失败就丢事件 $\to$ 商家收不到发货事件、库存不回。Outbox 把事件写进 DB，由轮询消化，事件丢失等价于 DB 数据丢失。\srcnote{internal/order/cancel.go:31--51}
...
\hspace{4mm}\small A：DB 是业务真相，cache 是性能侧。Cache 失败让库存"短暂少一点"，最坏拒单不超卖；回滚 DB 反而让"已关单"被复活，业务码乱套，客服更难解释。\srcnote{internal/order/cancel.go:55--62}

\vspace{2mm}
\textbf{Q6}：客服来问"用户的退款 24h 没回来"怎么查？\\
\hspace{4mm}\small A：\texttt{SELECT status, attempts, last\_error FROM outbox\_event WHERE aggregate\_type='order' AND aggregate\_id=? ORDER BY id}。status=1 排队中 / 2 已发下游 / 3 dead 需升 SRE。\srcnote{internal/shared/outbox/model.go:24--35}
...
\hspace{4mm}\small A：PollInterval=1s + BatchSize=100 + 单实例 $< 100$ ev/s 峰值 $\to$ 平均延迟 $< 500$ ms，最坏 1s。监控 \texttt{outbox\_publish\_latency\_p99 > 1s} 报警。\srcnote{internal/shared/outbox/publisher.go:21--32}
\end{frame}

\begin{frame}{面试 Q\&A（3/3）}
\textbf{Q9}：MaxAttempts=10 怎么算出来的？退避 5min 封顶又是为什么？\\
\hspace{4mm}\small A：1+2+4+8+16+32+64+128+256+300=811s$\approx$13.5min 累计窗口，覆盖一次"中等故障"够用；封顶 5min 防止退避无限拉长导致积压堆 outbox 表。\srcnote{internal/shared/outbox/repo.go:62--78}
...
\hspace{4mm}\small A：静默降级。RMQ 未起时 publisher 不发事件、不动 DB；主链路（下单 / 支付）继续在线，事件堆 outbox 等 RMQ 恢复后自动消化。\srcnote{internal/shared/outbox/publisher.go:66--68}

\vspace{2mm}
\textbf{Q12}：业务侧怎么承诺"最终一致"的窗口？\\
\hspace{4mm}\small A：分三档：库存预扣 100ms、订单/搜索可见 1s、跨服务对账 5min。对外 SLO 写明上限，超过触发 P2 报警；客服话术按这三档给用户解释。\srcnote{internal/shared/outbox/publisher.go:21--32 与 README SLO 表}
\end{frame}

\begin{frame}{代码位置一览}
\begin{description}
  \item[outbox\_event 表]\srcnote{internal/shared/outbox/model.go:9--37}
  \item[Outbox DAO (Insert / FetchBatch / MarkSent / MarkFailed)]\srcnote{internal/shared/outbox/repo.go:13--86}
  \item[Publisher (Start / loop / drainBatch)]\srcnote{internal/shared/outbox/publisher.go:34--86}
  \item[初始化 publisher]\srcnote{initialize/outbox.go (InitOutboxPublisher)}
  \item[OrderCreate (业务 + outbox 同事务)]\srcnote{internal/order/service.go::CreateOrder}
  \item[CancelUnpaidOrder (Saga 补偿)]\srcnote{internal/order/cancel.go:14--63}
