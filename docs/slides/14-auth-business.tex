...
\vspace{2mm}
\begin{itemize}
  \item 鉴权每个 admin 请求都走——若每次 \texttt{SELECT role FROM user} 则 QPS 翻倍 + 连接池竞争。
  \item 真实场景：用户角色变更频率 < 一年 10 次。"30s 最终一致"换"高 QPS 不打挂 DB"——业务可接受。
  \item \textbf{命中}微秒级 / \textbf{未命中}打一次 DB 回填 / \textbf{显式失效} \texttt{InvalidateRoleCache(userId)}：提权 / 降权动作主动调，客服关心的"立刻生效"仍是秒级。
\end{itemize}
\srcnote{middleware/rbac.go:24 roleCacheTTL=30s；:28--45 lookupRole；:88--91 Invalidate；service/admin.go:59 PromoteToAdmin 后调用}
\end{frame}
...
\end{tabular}
\vspace{3mm}
\small 30001 vs 30002 表面像："鉴权失败"通常是 token 被改了 / 没带；"超时"是过期。30005 客服遇到要警惕：要么用户走错入口，要么有人在试 admin 接口。\\
gomall 所有鉴权失败统一返 \texttt{HTTP 200} + \texttt{\{status: 3000x\}}（老版本 app 网关吞不下 401/403）——所以\textbf{监控告警必须基于业务 status，不是 HTTP code}。
...
\end{tabular}
\vspace{2mm}
\small 鉴权不是"开发自己写"的事，是"几个角色谈出来"的事。开发把边界翻译成 router.go 里几行 \texttt{authed} / \texttt{admin}。
\vspace{2mm}
\textbf{业务侧最终目标}：用户感受不到登录（双 token 续命）/ 客服 1 分钟补券（admin 接口走通）/ 黑产领不到券（L1+L2+L3）/ 内鬼搞不出大损失（审计 + 额度，待补）/ 出事能止血（token 黑名单，待补）。
