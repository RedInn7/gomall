...
\begin{tikzpicture}[node distance=8mm and 14mm]
  ...
  \draw[arrow-bad] (rv.east) -- ++(1.2,0) node[right,font=\tiny,align=center]{支付 commit\\真扣减};
\end{tikzpicture}
\vspace{2mm}
\begin{itemize}
...
\begin{itemize}
  \item Lua 脚本先判 \texttt{avail $<$ need} 再 \texttt{DECRBY}：库存不够直接拒绝，没有"先扣了再说"的中间态。
  \item Redis 单线程 + Lua 原子执行：500 个 goroutine 同时 \texttt{ReserveStock(pid, 1)}，初始库存 100 件，最终成功数\textbf{恰好 100}。
  \item DB 那一桶（\texttt{product.num}）只在\textbf{支付落地}和\textbf{Cron 回滚}两条路径上动。
  \item 业务承诺：\textbf{任何瞬间，已售 + 已锁 $\le$ 总水位}，这条不变量永不破。
\end{itemize}
\srcnote{repository/cache/inventory\_test.go:52--82 TestInventory\_NoOversellUnderConcurrency}
\end{frame}
...
承诺 & 实现 & 复核 \\
\midrule
\textbf{不超卖}      & Redis Lua 原子扣        & 500 并发抢 100 张零超发 \\
\textbf{30min 释放} & MQ TTL 30min            & RMQ 单元测试 + 端到端 \\
\textbf{兜底释放}    & Cron 5min / 15min 窗口  & RMQ 关停后 Cron 接管 \\
\textbf{支付即扣}    & CommitReservation       & service/order.go 单测 \\
\textbf{幂等关单}    & WHERE type=UnPaid       & 50 VU 同 Key 15s = 1 单 \\
\textbf{显式回滚}    & order.go release 分支    & tx 失败注入测试 \\
\bottomrule
\end{tabular}
\vspace{2mm}
\small 六条承诺缺一不可。压测脚本与数据在 \texttt{stressTest/} 下，PR 可复核。\\
/orders/create 单机 50{,}319 RPS、p95 2.33ms ——\textbf{业务承诺的实测证据}。
\srcnote{stressTest/REPORT.md 链路压测表 idempotency\_replay 行}
\end{frame}

\begin{frame}{回到那条 30 分钟的线}
\begin{itemize}
  \item 30 分钟是\textbf{用户友好}与\textbf{库存周转}的平衡，不是 SLA、不是延迟指标——它是\textbf{业务的合同条款}。
  \item 改 30 分钟，要走的不是一个 PR，是一次跨部门决策（商家锁单 / 运营促销 / 风控黑产）。
  \item 业务侧落到代码的总结：
  \begin{itemize}
    \item 两桶库存 = \textbf{可售 / 已锁定}，对应运营两套报表。
    \item 超卖必赔、误锁仅流失，默认偏好\textbf{宁可误锁}。
    \item 一致性靠 Saga + 对账，而非分布式事务。
    \item 六条承诺都\textbf{要压测或单测复核}，否则只是口号。
  \end{itemize}
\end{itemize}
\keypoint{库存治理的核心，不是 Redis Lua 怎么写，是\textbf{这六条承诺怎么落到代码里且能被复核}。}
\end{frame}
