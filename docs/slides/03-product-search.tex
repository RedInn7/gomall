...
\srcnote{service/search/routes.go:9 /product/search 关键词入口；:10 /product/semantic-search hybrid 入口}
\end{frame}
...
\end{itemize}
\srcnote{业务码表 pkg/e/code.go:49--58（50001 / 60002 / 70001 / 70002）}
\end{frame}
...
\end{itemize}
\srcnote{service/search/semantic.go:14--54 hybrid 主入口与默认权重}
\end{frame}
...
\end{itemize}
\srcnote{internal/product/repo.go:94--112 是当前 DB 兜底用的 LIKE 实现}
\end{frame}
...
\srcnote{repository/es/product\_index.go:20--41 mapping 沿用集群默认 BM25；service/search/semantic.go:105--109 是 function\_score / 精排的统一挂载点}
\end{frame}
...
\end{itemize}
\srcnote{internal/product/service.go:153, 251, 287 三处商品写入后调 emitProductChanged}
\end{frame}
...
\end{tabular}
\vspace{1mm}
\srcnote{internal/product/service.go:256--263 emitProductChanged；internal/shared/outbox/publisher.go:19--30 withDefaults PollInterval=1s；service/search/indexer.go:25 Qos=32}
\end{frame}
...
\end{itemize}
\srcnote{internal/product/service.go:256--263 outbox 写入；service/search/indexer.go:32--47 消费 + Nack 重投}
\end{frame}
...
\end{itemize}
\srcnote{internal/product/service.go:256--263 emitProductChanged；service/search/indexer.go:17--49 indexer 主循环}
\end{frame}
...
\end{itemize}
\srcnote{service/search/product\_query.go:15--49 兜底；cmd/main.go:69--78 tryInitES；initialize/search.go:13--31}
\end{frame}
...
\srcnote{service/search/product\_query.go:15--49 ES nil $\to$ DB LIKE；service/search/backfill.go:13--43 全量回填；internal/admin/routes.go:15 admin/search/backfill}
\end{frame}
...
\srcnote{service/search/semantic.go:81--85 ES 失败时 keywordHits=nil；service/search/product\_query.go:15--49 ES 全挂回 LIKE}
\end{frame}
...
\end{itemize}
\srcnote{service/search/semantic.go:19--20 默认权重 weightSemantic=0.5 / weightKeyword=0.5}
\end{frame}
...
\end{itemize}
\srcnote{service/search/semantic.go:14--54 hybrid 入口注释与默认权重}
\end{frame}
...
\srcnote{service/search/semantic.go:56--141 hybrid 主流程；68--85 embedding + 向量 + 关键词三路召回}
\end{frame}
...
\srcnote{service/search/semantic.go:56--141 semanticSearchWith 主流程}
\end{frame}
...
\end{itemize}
\srcnote{service/search/semantic.go:19--20 权重常量；:105--109 Score 处可挂业务 boost}
\end{frame}
...
\end{lstlisting}
\srcnote{service/search/semantic.go:87--109 融合主体（折行示意）}
\end{frame}
...
\end{itemize}
\srcnote{service/search/semantic.go:169--196 minMaxNormalize 全等集合返回 1}
\end{frame}
...
\end{itemize}
\srcnote{service/search/embedding.go:52--113；service/search/milvus\_stub.go:9--47；cmd/main.go:90--102 tryInitMilvus}
\end{frame}
...
\end{itemize}
\srcnote{repository/milvus/product\_vector.go:25--27 hnswM=16 / hnswEfConstruction=200 / hnswEfSearch=64}
\end{frame}
...
\end{itemize}
\srcnote{repository/milvus/product\_vector.go:25--27 三个参数常量}
\end{frame}
...
\end{itemize}
\srcnote{service/search/product\_query.go:15--49 ES nil 走 product.SearchProduct LIKE 兜底；service/search/semantic.go:81--85 ES 失败 keywordHits=nil}
\end{frame}
...
\end{itemize}
\srcnote{internal/product/service.go:256--263 emitProductChanged 写 outbox；internal/shared/outbox/repo.go status 字段 pending/sent/dead；internal/admin/routes.go:15 admin/search/backfill}
\end{frame}
...
\end{itemize}
\srcnote{service/search/indexer.go:51--60 handleProductChanged 处理 delete；repository/es/product\_index.go:134--154 DeleteProduct}
\end{frame}
...
\srcnote{repository/es/product\_index.go:20--41 mapping 无 synonym filter；service/search/semantic.go:105--109 个性化挂载点 TBD}
\end{frame}
...
\end{itemize}
\srcnote{service/search/semantic.go:105--109 融合分赋值处是个性化加权的唯一注入点}
\end{frame}
...
\end{tabular}
\vspace{2mm}
\srcnote{service/search/routes.go:9--10 search + semantic-search 两个入口；internal/admin/routes.go:15 admin/search/backfill}
\end{frame}
...
\hspace{4mm}\small A：DB 与 ES 不同事务，同步写要么失败回滚（伤体验），要么不一致（伤召回）。outbox 把"DB 写成功" 和 "ES 写成功" 两件事拆开，分别保证。代码：\srcnote{internal/product/service.go:256--263}

\vspace{2mm}
\textbf{Q2}：ES 挂掉，搜索接口为什么不直接 5xx？\\
\hspace{4mm}\small A：搜索是入口，5xx 等于 app 崩页。LIKE 慢一点也比白屏强 —— 业务承诺"搜索框永远可用"。代码：\srcnote{service/search/product\_query.go:15--49}

\vspace{2mm}
\textbf{Q3}：hybrid 50/50 这个权重你怎么调？\\
\hspace{4mm}\small A：业务值，不是技术值。先 50/50 跑一轮，A/B 观察 CTR / 转化。短 query 偏关键词，长 query 偏语义。代码：\srcnote{service/search/semantic.go:19--20}
...
\hspace{4mm}\small A：所有分数等值时 span=0。代码里返回常量 1，意味着"两路一样可信"，让另一路决定排序。代码：\srcnote{service/search/semantic.go:184--190}
...
\hspace{4mm}\small A：nopMilvusSearcher 返回空，hybrid 退化为纯 ES。关键词召回不受影响，只丢自然语言长尾。代码：\srcnote{service/search/milvus\_stub.go:20--24}
...
\hspace{4mm}\small A：ES mapping 字段类型不能原地改，必须 \texttt{reindex} 或新建索引 + alias 切换。回填走 admin/search/backfill。\srcnote{internal/admin/routes.go:15}
\end{frame}

\begin{frame}{代码位置总览 + 配套材料}
\footnotesize
\begin{description}
  \item[搜索 / 语义入口] \srcnote{service/search/routes.go:9--10}
  \item[ES 检索 + mapping] \srcnote{repository/es/product\_index.go:20--41, 163--227}
  \item[outbox 事件] \srcnote{internal/product/service.go:256--263}
  \item[indexer 消费] \srcnote{service/search/indexer.go:17--49}
  \item[ES 兜底分支] \srcnote{service/search/product\_query.go:15--49}
  ...
  \item[Milvus HNSW] \srcnote{repository/milvus/product\_vector.go:25--27, 80--96, 141--162}
  \item[Milvus 接口抽象] \srcnote{service/search/milvus\_stub.go:9--47}
  \item[启动静默] \srcnote{cmd/main.go:69--78 + initialize/search.go:13--31}
  \item[业务码表] \srcnote{pkg/e/code.go:49--58}
\end{description}
\vspace{2mm}
