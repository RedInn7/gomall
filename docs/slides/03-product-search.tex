...
\begin{frame}{先回答一个业务问题：为什么搜索值得单独立项}
\begin{itemize}
...
\srcnote{routes/router.go:41 /product/search 关键词入口；:42 /product/semantic-search hybrid 入口}
\end{frame}

\begin{frame}{五角色对"搜索"四件事的关心}
\begin{tabular}{@{}lll@{}}
\toprule
...
\end{itemize}
\srcnote{业务码表 pkg/e/code.go:46--58（50001 / 60002 / 70001 / 70002）}
\end{frame}

\begin{frame}{用户漏斗：每一跳的业务意义}
...
\end{itemize}
\srcnote{stressTest/REPORT.md：/product/show 同库 PK 查询 62{,}226 RPS / p95 3ms 为时延锚点}
\end{frame}

\begin{frame}{搜不到 = GMV 静默漏走（定量算账）}
...
\end{itemize}
\srcnote{service/search/semantic.go:1--56 hybrid 主入口与默认权重}
\end{frame}
...
\end{itemize}
\srcnote{internal/product/repo.go:80--98 是当前 DB 兜底用的 LIKE 实现}
\end{frame}
...
\end{itemize}
\srcnote{stressTest/REPORT.md：product 956K 行 / 364MB；/product/list 24.5 RPS / 2.50s 反例；/product/show 62{,}226 RPS / 3ms 锚点}
\end{frame}
...
\begin{frame}{ES 替 LIKE 做什么（业务视角）}
...
\begin{frame}{代码段 1 逐行讲解（业务对照）}
\begin{itemize}
  \item \textbf{L170--175}：\texttt{multi\_match} 同时查 name / title / info，靠 \texttt{\^{}N} 给字段加权 —— 这就是运营"今日特卖 title 加权"的代码落点。
  \item \textbf{L176--180}：\texttt{filter} 走 \texttt{term}，类目过滤\textbf{不参与打分}，只做 yes/no 过滤。业务上对应"用户在 3C 类目里搜，绝不让美妆混进来"。
  \item \textbf{L182--189}：\texttt{bool.must + filter} 是 ES 标准组合 —— must 决定\textbf{相关性}，filter 决定\textbf{可见性}。两者分离，分页 + 打分互不干扰。
  \item \textbf{运营能改 / 不能改}：fields 数组里的 \texttt{\^{}N} 数字是运营旋钮，filter 数组是开发硬约束。这个边界必须写进运营手册。
\end{itemize}
\end{frame}

\begin{frame}{must / should / filter / must\_not 四件套（业务怎么用）}
\begin{tabular}{@{}lll@{}}
\toprule
子句         & 业务用途                        & gomall 现状     \\
\midrule
...
\end{itemize}
\srcnote{repository/es/product\_index.go:170--191 当前实现 must + filter；should / must\_not 路线图}
\end{frame}

\begin{frame}{BM25 + function\_score：把相关性与业务分挂在一起}
\begin{itemize}
...
\srcnote{repository/es/product\_index.go:20--41 mapping 沿用集群默认 BM25；service/search/semantic.go:107--111 是 function\_score / 精排的统一挂载点}
\end{frame}
...
\end{itemize}
\srcnote{internal/product/service.go:155, 253, 289 三处商品写入后调 emitProductChanged}
\end{frame}

\begin{frame}{5min SLA 的时间预算拆解}
\begin{tabular}{@{}lrl@{}}
\toprule
环节                                   & 时间预算    & 商家体验      \\
\midrule
...
\end{tabular}
\vspace{1mm}
\srcnote{internal/product/service.go:257--265 emitProductChanged；internal/shared/outbox/publisher.go:21--32 PollInterval=1s 默认；service/search/indexer.go:25 Qos=32}
\end{frame}

\begin{frame}{每步如果挂掉，商家看到什么 / 客服怎么答}
\begin{tabular}{@{}lll@{}}
\toprule
挂的环节       & 商家体感                        & 客服应答模板                    \\
\midrule
...
\end{itemize}
\srcnote{internal/product/service.go:257--265 outbox 写入；service/search/indexer.go:32--46 消费 + Nack 重投}
\end{frame}
...
\end{itemize}
\srcnote{internal/product/service.go:257--265 emitProductChanged；service/search/indexer.go:14--49 indexer 主循环}
\end{frame}
...
\begin{frame}{代码段 2 逐行讲解（业务对照）}
\begin{itemize}
  \item \textbf{L18--19}：\texttt{BindDomainQueue} 把 \texttt{product.changed} 路由绑到 \texttt{search.product.indexer} 队列。上架 / 改价 / 下架\textbf{共用一条路由}，业务上一致。
  \item \textbf{L22}：\texttt{Qos(32, 0, false)} = 给 ES 写入设的\textbf{流量阀}。大商家批量上架 1 万个 SKU 时不会冲垮 ES。
  \item \textbf{L30--34}：脏消息 $\to$ \texttt{Nack(false, false)} 进死信，\textbf{不阻塞健康消息}。运营每天扫一次死信队列。
  \item \textbf{L35--38}：业务处理失败 $\to$ \texttt{Nack(false, true)} 重投，对应"ES 短抖动 1s 自愈"。
  \item \textbf{L39}：成功才 ack。索引是\textbf{幂等}的（同 docID 覆盖），重复消费无害 —— 这是 at-least-once 能成立的前提。
\end{itemize}
\srcnote{service/search/indexer.go:25 Qos=32；:37/:42 Nack 语义；repository/es/product\_index.go:115--120 DocumentID = product.ID 决定幂等}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{on\_sale 是 \texttt{boolean}}：\textbf{下架商品要从结果里隐藏} —— 客诉里"为什么搜到下架商品"基本都是这里没维护好。
...
\end{itemize}
\srcnote{service/search/product\_query.go:294--323 兜底；cmd/main.go:64--73 tryInitES；initialize/search.go:13--31}
\end{frame}

\begin{frame}{ES 全挂的运营 SOP（5 步）}
\begin{enumerate}
...
\srcnote{service/search/product\_query.go:294--325 ES nil $\to$ DB LIKE；service/search/backfill.go:13--43 全量回填；routes/router.go:163 admin/search/backfill}
\end{frame}
...
\srcnote{service/search/semantic.go:84--87 ES 失败时 keywordHits=nil；service/search/product\_query.go:294--323 ES 全挂回 LIKE}
\end{frame}
...
\begin{frame}{纯 ES vs 纯向量：业务漏斗对照}
\begin{tabular}{@{}lll@{}}
\toprule
策略     & 优点                & 业务漏斗损失                  \\
\midrule
...
\end{itemize}
\srcnote{service/search/semantic.go:17--23 默认权重 weightSemantic=0.5 / weightKeyword=0.5}
\end{frame}
...
\srcnote{service/search/semantic.go:54--143 hybrid 主流程；70--87 embedding + 向量 + 关键词三路召回}
\end{frame}
...
\begin{frame}{代码段 4 逐行讲解（业务对照）}
\begin{itemize}
  \item \textbf{L89--90}：两路分数各自 min-max 归一到 \([0,1]\)，避免 BM25 的 0--100 量纲压死向量的 0--1 量纲 —— 业务上对应"两边权重\textbf{真正平等}才能调"。
  \item \textbf{L93--99}：两路按 \texttt{product\_id} 取并集 —— \textbf{仅一路出现的 ID 也计入}（另一路补 0）。业务上对应"向量召回到、ES 没召回到的长尾商品也有机会出现"。
  \item \textbf{L100--103}：加权融合。\texttt{weightSemantic / weightKeyword} 是常量，\textbf{A/B 时换 hash 分桶决定} —— 运营调权的工程出口。
...
\end{itemize}
\srcnote{service/search/product\_query.go:294--323 ES nil 走 product.SearchProduct LIKE 兜底；service/search/semantic.go:84--87 ES 失败 keywordHits=nil}
\end{frame}

\begin{frame}{客诉案例 2：商家"我上架了为什么搜不到" + 客服话术}
...
\end{itemize}
\srcnote{internal/product/service.go:257--265 emitProductChanged 写 outbox；internal/shared/outbox/repo.go status 字段 pending/sent/dead；routes/router.go:163 admin/search/backfill}
\end{frame}

\begin{frame}{客诉案例 3：用户搜到下架商品 + 客服话术}
...
\end{itemize}
\srcnote{service/search/indexer.go:51--60 handleProductChanged 处理 delete；repository/es/product\_index.go:134--150 DeleteProduct}
\end{frame}
...
\srcnote{repository/es/product\_index.go:20--41 mapping 无 synonym filter；service/search/semantic.go:107--111 个性化挂载点 TBD}
\end{frame}

\begin{frame}{个性化排序路线图：三类特征 + 挂载点}
\begin{tabular}{@{}lll@{}}
\toprule
特征类      & 例子                          & 来源                  \\
\midrule
...
DB 兜底     & ES 客户端为 nil $\to$ LIKE        & service/search/product\_query.go:294 \\
...
\begin{itemize}
  ...
  \item 反例 \texttt{/product/list} 全表 count 实测 \textbf{24.5 RPS / p95 2.50s}，验证"搬到 ES"是必选项。
\end{itemize}
\vspace{1mm}
...
\hspace{4mm}\small A：DB 与 ES 不同事务，同步写要么失败回滚（伤体验），要么不一致（伤召回）。outbox 把"DB 写成功" 和 "ES 写成功" 两件事拆开，分别保证。代码：\srcnote{internal/product/service.go:257--265}

\vspace{2mm}
\textbf{Q2}：ES 挂掉，搜索接口为什么不直接 5xx？\\
\hspace{4mm}\small A：搜索是入口，5xx 等于 app 崩页。LIKE 慢一点也比白屏强 —— 业务承诺"搜索框永远可用"。代码：\srcnote{service/search/product\_query.go:294--323}
...
\hspace{4mm}\small A：四步走：查 product 表确认入库 $\to$ 查 outbox 行 status $\to$ 查 RMQ 队列深度 $\to$ 查 ES doc。每步对应一个客服话术（见客诉案例 2）。代码：\srcnote{internal/shared/outbox/repo.go status 字段}
\end{frame}
...
\hspace{4mm}\small A：三种：DB 改 \texttt{on\_sale} 但 outbox 漏写、outbox 写了但 indexer 没消费、indexer 调 DeleteProduct 但 ES 拒收。长期方案：返回前再过一次 DB 校验。代码：\srcnote{service/search/indexer.go:51--60}
\end{frame}
...
\hspace{4mm}\small A：业务边界。当前流量没到拐点，同义词靠 hybrid 向量召回兜，搜索历史涉及用户隐私单独立项，trending 词运营手动维护轮播。详见"业务边界"帧。\srcnote{service/search/semantic.go hybrid 主入口}
...
\begin{description}
  ...
  \item[outbox 事件] \srcnote{internal/product/service.go:257--265}
  \item[indexer 消费] \srcnote{service/search/indexer.go:17--49}
  \item[ES 兜底分支] \srcnote{service/search/product\_query.go:294--323}
  ...
  \item[业务码表] \srcnote{pkg/e/code.go:46--58}
\end{description}
\vspace{2mm}
