...
\end{tabular}
\vspace{1mm}
\srcnote{service/product.go:257--265 emitProductChanged；service/outbox/publisher.go:21--32 PollInterval=1s 默认；service/search/indexer.go:25 Qos=32}
\end{frame}
...
\end{itemize}
\srcnote{service/product.go:257--265 outbox 写入；service/search/indexer.go:32--46 消费 + Nack 重投}
\end{frame}
...
\srcnote{service/product.go:294--325 ES nil $\to$ DB LIKE；service/search/backfill.go:13--43 全量回填；routes/router.go:163 admin/search/backfill}
\end{frame}
...
\end{itemize}
\srcnote{service/product.go:294--323 ES nil 走 dao.SearchProduct LIKE 兜底；service/search/semantic.go:84--87 ES 失败 keywordHits=nil}
\end{frame}
...
\end{itemize}
\srcnote{service/product.go:257--265 emitProductChanged 写 outbox；repository/db/dao/outbox.go status 字段 pending/sent/dead；routes/router.go:163 admin/search/backfill}
\end{frame}
...
\hspace{4mm}\small A：四步走：查 product 表确认入库 $\to$ 查 outbox 行 status $\to$ 查 RMQ 队列深度 $\to$ 查 ES doc。每步对应一个客服话术（见客诉案例 2）。代码：\srcnote{repository/db/dao/outbox.go status 字段}
\end{frame}
