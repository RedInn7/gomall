...
\begin{frame}[shrink]{本节角色视角：C 端用户 + SRE + 客服}
\begin{itemize}
...
\begin{frame}[shrink]{24h / 10d 这两个数字的业务依据}
\begin{tabular}{@{}lll@{}}
\toprule
参数 & 取值 & 业务依据 \\
\midrule
...
\begin{frame}[shrink]{逐行讲解 1：AuthMiddleware}
\begin{itemize}
...
\begin{frame}[shrink]{双 token 不查黑名单 + 鉴权链路开销}
\begin{tabular}{@{}lrr@{}}
\toprule
场景 & RPS & p95 \\
\midrule
...
\begin{frame}[shrink]{本节角色视角：运营 + 客服 + SRE}
\begin{itemize}
...
\begin{frame}[shrink]{逐行讲解 2：lookupRole}
\begin{itemize}
...
\begin{frame}[shrink]{本节角色视角：SRE + 运营 + 商家（路线图）}
\begin{itemize}
...
\begin{frame}[shrink]{逐行讲解 3：BootstrapPromoteSelf}
\begin{itemize}
...
\begin{frame}[shrink]{admin bootstrap "占用"的运维 SOP（客服 + SRE 协同）}
\footnotesize
...
\begin{frame}[shrink]{merchant group 的路线图：路由 + DAO 双层}
\begin{center}
...
\begin{frame}[shrink]{本节角色视角：客服 + 运营（主角）}
\begin{itemize}
...
\begin{frame}[shrink]{业务码 + 客服话术（鉴权三种姿势）}
\begin{tabular}{@{}lll@{}}
\toprule
码 & 含义 & 客服话术 \\
\midrule
...
\begin{frame}[shrink]{逐行讲解 4：preflight + 失败计数器}
\begin{itemize}
...
\begin{frame}[shrink]{本节角色视角：客服 + 风控 + 商家（路线图）}
\begin{itemize}
...
\begin{frame}[shrink]{refresh 被偷的恢复路径}
\begin{itemize}
...
\begin{frame}[shrink]{第三方登录链路：把 OAuth 收敛回 gomall 双 token}
\begin{tikzpicture}[node distance=6mm and 10mm]
...
\begin{frame}[shrink]{bcrypt cost 调优：为什么是 12 不是 10 也不是 14}
\begin{tabular}{@{}lrrr@{}}
\toprule
...
\begin{frame}[fragile,shrink]{代码片段 5：cost=12 与未来无痛升级路径}
...
\begin{frame}[shrink]{多端同账号：app / web / 小程序 token 隔离}
\begin{itemize}
...
\begin{frame}[fragile,shrink]{用户域其它能力：关注关系的脏边治理}
...
\begin{frame}[shrink]{鉴权链路涉及的全部业务码 + 客服对照}
\begin{tabular}{@{}lll@{}}
\toprule
码 & 含义 & 客服话术 \\
\midrule
...
\begin{frame}[shrink]{鉴权链路容量规划：1k / 10k / 100k QPS 怎么撑}
\begin{tabular}{@{}lrrr@{}}
\toprule
