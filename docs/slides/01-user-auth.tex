...
\end{itemize}
\srcnote{stressTest/REPORT.md 链路压测表 行 1, 3；middleware/jwt.go:32--57 当前无黑名单}
\end{frame}
...
\end{itemize}
\srcnote{internal/admin/service.go:71--77 count 校验；internal/admin/routes.go:10}
\end{frame}
...
\end{itemize}
\srcnote{internal/admin/service.go:71--77 count 校验；deck 12 admin\_audit 路线图}
\end{frame}
...
\end{itemize}
\srcnote{routes/router.go:52--53 authed；internal/product/routes.go:20--22 product/create 待迁；internal/admin/routes.go:13--15 admin}
\end{frame}
...
\srcnote{internal/user/model.go:28 Role 字段已就绪；internal/product/routes.go:20--22 待迁移}
\end{frame}
...
\end{itemize}
\srcnote{internal/admin/routes.go:13--15 admin 子组当前只有 3 个接口；deck 10 SlidingWindow 防自助补券黑产}
\end{frame}
...
\end{itemize}
\srcnote{internal/user/service.go:82--118 login 当前无失败限频；pkg/e/code.go:10003 vs 10005 错误码差异是枚举漏洞}
\end{frame}
...
\end{itemize}
\srcnote{repository/cache/common.go:14 cache.RedisClient 已就绪；deck 10 SlidingWindow 提供窗口算法}
\end{frame}
...
\end{itemize}
\srcnote{internal/user/model.go:33 PassWordCost=12；:40--53 SetPassword/CheckPassword}
\end{frame}

\begin{frame}[fragile]{代码片段 5：cost=12 与未来无痛升级路径}
\begin{lstlisting}[language=Go,firstnumber=33]
...
\end{lstlisting}
\srcnote{internal/user/model.go:33--53 + 升级钩子}
\end{frame}
...
\end{itemize}
\srcnote{pkg/utils/jwt/jwt.go:18--22 Claims 待加 ClientType；internal/user/service.go:82--118 login 待区分入口}
\end{frame}


\begin{frame}[fragile]{用户域其它能力：关注关系的脏边治理}
\textbf{业务困局}：用户 A 关注一个不存在的 user id（前端传错 / 接口被刷），旧代码\textbf{吞掉"查目标用户"的错误}，对主键为 0 的零值 \texttt{User\{\}} 直接写关系表 \(\to\) 关系图里多出一条指向 id=0 的\textbf{幽灵关注边}：展开关注列表出现空白用户、计数虚高、后续 join 拿到脏数据。
\begin{lstlisting}[language=Go,firstnumber=67,
  caption={\srcnote{internal/user/repo.go:67--84 loadRelationUsers}}]
func (d *UserDao) loadRelationUsers(uId, followerId uint) (u, f *User, err error) {
    ...
    if err = d.DB.Model(&User{}).Where(`id = ?`, uId).First(&u).Error; err != nil {
        if errors.Is(err, gorm.ErrRecordNotFound) {
            return nil, nil, ErrUserNotExist   // 目标不存在即拒绝，不写关系表
        }
        log.LogrusObj.Error(err); return nil, nil, err
    }
    // followerId 同样先校验存在性，任一端缺失都不落库
    ...
}
\end{lstlisting}
\begin{itemize}\footnotesize
  \item 修法：关注 / 取关前\textbf{先校验两端用户查询的 error}，命中 \texttt{ErrRecordNotFound} 即返回 \texttt{ErrUserNotExist}，不再对零值 \texttt{User} 执行 \texttt{Association.Append}。
  \item \texttt{FollowUser} / \texttt{UnFollowUser} 收口到同一加载函数，校验逻辑只写一处，关系写入永远基于已存在的真实用户。
\end{itemize}
\srcnote{internal/user/repo.go:30--46 FollowUser；:49--63 UnFollowUser；:67--84 loadRelationUsers；internal/user/service.go:326--346 UserFollow/UserUnFollow}
\end{frame}
