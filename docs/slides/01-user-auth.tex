...
... // 解析成功后还有一道 token_version 撤销门（见第五节）
...
B. 用户 token 版本号    & users.token\_version，签进 claims & 每请求查 DB / 本地缓存 \\
...
\begin{itemize}
  ...
  \item gomall \textbf{已落地 B+C 合并}：版本号 \texttt{++} 即生效，本地 sync.Map 命中 0ms——改密码"立即生效"从路线图变成现状。
\end{itemize}
\end{frame}

\begin{frame}[fragile,shrink]{代码片段：B+C 落地——签发透传 / 撤销门 / 原子 bump}
\begin{lstlisting}[language=Go]
type Claims struct {                      // pkg/utils/jwt/jwt.go
    ID           uint   `json:"id"`
    Username     string `json:"username"`
    TokenVersion uint   `json:"token_version"` // 与 users.token_version 比对
    jwt.RegisteredClaims
}
// 续签透传旧版本号，不"洗白"：被 bump 的 token 续签出的新 token 照样被拒
return GenerateToken(accessClaim.ID, accessClaim.Username, accessClaim.TokenVersion)

// middleware/jwt.go —— AuthMiddleware 解析成功后的撤销门（fail-closed）
curVer, err := currentTokenVersion(ctx, claims.ID)   // sync.Map 命中 0 DB
if err != nil || claims.TokenVersion != curVer { /* 拒绝，重新登录 */ }

// internal/user/repo.go —— 版本号只允许这一条路径写入
...
\end{lstlisting}
\begin{itemize}\scriptsize
  \item 中间件里\textbf{续签发生在版本门之前}——若续签时取当前版本号，旧 token 一续签就洗白复活。续签只延长寿命、不重建信任，重建信任只能走登录。
  \item 查询走\textbf{组合根注入 + sync.Map 缓存 + 主动失效}——与 RBAC \texttt{lookupRole} 同一套三件套；撤销门是安全边界，故 \textbf{fail-closed}（对比 IP 限频的 fail-open）。
  \item 存量平滑：旧 token 无该字段解码为 0 == 存量行默认值 0，放行；首次改密码才强制重登。
  \item 审查抓到的隐藏漏洞：\texttt{UpdateUserById} 整行回写会把旧版本号写回、\textbf{回滚撤销}（慢速头像上传与改密码并发即复活被盗 token）——修复一行 \texttt{Omit("token\_version")}。教训：\textbf{撤销字段必须单写路径}。
\end{itemize}
\end{frame}

\begin{frame}[shrink]{客服强制下线时序（接口路线图，机制已就位）}
\begin{tikzpicture}[node distance=7mm and 14mm]
  \node[node-box] (cs)    {客服后台};
  \node[node-box,right=of cs] (api)   {/admin/users/\\force\_logout};
  \node[node-box,right=of api,node-warn] (db)   {users.token\_version\\++};
  \node[node-box,below=of db] (inval) {InvalidateToken-\\VersionCache};
...
\vspace{2mm}
\begin{itemize}
  \item 机制已就位：\texttt{BumpTokenVersion} + \texttt{InvalidateTokenVersionCache} 就是改密码链路在用的两个函数，接口只差一行调用 + admin 墙（路线图）。
  \item 失效本地缓存：与 RBAC \texttt{InvalidateRoleCache} 同款模式，按 userId 删 sync.Map 槽。
...
失效粒度    & 删 session $O(1)$   & 黑名单 / token\_version \\
...
\begin{itemize}
  ...
  \item 代价：失效慢——\texttt{token\_version} 版本号兜底已落地（用户粒度）；token 粒度黑名单留给"只踢一端"。
...
\hspace{3mm}A：改密码已根治——Claims 带 \texttt{token\_version}，bump 后全端立即失效。登出靠客户端丢 token；要"登出即全端失效"复用同一个 bump。
...
\hspace{3mm}A：机制已落地——\texttt{users.token\_version} 签进 claims，改密码 \texttt{++} 全端作废；强制下线接口复用 \texttt{BumpTokenVersion}（路线图）。多端按 \texttt{client\_type} 分桶（路线图）。

\vspace{1mm}
\textbf{Q10}：为什么 gomall 选 JWT 不选 session？\\
\hspace{3mm}A：零 RTT + 无状态扩展 + 多端 header 无跨域。代价是失效慢，\texttt{token\_version} 兜底。银行业务选 session 更合理。
