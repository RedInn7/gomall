...
\begin{frame}{gomall 的三层墙：公开 / authed / admin}
...
\begin{itemize}
  ...
  \item 客户端约定：续期 token 写 header 后，C 端 SDK 需\textbf{用最新 header 覆盖本地存储}，否则下一次仍带老 token。
  \item 设计取舍：当前每次请求都续期，逻辑简单、access 永远满时窗；若要省签名开销，可演进为"剩余 < 1h 才续"，HS256 纯 CPU 开销本身极低（解析 \textless 1ms）。
\end{itemize}
...
\begin{itemize}
  ...
  \item 并发边界：bootstrap 设计为\textbf{首次部署一次性接口}，部署窗口只有运维一人执行，单线程调用语义下 count 自检即可自锁；若要在多人同发场景下也保证强互斥，加 \texttt{SELECT ... FOR UPDATE} 或表级 advisory lock 即可，无需改动主流程。
\end{itemize}
\end{frame}
