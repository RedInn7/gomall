...
\end{center}
\vspace{2mm}
\srcnote{routes/router.go:34--35 路由；internal/user/service.go:38--82 注册；:85--121 登录；internal/user/model.go:28 PassWordCost=12}
\end{frame}
...
\end{itemize}
\srcnote{internal/user/model.go:34--48 SetPassword / CheckPassword；internal/user/service.go:56 注册调用；pkg/utils/jwt/jwt.go:95--134 EmailClaims}
\end{frame}
...
\end{itemize}
\srcnote{internal/user/model.go:28 PassWordCost=12}
\end{frame}
...
\end{itemize}
\srcnote{middleware/rbac.go:88--91 InvalidateRoleCache；internal/admin/service.go:47--62 PromoteToAdmin 写完即 Invalidate}
\end{frame}
...
\srcnote{routes/router.go:146 admin/bootstrap 仅挂 authed，无 RequireRole；internal/admin/service.go:66--80 BootstrapPromoteSelf}
\end{frame}
...
\end{itemize}
\srcnote{internal/admin/service.go:72--78 count 校验；routes/router.go:146}
\end{frame}

\begin{frame}[fragile]{代码片段 3：BootstrapPromoteSelf 一次性校验}
\begin{lstlisting}[language=Go,firstnumber=66,
  caption={\srcnote{internal/admin/service.go:66--80 BootstrapPromoteSelf}}]
...
\end{itemize}
\srcnote{internal/admin/service.go:72--78 count 校验；deck 12 admin\_audit 路线图}
\end{frame}
...
\srcnote{internal/user/model.go:23 Role 字段已就绪；routes/router.go:61--63 待迁移}
\end{frame}
...
\srcnote{middleware/jwt.go：待加黑名单；internal/admin/service.go：待加 advisory lock；routes/router.go：待加 merchant group}
\end{frame}
...
\end{itemize}
\srcnote{internal/user/service.go:85--121 login 当前无失败限频；pkg/e/code.go:10003 vs 10005 错误码差异是枚举漏洞}
\end{frame}
...
\end{itemize}
\srcnote{repository/cache/ratelimit/sliding\_window.go 可直接复用；internal/user/service.go:85 是接入点}
\end{frame}
...
\end{itemize}
\srcnote{internal/user/model.go:14 待加 TokenVer；pkg/utils/jwt/jwt.go:18--22 Claims 加字段}
\end{frame}
...
\end{itemize}
\srcnote{internal/admin/service.go:47--62 PromoteToAdmin 已是模板；路线图：internal/admin/service.go 加 ForceLogout(userId)}
\end{frame}
...
\end{itemize}
\srcnote{路线图：internal/user/model\_oauth.go；internal/user/oauth.go；router.go 加 /oauth/callback}
\end{frame}
...
\end{itemize}
\srcnote{pkg/utils/jwt/jwt.go:25--52 GenerateToken 复用；路线图：internal/user/oauth.go BindOrCreate}
\end{frame}
...
\end{itemize}
\srcnote{internal/user/model.go:28 PassWordCost=12；:34--48 SetPassword/CheckPassword}
\end{frame}
...
\end{lstlisting}
\srcnote{internal/user/model.go:28--48 + 升级钩子}
\end{frame}
...
\end{itemize}
\srcnote{pkg/utils/jwt/jwt.go:18--22 Claims 待加 ClientType；internal/user/service.go:85--121 login 待区分入口}
\end{frame}
...
\end{itemize}
\srcnote{routes/router.go:148--155 admin 子组待加 AuditMiddleware；internal/admin/service.go 所有写操作待加审计}
\end{frame}
...
\begin{description}
  ...
  \item[admin bootstrap / promote]\srcnote{internal/admin/service.go:47--62, 66--80}
  \item[bcrypt cost=12 / Role]\srcnote{internal/user/model.go:23, 28, 34--48}
