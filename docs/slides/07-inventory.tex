...
\end{itemize}
\srcnote{repository/cache/inventory.go:11--14；service/order\_task.go:23 现行口径 15 分钟}
\end{frame}
...
\vspace{2mm}
\small 业务对齐三原则：\\
1. C 端\textbf{只看 available} —— 商详页绝不显示 reserved（会让用户"焦虑性下单"）。\\
...
\textbf{诚实标注}：当前代码里 \texttt{ErrStockInsufficient} 是 Go 错误对象，\textbf{未映射到独立业务码}（不是 50001，50001 是 OSS 错误），上层 handler 返回的是通用 ERROR；建议路线图加 \texttt{ErrStockInsufficient = 80001}。
...
\quad - Cron 兜底扫表 \texttt{GetTimeoutOrders(15, 100)} 视 15min 为超时 \srcnote{service/order\_task.go:23}\\
...
\begin{lstlisting}[language=Go,firstnumber=31,
  caption={\srcnote{service/order\_cancel.go:31--62 关单 + 库存退回}}]
...
\end{lstlisting}
\small \textbf{closed 标志位是幂等的关键}：\texttt{CloseOrderWithCheck} 只对 UnPaid 生效，二次调用 ok=false 整段 no-op，库存\textbf{不会退第二次}。客服 / RMQ / Cron / 用户主动取消 4 个调用口，统一从这个入口走。
\end{frame}
...
\srcnote{repository/rabbitmq/order\_delay.go:22 OrderCancelDelay；service/order\_task.go:23 Cron 15min}
\end{frame}
...
\end{itemize}
\srcnote{repository/cache/inventory.go:127--138 GetStockSnapshot；service/order\_cancel.go:59 release 调用}
\end{frame}
...
\textbf{业务真相}：用户在支付后退款，库存应当从 committed 回到 available 或进入 defective，但当前 gomall \textbf{没有售后链路}（README 自爆 / 路线图阶段）。
\vspace{1mm}
\begin{itemize}
  ...
  \item \textbf{路线图}：\\
  \quad - 退款 outbox 事件 $\to$ 触发 \texttt{ReleaseFromCommitted} Lua（commit 的逆操作）。\\
  \quad - 退货寄回 / 残次品分流 $\to$ WMS 仓储系统对接。\\
  \quad - 售后工单 → 评价 → 二次售后的全链路在 deck 09 路线图章节。
\end{itemize}
\srcnote{当前未实现售后链路；扩展点：service/refund.go + ReleaseFromCommitted Lua}
\end{frame}
...
\end{tabular}
\vspace{2mm}
\small \textbf{诚实标注}：50001 在本仓库是 \texttt{ErrorOss}（对象存储错误），\textbf{不是}库存不足。库存不足在代码里只是 \texttt{cache.ErrStockInsufficient} Go 错误对象，handler 返回的是通用 ERROR。\\
\textbf{路线图}：建议给库存独立业务码 \texttt{ErrStockInsufficient = 80001} + \texttt{ErrStockNotInit = 80002}，客服 / 监控 / 链路追踪都能精准区分。
...
\begin{tabular}{@{}lp{4.6cm}p{2.8cm}@{}}
\toprule
时间窗 & 动作 & 责任方 \\
\midrule
...
\end{tabular}
\vspace{2mm}
\small 关键指标：\textbf{T+5 min 内能锁单}是底线。商详页"下单入口"必须是动态开关，不能等发版（动态开关本身也是路线图）。\\
\textbf{补偿档位}：< 24h = 退款 + 10 元券；24--72h = 退款 + 30 元券 + 致歉短信；> 72h = 退款 + 50 元券 + 同档替代 + 1v1 客服。
...
前两项来自 \texttt{GetStockSnapshot}，committed = DB \texttt{SUM(num) WHERE type=Paid}。
\vspace{1mm}
...
\begin{itemize}
  ...
  \item \textbf{不做预购 / 期货 / 排队购}：所有库存必须实物到仓才上架。
  ...
  \item \textbf{不做防黄牛单用户上限}：reserveScript 暂未加 owned $<$ perUser check（coupon.go:30--44 已实现该范式可复用）。
\end{itemize}
...
能力 & 业务价值 & 优先级 \\
\midrule
独立库存业务码 80001/80002 & 客服 / 监控 / 链路精准分类 & P0 \\
...
\srcnote{coupon.go:30--44 已实现的 owned check 可复用到 reserveScript}
\end{frame}
...
\begin{frame}{Q\&A（下）：业务边界 / 路线图 / 诚实标注}
\textbf{Q5}：50001 是不是缺货码？\\
\hspace{4mm}\small A：\textbf{不是}。50001 在本仓库是 \texttt{ErrorOss}（对象存储错误）。库存不足现在只是 \texttt{cache.ErrStockInsufficient} Go 错误对象，未映射独立业务码。路线图：升 80001。代码：\texttt{pkg/e/code.go} + \texttt{repository/cache/inventory.go:25}。

\vspace{1mm}
\textbf{Q6}：当前关单超时是 15 还是 30 分钟？\\
\hspace{4mm}\small A：\textbf{口径不一致}（README 自爆）。RMQ TTL 30min，Cron 兜底 15min，实际生效 = min(两者) = 15min。修法：统一到 \texttt{consts.OrderCancelTimeout}。代码：\texttt{repository/rabbitmq/order\_delay.go:22} + \texttt{service/order\_task.go:23}。

\vspace{1mm}
\textbf{Q7}：用户退款后库存怎么回？\\
\hspace{4mm}\small A：当前\textbf{没有自动回流}（路线图）。SRE 用 \texttt{InitStock} 人工覆盖。退款 outbox → \texttt{ReleaseFromCommitted} Lua 是 P1 路线图。

\vspace{1mm}
\textbf{Q8}：商家想加库存怎么操作？\\
\hspace{4mm}\small A：当前没有商家自助 API，得找运营手动改 \texttt{product.num} + 跑 \texttt{InitStock}。商家自助补货 + 缺货 SKU 看板 + 库存预警事件三件是"商家入驻"P1 路线图。

\vspace{1mm}
\textbf{Q9}：怎么证明 500 并发不会超卖？\\
\hspace{4mm}\small A：\texttt{NoOversellUnderConcurrency} 单测写死 \texttt{success == exactly 100}，少卖 / 超卖 CI 都红。本质是 Redis 单线程 + Lua 原子。代码：\texttt{inventory\_test.go:52--82}。
\end{frame}
...
\begin{description}\setlength\itemsep{-1mm}
  ...
  \item[OrderCreate reserve + Saga 补偿]\srcnote{service/order.go:57--86}
  \item[CancelUnpaidOrder release（幂等）]\srcnote{service/order\_cancel.go:14--63}
\end{description}
\textbf{关单口径（双口径自爆）}
\begin{description}\setlength\itemsep{-1mm}
  \item[RMQ TTL 30 分钟]\srcnote{repository/rabbitmq/order\_delay.go:22 OrderCancelDelay}
  \item[Cron 兜底 15 分钟]\srcnote{service/order\_task.go:23 GetTimeoutOrders(15, 100)}
\end{description}
\textbf{业务码（诚实标注）}
\begin{description}\setlength\itemsep{-1mm}
  \item[50001 = ErrorOss，非缺货码]\srcnote{pkg/e/code.go:49}
  \item[库存错误待映射业务码 80001]\srcnote{repository/cache/inventory.go:25 ErrStockInsufficient}
\end{description}
\textbf{测试 \& 压测}
\begin{description}\setlength\itemsep{-1mm}
