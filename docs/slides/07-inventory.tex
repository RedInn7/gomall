...
\begin{frame}[shrink]{同一个"库存数字"，五个角色五种关心点}
...
\begin{frame}[shrink]{下单到付款，中间那 15 分钟的业务窗}
\begin{itemize}
...
\begin{frame}[shrink]{本 deck 七条业务问题}
\begin{enumerate}
...
\begin{frame}[shrink]{1 笔超卖订单 = 一连串的钱、工时、品牌损失}
...
\begin{frame}[shrink]{零超卖怎么证明：500 个用户同时抢 100 件}
\begin{tabular}{@{}lr@{}}
\toprule
业务指标 & 实测 \\
\midrule
...
\begin{frame}[shrink]{尾延迟为什么是业务问题：Redis Lua vs DB 行锁}
\begin{tabular}{@{}lrr@{}}
\toprule
...
\begin{frame}[shrink]{available / reserved 翻译成业务话}
...
\begin{frame}[shrink]{物理库存 vs 可售库存：商家 / 客服 / 财务的语言对齐}
\footnotesize
...
in\_transit         & 物流 (WMS)      & 在途，归 WMS 仓储系统 \\
...
\begin{frame}[shrink]{三态生命周期：业务侧四件事}
...
\begin{frame}[fragile,shrink]{8 行 Lua = "宁可误锁不可超卖"的工程化}
...
\textbf{业务码口径}：库存不足在代码里是 \texttt{cache.ErrStockInsufficient} 错误对象，由上层 handler 统一映射成业务码后下发（注意 50001 是 \texttt{ErrorOss} 对象存储错误，与库存无关，两者语义独立）。业务码维度的细分（如把库存类错误单列到 90001 段，与满减引擎占用的 80001--80003 区隔开）属业务码体系的演进项。
\srcnote{pkg/e/code.go 业务码总表；repository/cache/inventory.go:25 ErrStockInsufficient 错误对象}
\end{frame}

\begin{frame}[fragile,shrink]{下单链路 + 失败自愈：业务侧三步走}
...
\begin{frame}[shrink]{业务事故：DB 落库失败而预占未退 = 库存泄漏}
\begin{center}
...
\begin{frame}[fragile,shrink]{commit / release 成对设计的业务理由}
...
\begin{frame}[shrink]{关单超时定多少分钟：商家利用率 vs 用户犹豫时间}
...
\end{tabular}
\vspace{2mm}
\small gomall 当前实现是\textbf{双保险双口径}，各司其职：\\
\quad - RMQ TTL 延迟队列 \texttt{OrderCancelDelay = 30 \* time.Minute}：主路径，到点精准投递关单 \srcnote{repository/rabbitmq/order\_delay.go:22}\\
\quad - Cron 兜底扫表 \texttt{GetTimeoutOrders(15, 100)}：DB 驱动的兜底，扫 WaitPay 超过 15min 的订单 \srcnote{internal/order/task.go:24}\\
两路都汇到同一个幂等入口 \texttt{CancelUnpaidOrder}：先到者关单，后到者命中 \texttt{WHERE type=待付款} 条件 update 自然 no-op，\textbf{库存不会被退两次}。Cron 口径取得更短，是\textbf{有意让兜底先于主路径触发}—— 即便建单后延迟消息因进程崩溃丢失，DB 扫表仍能在 15min 内补偿关单、退还预占，避免库存被永久占死。
\srcnote{internal/order/task.go:30--42 Cron 兜底走与 MQ 完全相同的幂等关单逻辑}
\end{frame}

\begin{frame}[fragile,shrink]{关单释放的幂等保证：客服可以放心"再点一次"}
...
\begin{frame}[shrink]{关单延迟链路：MQ 主动 + Cron 兜底}
\begin{tikzpicture}[node distance=8mm and 14mm]
  \node[node-box] (ord) {下单};
...
\begin{frame}[shrink]{业务场景：Redis 重启，库存数字从哪儿来？}
\begin{tikzpicture}[node distance=8mm and 16mm]
...
\begin{frame}[fragile,shrink]{SeedFromDB 主循环：游标分页 + EXISTS 跳过}
...
\begin{frame}[shrink]{重建窗口里能不能下单？客服怎么解释？}
...
\begin{frame}[shrink]{工单 A："我加购后还有 5 件，下单时说缺货"}
...
\begin{frame}[shrink]{工单 B："我下单成功了为什么显示缺货"}
...
\begin{frame}[shrink]{工单 C："我退款了为什么库存没还回去"}
\textbf{业务真相}：用户在支付后退款，资金侧的退款状态机已经完整落地（\texttt{internal/refund/service.go}，含 \texttt{order.refunded} outbox 事件按实付口径退款）。库存侧的"从 committed 回到 available"则是\textbf{有意与退货物流解耦}的一段 —— 物理货是否回仓、是否残次，要等仓储确认，不能在退款那一刻就盲目把库存放回可售池。
\vspace{1mm}
\begin{itemize}
  \item \textbf{现行处理}：库存回填由商家 / SRE 在确认货品状态后操作 \texttt{InitStock} 覆盖 available（这条路径恪守"误锁可接受、超卖不可"的红线）。
  \item \textbf{话术}：\textit{"您的退款已生效，库存回填会在 24 小时内由商家确认完成（避免在途货退回错位）。"}
  \item \textbf{扩展方向}：\\
  \quad - \texttt{order.refunded} outbox 事件已就位 $\to$ 由消费侧触发 \texttt{ReleaseFromCommitted} Lua（commit 的逆操作）做自动回流。\\
  \quad - 退货寄回 / 残次品分流 $\to$ 与 WMS 仓储系统对接。\\
  \quad - 售后工单 $\to$ 评价 $\to$ 二次售后的全链路见 deck 09。
\end{itemize}
\srcnote{退款状态机已实现 internal/refund/service.go:94--135 ApproveRefund；库存回流扩展点：ReleaseFromCommitted Lua}
\end{frame}

\begin{frame}[shrink]{业务码 / 错误的客服对照表（含诚实标注）}
\footnotesize
\begin{center}
...
状态码 / 错误 & 现行实现 & 客服话术 \\
\midrule
\textbf{ErrStockInsufficient}（库存不足） & Go 错误对象，handler 统一映射下发 & "本商品已售罄" \\
...
\end{center}
\smallskip
\keypoint{\footnotesize \textbf{口径说明}：50001 在本仓库是 \texttt{ErrorOss}（对象存储错误），与库存语义独立；库存不足由 \texttt{cache.ErrStockInsufficient} 错误对象经 handler 统一映射下发。业务码维度的进一步细分（如把库存类错误单列到 \texttt{90001} / \texttt{90002} 段，与满减引擎占用的 80001--80003 区隔），可让客服 / 监控 / 链路追踪更精准地分类。}
\srcnote{pkg/e/code.go：50001=ErrorOss / 70001 限流 / 70002 熔断；repository/cache/inventory.go:25 ErrStockInsufficient 错误对象}
\end{frame}
...
\begin{frame}[shrink]{库存子系统的业务 SLO}
\begin{tabular}{@{}lll@{}}
\toprule
业务承诺 & 指标 & 目标 \\
\midrule
...
\begin{frame}[shrink]{发生超卖了：4 小时应急 SOP}
\footnotesize
...
\begin{frame}[shrink]{本库存系统不做什么（诚实清单）}
\begin{itemize}
  ...
  \item \textbf{不做跨仓调拨 / 在途回流}：发货后的在途、退货寄回、残次品分流统一归 WMS 仓储系统，不在本库存模块范围内。
  ...
  \item \textbf{不做防黄牛单用户上限}：库存预扣按件扣减、不绑单用户限购；需要时可直接复用优惠券 \texttt{claimScript} 里成熟的 owned $<$ perUser 范式（repository/cache/coupon.go:30--44）。
\end{itemize}
\srcnote{docs/architecture/feature-matrix.md 路线图全表}
\end{frame}

\begin{frame}[shrink]{业务路线图：库存能力扩展}
...
\begin{frame}[shrink]{业务承诺：超卖 vs 误锁，两条不同的红线}
\begin{tabular}{@{}lll@{}}
\toprule
事故 & 用户体感 & 业务代价 \\
\midrule
...
\begin{frame}[shrink]{Q\&A（上）：业务承诺 / 工单 / 应急}
...
\hspace{4mm} A：\textbf{不是}。50001 在本仓库是 \texttt{ErrorOss}（对象存储错误），与库存语义独立。库存不足由 \texttt{cache.ErrStockInsufficient} 错误对象经 handler 统一映射下发；业务码维度可进一步细分到 90001 段（与满减引擎占用的 80001--80003 区隔）。代码：\texttt{pkg/e/code.go} + \texttt{repository/cache/inventory.go:25}。

\vspace{1mm}
\textbf{Q6}：当前关单超时是 15 还是 30 分钟？\\
\hspace{4mm} A：\textbf{双保险双口径}。RMQ TTL 30min 是主路径精准关单，Cron 兜底 15min 是 DB 驱动的兜底扫表；Cron 口径更短是\textbf{有意让兜底先触发}，两路汇到同一个幂等 \texttt{CancelUnpaidOrder}，后到者 no-op、库存不会退两次。代码：\texttt{repository/rabbitmq/order\_delay.go:22} + \texttt{internal/order/task.go:24}。

\vspace{1mm}
\textbf{Q7}：用户退款后库存怎么回？\\
\hspace{4mm} A：资金侧退款状态机已完整落地；库存回流\textbf{有意与退货物流解耦}，由商家 / SRE 确认货品状态后用 \texttt{InitStock} 覆盖（恪守"误锁可接受、超卖不可"）。\texttt{order.refunded} outbox 事件已就位，消费侧 \texttt{ReleaseFromCommitted} Lua 自动回流是后续扩展项。
...
\textbf{关单口径（双保险双口径）}
\begin{description}\setlength\itemsep{-1mm}
  \item[RMQ TTL 30 分钟（主路径）]\srcnote{repository/rabbitmq/order\_delay.go:22 OrderCancelDelay}
  \item[Cron 兜底 15 分钟（DB 驱动兜底）]\srcnote{internal/order/task.go:24 GetTimeoutOrders(15, 100)}
\end{description}
\textbf{业务码（口径说明）}
\begin{description}\setlength\itemsep{-1mm}
  \item[50001 = ErrorOss，与库存语义独立]\srcnote{pkg/e/code.go:49}
  \item[库存错误统一映射，可细分 90001 段]\srcnote{repository/cache/inventory.go:25 ErrStockInsufficient}
\end{description}
\textbf{测试 \& 压测}
\begin{description}\setlength\itemsep{-1mm}
