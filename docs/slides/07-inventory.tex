...
\quad - RMQ TTL 延迟队列 \texttt{OrderCancelDelay = 30 \* time.Minute}：主路径，到点精准投递关单 \\
\quad - Cron 兜底扫表 \texttt{GetTimeoutOrders(15, 100)}：DB 驱动的兜底，扫 WaitPay 超过 15min 的订单 \\
...
\begin{description}\setlength\itemsep{-1mm}
  \item[reserve / commit / release Lua]
  \item[InitStock + Snapshot]
  \item[SeedFromDB + 启动注入]
  \item[OrderCreate reserve + Saga 补偿]
  \item[CancelUnpaidOrder release（幂等）]
\end{description}
\textbf{关单口径（双保险双口径）}
\begin{description}\setlength\itemsep{-1mm}
  \item[RMQ TTL 30 分钟（主路径）]
  \item[Cron 兜底 15 分钟（DB 驱动兜底）]
\end{description}
\textbf{业务码（口径说明）}
\begin{description}\setlength\itemsep{-1mm}
  \item[50001 = ErrorOss，与库存语义独立]
  \item[库存错误统一映射，可细分 90001 段]
\end{description}
\textbf{测试 \& 压测}
\begin{description}\setlength\itemsep{-1mm}
  \item[NoOversell 500 抢 100 = 恰好 100]
  \item[Lua vs DB 尾延迟 136ms vs 453ms]
\end{description}
