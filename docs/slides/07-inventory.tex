...
\end{itemize}
\srcnote{repository/cache/inventory.go:11--14；internal/order/task.go:23 现行口径 15 分钟}
\end{frame}
...
\begin{lstlisting}[language=Go,firstnumber=58,
  caption={\srcnote{internal/order/service.go:58--86 reserve + tx + 补偿 release}}]
...
\srcnote{internal/order/service.go:79--85 显式调用 ReleaseReservation 补偿}
\end{frame}
...
\quad - Cron 兜底扫表 \texttt{GetTimeoutOrders(15, 100)} 视 15min 为超时 \srcnote{internal/order/task.go:23}\\
...
\begin{lstlisting}[language=Go,firstnumber=31,
  caption={\srcnote{internal/order/cancel.go:31--62 关单 + 库存退回}}]
err = baseDao.DB.Transaction(func(tx *gorm.DB) error {
    ...
    closed = true
    return outbox.NewOutboxDaoByDB(tx).Insert(/* OrderCancelled */)
})
if err != nil { return err }
...
\srcnote{repository/rabbitmq/order\_delay.go:22 OrderCancelDelay；internal/order/task.go:23 Cron 15min}
\end{frame}
...
\end{itemize}
\srcnote{repository/cache/inventory.go:127--138 GetStockSnapshot；internal/order/cancel.go:59 release 调用}
\end{frame}
...
\end{itemize}
\srcnote{当前未实现售后链路；扩展点：internal/refund/service.go + ReleaseFromCommitted Lua}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{不做防黄牛单用户上限}：reserveScript 暂未加 owned $<$ perUser check（internal/coupon/service.go:30--44 已实现该范式可复用）。
\end{itemize}
...
\srcnote{internal/coupon/service.go:30--44 已实现的 owned check 可复用到 reserveScript}
\end{frame}
...
\hspace{4mm} A：\textbf{口径不一致}（README 自爆）。RMQ TTL 30min，Cron 兜底 15min，实际生效 = min(两者) = 15min。修法：统一到 \texttt{consts.OrderCancelTimeout}。代码：\texttt{repository/rabbitmq/order\_delay.go:22} + \texttt{internal/order/task.go:23}。
...
\begin{description}\setlength\itemsep{-1mm}
  ...
  \item[OrderCreate reserve + Saga 补偿]\srcnote{internal/order/service.go:57--86}
  \item[CancelUnpaidOrder release（幂等）]\srcnote{internal/order/cancel.go:14--63}
\end{description}
\textbf{关单口径（双口径自爆）}
\begin{description}\setlength\itemsep{-1mm}
  \item[RMQ TTL 30 分钟]\srcnote{repository/rabbitmq/order\_delay.go:22 OrderCancelDelay}
  \item[Cron 兜底 15 分钟]\srcnote{internal/order/task.go:23 GetTimeoutOrders(15, 100)}
\end{description}
\textbf{业务码（诚实标注）}
\begin{description}\setlength\itemsep{-1mm}
