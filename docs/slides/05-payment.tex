...
\begin{lstlisting}[language=Go,firstnumber=69,caption={}]
...
\begin{lstlisting}[language=Go,firstnumber=17,caption={}]
...
\begin{description}
  \item[PayDown 主体]
  \item[资金台账 / 复式记账]
  \item[实付口径 FinalCents]
  \item[台账 AutoMigrate 注册]
  \item[AES 加解密]
  \item[CircuitBreaker]
  \item[Idempotency]
  \item[paydown 路由 + 中间件栈]
  \item[支付失败 Saga]
  \item[业务码表]
  \item[压测数字]
  \item[业务边界 / SLO]
