...
\srcnote{internal/payment/service.go:53--186}
\end{frame}

\begin{frame}[fragile]{PayDown 的关键 18 行}
\begin{lstlisting}[language=Go,firstnumber=56,
  caption={\srcnote{internal/payment/service.go:56--103 节选}}]
...
userMoney, err := buyer.DecryptMoney(req.Key)  // 密码错 -> ErrMoneyKeyIncorrect
if err != nil { return err }
...
buyer.Money, err = buyer.EncryptMoney(req.Key) // 同密码重新加密
if err != nil { return err }
err = userDao.UpdateUserById(uId, buyer)       // 落 DB
\end{lstlisting}
\end{frame}

\begin{frame}{逐行讲解：扣款这一步的业务含义}
\begin{itemize}\small
  \item \textbf{62 行 OrderWaitPay 校验在 TX 内}：不能 TX 外读再进事务 —— 否则典型 TOCTOU，两并发支付都看到待付款 $\to$ 双扣。这是"重复扣款"客诉的根因之一。
  \item \textbf{82 行 \texttt{DecryptMoney(req.Key)}}：密码错 $\to$ 底层去填充 panic / 解出乱码，统一折叠为 \texttt{ErrMoneyKeyIncorrect} $\to$ \texttt{return err} $\to$ TX 回滚。用户拿到明确的"支付密码错误"，\textbf{穷举防护交给前置的限流 + 熔断 + 幂等三层}，不再靠错误信息含混。
  \item \textbf{92--93 行 重新加密}：\texttt{Money} 是 string 不能直接减；必须 decrypt $\to$ int64 减 $\to$ encrypt。整段在 TX 里，失败回滚干净。
  \item \textbf{金额不足也 \texttt{return err}}：让 TX 走 rollback，\texttt{order.Type} 不会推进，客户端可原 OrderId + 充值后重试 —— 客服话术："充值后原订单可直接付款，30 分钟内有效"。
\end{itemize}
\srcnote{internal/payment/service.go:56--103；internal/user/model.go:15--16 ErrMoneyKeyIncorrect}
\end{frame}

\begin{frame}[fragile]{outbox 写在事务尾 —— 业务承诺的兑现点}
\begin{lstlisting}[language=Go,firstnumber=175,
  caption={\srcnote{internal/payment/service.go:175--185}}]
...
\srcnote{internal/payment/service.go:193--198 cache.CommitReservation；deck 11 outbox 主题}
\end{frame}
...
\bottomrule
\end{tabular}
\srcnote{internal/payment/service.go:175--185；internal/shared/outbox/repo.go}
\end{frame}
...
\scriptsize
\begin{description}
  \item[PayDown 主体]\srcnote{internal/payment/service.go:35--201}
