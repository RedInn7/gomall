...
\begin{itemize}
  ...
  \item 客诉一通电话客服平均 \textbf{8 分钟}：日 1000 单掉单 → 客服需 \textbf{2.2 个人日} 兜底。
...
\begin{itemize}\small
  ...
  \item 下游：支付成功 → outbox 事件 \texttt{order.paid} → 通知发货 / 优惠券核销 / 索引更新。
  \item 失败兜底：30 分钟未付 → Cron + RMQ 双保险关单（deck 09）。
\end{itemize}
\srcnote{routes/router.go:123--130；service/order\_cancel.go}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{内控}：明文金额一旦被 \texttt{log.Debug(user)} 序列化 → 上传 ELK → 全公司有 ELK 账号的人都能看。
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{商家结算单脱敏}：商家后台看到的金额是 \textbf{明文（业务必须）}，但需要客服 / 平台审计能力时，DB 直查永远只能拿到密文 —— 一次明文访问必须走 \texttt{DecryptMoney} 函数 → 可被审计日志覆盖。
...
\end{itemize}
\srcnote{repository/db/model/user.go:59--79；pkg/utils/encryption/money\_encrypt.go}
\end{frame}
...
\end{itemize}
\srcnote{repository/db/model/user.go:59--79；config/locales/config.yaml.example 中 MoneySecret 字段}
\end{frame}
...
\begin{itemize}\small
  ...
  \item \textbf{错误传播}：密码错 → 乱码 → \texttt{ToInt64} 返 0 → 金额校验失败 → TX 回滚。表象与"余额不足"一致，防穷举区分。
\end{itemize}
\srcnote{repository/db/model/user.go:59--79}
\end{frame}
...
\begin{itemize}
  \item \textbf{实名 KYC 的合规成本}：要接公安一二三要素接口（人均 0.5--2 元 / 次），运营商短信通道（0.05 元 / 条），并备案"信息处理者"身份 → \textbf{合规审批 6+ 月}。
  \item \textbf{业务收益对照}：电商扣款金额一般 < 1000 元 / 笔，风险敞口与全链 KYC 不匹配。
  \item \textbf{客诉路径}：短信验证码到达率 \textbf{96--98\%} → 每天 \textbf{2--4\%} 用户因收不到验证码放弃支付。
  \item \textbf{业务侧的选择}：用\textbf{业务侧 6 位支付密码} —— 不存盘、入口校验、AES 入参，覆盖 95\% 场景；KYC + 短信留给"大额提现 / Web3 / 银行卡绑定"路线图。
\end{itemize}
\srcnote{service/payment.go:42--45；consts/user.go EncryptMoneyKeyLength}
\end{frame}
...
\begin{itemize}\small
  ...
  \item \textbf{为何还能用}：因为接口前面挂了 \textbf{CircuitBreaker + RateLimit + Idempotency} 三层，单 IP 单用户实际穷举速率 < 0.5 RPS → 跑完平均 \textbf{23 天}，余额早被风控冻结。
  \item 路线图：路线图阶段对\textbf{风控引擎}（异地 IP / 行为模型）做对接。
\end{itemize}
\srcnote{middleware/ratelimit.go；middleware/circuitbreaker.go；service/payment.go:42--45}
\end{frame}
...
\begin{itemize}\small
  \item \textbf{57 行 UnPaid 校验在 TX 内}：不能 TX 外读再进事务 —— 否则典型 TOCTOU，两并发支付都看到 UnPaid → 双扣。这是"重复扣款"客诉的根因之一。
  \item \textbf{65 行 \texttt{DecryptMoney(req.Key)}}：密码错 → 解出乱码 → \texttt{userMoney} 几乎一定不够 → 走"金额不足"分支。\textbf{密码错和余额不足表象一致}，防穷举 —— 这是产品和安全协商出的结果。
...
\srcnote{service/payment.go:193--197 cache.CommitReservation；deck 11 outbox 主题}
\end{frame}
...
\bottomrule
\end{tabular}
\srcnote{service/payment.go:174--184；repository/db/dao/outbox.go}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{路线图 step 3}：第三方回调 → 验签 → 翻订单状态 → outbox \texttt{order.paid} —— 与本 deck 主链路衔接。
  \item \textbf{已就位}：熔断器、幂等、outbox、业务码 70002 都已经在生产路径上跑过了 —— \textbf{接入真实网关不改架构，只换 Gateway 实现}。
\end{itemize}
\srcnote{service/payment.go:35--200；routes/router.go:123--130}
\end{frame}
...
\begin{itemize}\small
  \item \textbf{Closed 零开销}：只一次原子 Load → return nil，不抢锁不计数。前提是 paydown 不能因为加了熔断器就比 /ping 慢一个数量级。
  ...
  \item \textbf{report() 配套}：3 个全成功 → 回 Closed；任一失败 → 立即重新 Open。运营视角："恢复了就立刻全放，再挂就立刻再断"。
\end{itemize}
...
\vspace{1mm}
\begin{itemize}\small
  \item \textbf{只看 5xx 不看 4xx}：4xx 是密码错 / 余额不足等业务正常路径 → 不算熔断信号。200 + 业务码 70001/70002 也不算失败 —— 业务码是\textbf{已知的拒绝}，不是\textbf{下游真挂}。
...
时刻 & 谁做什么 \\
\midrule
T+0s & 监控告警：paydown 5xx 连续 $\geq$ 5 → \texttt{state=Open} \\
...
\begin{itemize}
  ...
  \item 自动监控触发 → 给受影响用户发"5 元无门槛券，48h 有效"。
  \item 行业数据：补偿券回赠 → \textbf{40\%+} 用户当周回访下单。
...
\begin{itemize}
  ...
  \item 单笔金额 100 元 → 平台成本 \textbf{30--50 元}（人工 + 退款手续费 + 口碑损失）。
  \item 防"重复扣款"的 ROI 极高 → 任何一笔被幂等拦下来都是赚的。
\end{itemize}
...
\begin{itemize}
  ...
  \item 返回 \texttt{200 + status=70002}，HTTP 仍 200 → app 端老网关不会按 5xx 处理。
\end{itemize}
...
\begin{itemize}\small
  ...
  \item \textbf{失败回滚到 init}：4xx 或 \texttt{c.Errors} → 同 IK 还能再试（业务失败 $\neq$ 永久失败）—— 对应客诉"我密码输错了重输一次不行"的合理诉求。
  \item \textbf{成功提交到 done}：把完整 JSON 写进 Redis done 态 → 后续同 IK 由 Lua 直接回放，\textbf{业务代码不再执行}—— 这是"重复扣款"客诉归零的关键。
\end{itemize}
...
\srcnote{pkg/e/code.go:35--58；service/payment.go:52--184}
\end{frame}
...
\begin{itemize}
  ...
  \item 30min 未续支付 → Cron + RMQ 双保险（deck 09）→ \texttt{CancelUnpaidOrder}。
  \item Saga 不回写 \texttt{product.Num}（从未扣过），仅释放 Redis \texttt{reserved}。
\end{itemize}
\srcnote{service/order\_cancel.go:14--62；deck 09 关单衔接}
\end{frame}
...
\end{lstlisting}
\vspace{1mm}
\small \textbf{CloseOrderWithCheck} 用 \texttt{WHERE type=UnPaid} 条件 update，第二次调用自然 affected=0 → 函数变 no-op。这是\textbf{用 DB 行做幂等键}。Redis 释放走 TX 外，失败仅记日志，对账兜底。业务承诺：\textbf{用户 / 客服 / Cron / RMQ 四个调用方都能反复点关单，库存绝不漏放}。
\end{frame}
...
\srcnote{routes/router.go:123--130 + service/payment.go:35--200}
\end{frame}
...
\hspace{4mm} A：密码错 → 解出乱码 → int64 几乎不够 → 走金额不足分支。\textbf{有意为之}：防穷举区分。安全和产品协商出的结果。
...
\scriptsize
\begin{description}
  \item[PayDown 主体]\srcnote{service/payment.go:35--200}
