...
\begin{frame}{PayDown 在一次事务里做的几件事}
...
\begin{frame}{PayDown 结算的标准时序：六方一次扣款}
\centering
...
\begin{frame}[fragile]{PayDown 扣款这一段的真实代码}
...
\begin{frame}{业务困局：余额是一个密文列，扣款不留痕}
\begin{itemize}
...
\begin{frame}{解法：复式记账（debit / credit）流水台账}
\begin{itemize}\small
...
\begin{frame}[fragile]{表结构：account\_transaction}
...
\begin{frame}{一次支付的双写：余额列 + 流水台账（同事务）}
...
\begin{frame}[fragile]{PayDown 同事务追加 debit / credit 两条流水}
...
\begin{frame}{(ref\_order\_id, direction) 唯一索引：同单只入账一次}
\begin{itemize}\small
...
\begin{frame}{资金台账全景：所有动钱路径都进复式记账台账}
\centering
...
\begin{frame}{满减实付口径：为什么不能用折前价计费 / 退款}
\begin{itemize}\small
...
\begin{frame}[fragile]{退款侧的实付口径（与支付侧对齐）}
