...
\begin{tikzpicture}[node distance=2mm and 2mm,every node/.append style={minimum width=11mm,font=\scriptsize}]
  \node[node-box,node-ok] (u1) {用户 1};
  \node[node-box,node-ok,right=of u1] (u2) {用户 2};
  \node[node-box,node-ok,right=of u2] (u3) {用户 3};
  \node[node-box,node-ok,right=of u3] (u4) {用户 4};
  \node[node-box,node-ok,right=of u4] (u5) {用户 5};
  \node[node-box,node-ok,right=of u5] (u6) {\dots 8};
  \node[node-box,node-bad,right=of u6] (u7) {\textbf{用户 9}};
...
\end{itemize}
\srcnote{repository/db/dao/order.go:35--76 ListOrderByCondition 含首页 5min 缓存}
\end{frame}
...
\vspace{2mm}
\begin{itemize}
  \item 四个角色看同一个 bug 的不同切面，单看任何一个都不触发 P0。
  \item gomall 现状：四个面都看不见——没 RUM / 没客诉关键词分类 / 监控没看 p95。
  \item 推送场景：10 万人点开，8.3 RPS 只服务 500 人，运营问"扛多少 RPS"，答案应是 58K 而不是 8.3。
\end{itemize}
\srcnote{routes/router.go:78--79 同业务两个 endpoint}
\end{frame}
...
\begin{description}
  ...
  \item[请求 LastId 字段]\srcnote{types/common.go:5--9 BasePage；types/order.go:43--46 OrderListResp 回写}
