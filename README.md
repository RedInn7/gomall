...
**支付结算（资金台账 + 折后实付）** —— 锁双方 → 校验支付密码 → 买家扣款 / 卖家入账并各写一条复式记账流水 → 扣库存 → 改单 → 写 Outbox，同一事务原子；Web3 路径走 Escrow + EVM 链上对账。
...
`internal/order/cancel.go` + `internal/order/task.go` + `initialize/cron.go`。**真实 bug**：现版本 cron 表达式 `* */5 * * * *` 实际触发 60×/5min（应为 `0 */5 * * * *`），靠幂等掩盖 60× DB 压力。
...
- `initialize/cron.go:21` cron 表达式 `* */5 * * * *` 实际触发 60×/5min（应 `0 */5 * * * *`），靠幂等掩盖
- `routes/router.go:115` `orders/delete` 走 `DeleteOrderById` 只 soft-delete，**没释放 Redis reserved、漏写 outbox** = 库存泄漏
- 三处订单超时口径不一致：`repository/rabbitmq/order_delay.go:22` 30min / `internal/order/task.go:24` 15min / `internal/order/service.go:143-147` 15min
