...
func RegisterPromoRoutes(public, authed, admin *gin.RouterGroup) {
	...
	if public != nil {
		public.POST("/promo/calculate", api.PromoCalculateHandler())
	}
	if admin != nil {
		admin.GET("/promo/rules", api.AdminListPromoRulesHandler())
		admin.POST("/promo/rules", api.AdminCreatePromoRuleHandler())
		admin.PATCH("/promo/rules/:id/stop", api.AdminStopPromoRuleHandler())
	}
}
