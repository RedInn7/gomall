...
这组题故意把异常放在正常路径中间：重复消息、第二条流水失败、状态更新失败、退款抢先发生，以及历史账目已经不一致。只让成功路径跑通不算完成。

| 作业 | 主要问题 |
| --- | --- |
| 05.01 结算状态守卫 | 当前订单和资金状态是否允许放款 |
| 05.02 原子卖家结算 | 余额、双边流水和状态怎样一起提交 |
| 05.03 并发幂等结算 | 大量重复事件怎样保证只入账一次 |
| 05.04 退款与结算竞争 | 同一笔托管资金最终应该流向谁 |
| 05.05 结算对账 | 怎样从订单、清算单、流水和余额中找出坏账 |
