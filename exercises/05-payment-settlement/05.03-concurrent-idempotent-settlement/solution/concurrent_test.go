...
func TestConcurrentRetriesPayExactlyOnce(t *testing.T) {
	s := NewStore(10000, Clearing{77, 9, 1250, StatusCleared})
	...
	got := s.Snapshot()
	if got.Escrow != 8750 || got.SellerBalances[9] != 1250 || len(got.Entries) != 1 || got.Clearings[77].Status != StatusSettled {
...
func TestDifferentOrdersToSameSellerDoNotLoseUpdates(t *testing.T) {
	...
	wg.Wait()
	got := s.Snapshot()
	if got.Escrow != 9200 || got.SellerBalances[42] != 800 || len(got.Entries) != 80 {
		t.Fatalf("snapshot=%+v", got)
	}
}
